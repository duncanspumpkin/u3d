...
extern "C" I32 IFXAPI_CALLTYPE IFXOSSocketOpen()
{
    int descriptor = -1;
...
extern "C" IFXRESULT IFXAPI_CALLTYPE IFXOSSocketClose(int descriptor)
{
...
extern "C" IFXRESULT IFXAPI_CALLTYPE IFXOSSocketConnect(
    int descriptor,
    const char* pHost,
    U16 port)
{
    ...
    if (IFXSUCCESS(result))
    {
        if (SOCKET_ERROR == connect(descriptor, (struct sockaddr*)&addr, sizeof(addr)))
        {
            IFXTRACE_GENERIC(
...
extern "C" IFXRESULT IFXAPI_CALLTYPE IFXOSSocketSetOptions(
    int descriptor,
    U32 option,
    const char* pBuffer,
    int bufferLength)
...
extern "C" IFXRESULT IFXAPI_CALLTYPE IFXOSSocketGetOptions(
    int descriptor,
    U32 option,
    char* pBuffer,
    int* pBufferLength)
