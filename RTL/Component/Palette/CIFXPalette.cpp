...
IFXRESULT CIFXPalette::QueryInterface(IFXREFIID interfaceId, void** ppInterface)
{
    ...
    if (ppInterface)
    {
        if (interfaceId == IID_IFXPalette || interfaceId == IID_IFXUnknown)
        {
...
IFXRESULT CIFXPalette::Initialize(U32 uInitialSize, U32 uGrowthSize)
{
    ...
    {
        IFXRESULT iResult;
        iResult = IFXCreateComponent(CID_IFXHashMap, IID_IFXHashMap, (void**)&m_pHashMap);
...
IFXRESULT CIFXPalette::Add(IFXString* pPaletteEntryName, U32* pPaletteEntryId)
{
    ...
    if (IFXSUCCESS(iResult) && iResult != IFX_W_ALREADY_EXISTS)
    {
        ...
        {
            U32 uNewPaletteSize = 0;
            void* pReallocBlock = NULL;
            if (m_uGrowthSize == 0)
            {
...
IFXRESULT CIFXPalette::DeleteById(U32 uPaletteEntryId)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        if (m_pPalette[uPaletteEntryId].m_pSimpleObject && uPaletteEntryId != IFX_NULL_RESOURCE)
        {
        ...
        if (pObject)
        {
            ...
            if (IFXSUCCESS(pObject->QueryInterface(IID_IFXModifier, (void**)&pMod2)))
            {
                if (IFXFAILURE(pMod2->GetModifierChain(&(m_pPalette[uIndex].m_pModChain))))
                {
                    ...
                    iResult = IFXCreateComponent(CID_IFXModifierChain, IID_IFXModifierChain, (void**)&(m_pPalette[uIndex].m_pModChain));
                    if (IFXSUCCESS(iResult))
                    {
                        ...
                        if (pResourceClient)
                        {
                            ...
                            if (IFXSUCCESS(iResult))
                            {
                                ...
                                if (IFXSUCCESS(iResult))
                                {
                                    pPalette->GetResourcePtr(uResourceIndex, IID_IFXModifier, (void**)&pResourceMod);
                                }
                                if (pResourceMod)
                                {
                                    ...
                                    if IFXSUCCESS (iResult)
                                    {
                                        iResult = m_pPalette[uIndex].m_pModChain->PrependModifierChain(pResourceChain);
                                    }
                                }
            ...
            {
                // Notify observers
                PostChanges(m_pPalette[uIndex].m_bHidden ? IFX_SUBJECT_PALETTE_ADD_HIDDEN : IFX_SUBJECT_PALETTE_ADD);
            }
        }
        else
        {
            ...
            {
                // Notify observers
                PostChanges(m_pPalette[uIndex].m_bHidden ? IFX_SUBJECT_PALETTE_REMOVE_HIDDEN : IFX_SUBJECT_PALETTE_REMOVE);
            }
        }
    }
    return iResult;
...
//---------------------------------------------------------------------------
IFXRESULT IFXAPI_CALLTYPE CIFXPalette_Factory(IFXREFIID interfaceId, void** ppInterface)
{
    IFXRESULT result;
...
IFXRESULT CIFXPalette::SetResourceObserver(U32 uCurrentIndex, U32 uNewIndex, IFXModel* pModel)
{
    ...
    if (pModel)
    {
        ...
        if (uCurrentIndex != uNewIndex)
        {
            ...
            U32 uChangeBits = IFX_SUBJECT_PALETTE_ADD_RESOURCE | IFX_SUBJECT_PALETTE_REMOVE_RESOURCE;
            ...
            if (IFXSUCCESS(result))
            {
                if (uCurrentIndex == IFX_NULL_RESOURCE)
                {
                    ...
                    if (m_pPalette[uNewIndex].m_pSimpleObject == NULL)
                    {
                        ...
                        result = IFXCreateComponent(CID_IFXSimpleObject, IID_IFXSubject, (void**)&(m_pPalette[uNewIndex].m_pSimpleObject));
                    }

                    if (IFXSUCCESS(result))
                    {
                        result = m_pPalette[uNewIndex].m_pSimpleObject->Attach(pObserver, uChangeBits);
                    }
                    if (IFXSUCCESS(result))
                    {
