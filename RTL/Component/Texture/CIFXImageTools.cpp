...
//---------------------------------------------------------------------------
IFXRESULT IFXAPI_CALLTYPE CIFXImageTools_Factory(IFXREFIID interfaceId, void** ppInterface)
{
    IFXRESULT result;
    ...
    void* pImage, U32 uPriority)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        if (pImageInfo->m_imageType == IFXTextureObject::IFXTEXTUREMAP_FORMAT_BGR24 || pImageInfo->m_imageType == IFXTextureObject::IFXTEXTUREMAP_FORMAT_BGRA32)
        {
            ...
            if (m_pContinuationFormats[0].m_uAttributes == 0)
            {
                ...
                if (IFXSUCCESS(iResult))
                {
                    iResult = BuildBlocksFromImageData(
                        pCompressedData, pImageInfo, contInd1, pDestinationQueue, 1);
        ...
        else if (m_uContinuationImageCount == 2)
        {
            ...
            if (IFXSUCCESS(iResult) && pColorChannels[0] && m_pContinuationFormats[0].m_uAttributes == 0)
            {
                iResult = CompressImage(
                    pColorChannels[0], pImageInfo, contInd1, &pCompressedData);
            ...
            if (IFXSUCCESS(iResult) && pColorChannels[1] && m_pContinuationFormats[1].m_uAttributes == 0)
            {
                iResult = CompressImage(
                    pColorChannels[1], pImageInfo, contInd2, &pCompressedData);

                if (IFXSUCCESS(iResult))
                {
                    iResult = BuildBlocksFromImageData(
                        pCompressedData, pImageInfo, contInd2, pDestinationQueue, 2);
                    if (NULL != pCompressedData)
                    {
                        delete (U8*)pCompressedData;
                        pCompressedData = NULL;
        ...
        else if (m_uContinuationImageCount == 3)
        {
            ...
            if (IFXSUCCESS(iResult) && pColorChannels[2] && m_pContinuationFormats[2].m_uAttributes == 0)
            {
                iResult = CompressImage(
                    pColorChannels[2], pImageInfo, contInd3, &pCompressedData);

                if (IFXSUCCESS(iResult))
                {
                    iResult = BuildBlocksFromImageData(
                        pCompressedData, pImageInfo, contInd3, pDestinationQueue, 3);
                    if (NULL != pCompressedData)
                    {
                        delete (U8*)pCompressedData;
                        pCompressedData = NULL;
        ...
        else if (m_uContinuationImageCount == 4)
        {
            ...
            if (IFXSUCCESS(iResult) && pColorChannels[3] && m_pContinuationFormats[3].m_uAttributes == 0)
            {
                iResult = CompressImage(
                    pColorChannels[3], pImageInfo, contInd4, &pCompressedData);

                if (IFXSUCCESS(iResult))
                {
                    iResult = BuildBlocksFromImageData(
                        pCompressedData, pImageInfo, contInd4, pDestinationQueue, 4);
                    if (NULL != pCompressedData)
                    {
                        delete (U8*)pCompressedData;
                        pCompressedData = NULL;
    ...
    void** ppImage)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        while ((m_uImageReadingState == IFXIMAGEREADING_NOTSTARTED) || (m_uImageReadingState == IFXIMAGEREADING_SUCCESS))
        {
            ...
            while ((m_uCurrentContinuationImage < m_uContinuationImageCount) && (m_pContinuationFormats[m_uCurrentContinuationImage].m_uAttributes == 0))
            {
            ...
            if (m_uCurrentContinuationImage < m_uContinuationImageCount)
            {
                ...
                iResult = IFXCreateComponent(
                    CID_IFXInternetReadBuffer, IID_IFXInet, (void**)&m_pReadBuffer);
                if (IFXSUCCESS(iResult))
                {
        ...
        for (contInd = 0; IFXSUCCESS(iResult) && (contInd < m_uContinuationImageCount); contInd++)
        {
            if (IFXSUCCESS(iResult))
            {
                ...
                if (m_pContinuationFormats[contInd].m_uAttributes == 0)
                {
                    ...
                    iResult = BuildImageFromBlocks(
                        pDataBlockQueueX, pImageInfo, contInd, &pCompressedData);
                    if (IFXSUCCESS(iResult) && pCompressedData)
                    {
                        iResult = DecompressImage(
                            pCompressedData, pImageInfo, contInd, &(pColorChannels[contInd]));
                    }
                    if (pCompressedData)
                    {
                ...
                if (IFXSUCCESS(iResult) && (m_uCurrentContinuationImage == m_uContinuationImageCount))
                {
                    U32 i;
                    for (i = 0; i < m_uContinuationImageCount; i++)
                    {
                        if (IFXSUCCESS(iResult))
                        {
                            if (m_pCompressedImages[i])
                            {
                                ...
                                pImageInfo->m_blockCompressionType[i] = m_pContinuationFormats[i].m_uCompressionType;

                                pImageInfo->m_size = m_uCompressedImageSizes[i];
                                iResult = DecompressImage(
                                    m_pCompressedImages[i], pImageInfo, i, &pColorChannels[i]);
                                delete (U8*)m_pCompressedImages[i];
                                m_pCompressedImages[i] = NULL;
                            }
                        }
        ...
        if (IFXSUCCESS(iResult))
        {
            ...
            switch (pImageInfo->m_imageType)
            {
                case IFXTextureObject::IFXTEXTUREMAP_FORMAT_LUMINANCE:
                    ...
                    iResult = MergeColorChannels(pColorChannels[0], pImageInfo, m_pContinuationFormats[0].m_uImageChannels, 0, NULL, NULL, 0, 0, 3, ppImage, pImageInfo);
                    break;
                ...
                case IFXTextureObject::IFXTEXTUREMAP_FORMAT_RGB24:
                    ...
                    else
                    {
                        ...
                        {
                            iResult = MergeColorChannels(pColorChannels[i], pImageInfo, m_pContinuationFormats[i].m_uImageChannels, i, NULL, NULL, 0, 0, 3, ppImage, pImageInfo);
                        }
                    }
                    break;
                ...
                case IFXTextureObject::IFXTEXTUREMAP_FORMAT_RGBA32:
                    ...
                    if (m_uContinuationImageCount == 2)
                    {
                        iResult = MergeColorChannels(
                            pColorChannels[0], pImageInfo, m_pContinuationFormats[0].m_uImageChannels, 0, pColorChannels[1], pImageInfo, m_pContinuationFormats[1].m_uImageChannels, 1, 4, ppImage, pImageInfo);
                    }
                    else
                    {
                        for (i = 0; IFXSUCCESS(iResult) && i < m_uContinuationImageCount; i++)
                        {
                            iResult = MergeColorChannels(pColorChannels[i], pImageInfo, m_pContinuationFormats[i].m_uImageChannels, i, NULL, NULL, 0, 0, 4, ppImage, pImageInfo);
                        }
                    }
                    break;
                ...
                case IFXTextureObject::IFXTEXTUREMAP_FORMAT_ALPHA:
                    IFXASSERT(m_uContinuationImageCount == 1)
                    iResult = MergeColorChannels(
                        pColorChannels[0], pImageInfo, m_pContinuationFormats[0].m_uImageChannels, 0, NULL, NULL, 0, 0, 1, ppImage, pImageInfo);
                    break;
...
IFXRESULT CIFXImageTools::GetURL(U32 nUrl, IFXString& rUrl)
{
    ...
    if (IFXSUCCESS(rc))
    {
        rUrl = m_pContinuationFormats[m_uCurrentContinuationImage].m_pImageURLNames[nUrl];
        ...
        if (IFXSUCCESS(iResult))
        {
            ...
            if (BlockType_ResourceTextureContinuationU3D == uBlockType)
            {
                ...
                if (IFXSUCCESS(iResult))
                {
                    ...
                    pImageInfo->m_blockCompressionType[continuationIndex] = m_pContinuationFormats[continuationIndex].m_uCompressionType;
                    ...
                    if (IFXSUCCESS(iResult))
                    {
                        ...
                        if (IFXSUCCESS(iResult))
                        {
                            ...
                            uBlockImageDataLength = uBlockImageDataLength - (uNameLength + sizeof(U16) + sizeof(U32));

                            IFXASSERT(uBlockImageDataLength == m_pContinuationFormats[continuationIndex].m_uImageDataByteCount);
                        }
                        ...
                        if (IFXSUCCESS(iResult))
                        {
                            ...
                            *ppImage = IFXReallocate(
                            ...
                            if (*ppImage == NULL && ((uOutputImageDataLength + uBlockImageDataLength) != 0))
                            {
                                iResult = IFX_E_OUT_OF_MEMORY;
                            }
    ...
    void** ppColorChannels)
{
    ...
    if (m_uContinuationImageCount != 1)
    {
        ...
        for (iImage = 0; iImage < m_uContinuationImageCount; iImage++)
        {
            ppColorChannels[iImage] = (U8*)new U8[pImageInfo->m_width * pImageInfo->m_height * m_pContinuationFormats[iImage].m_bpp];
        ...
        if (IFXSUCCESS(iResult))
        {
            ...
            switch (pImageInfo->m_imageType)
            {
                ...
                case IFXTextureObject::IFXTEXTUREMAP_FORMAT_RGB24:
                    src_bpp = 4;
                    if (pImageInfo->m_imageType == IFXTextureObject::IFXTEXTUREMAP_FORMAT_RGB24 || pImageInfo->m_imageType == IFXTextureObject::IFXTEXTUREMAP_FORMAT_BGR24)
                    {
                        src_bpp = 3;
                    }
                    for (iImage = 0; iImage < m_uContinuationImageCount; iImage++)
                    {
                        if (m_pContinuationFormats[iImage].m_uImageChannels & IFXIMAGECHANNEL_ALPHA)
                        {
                            SrcChannelIndex[iImage][DstChannelCount[iImage]] = 3;
                            DstChannelCount[iImage]++;
                        }
                        if (m_pContinuationFormats[iImage].m_uImageChannels & IFXIMAGECHANNEL_RED)
                        {
                            SrcChannelIndex[iImage][DstChannelCount[iImage]] = 0;
                            DstChannelCount[iImage]++;
                        }
                        if (m_pContinuationFormats[iImage].m_uImageChannels & IFXIMAGECHANNEL_GREEN)
                        {
                            SrcChannelIndex[iImage][DstChannelCount[iImage]] = 1;
                            DstChannelCount[iImage]++;
                        }
                        if (m_pContinuationFormats[iImage].m_uImageChannels & IFXIMAGECHANNEL_BLUE)
                        {
                ...
                case IFXTextureObject::IFXTEXTUREMAP_FORMAT_LUMINANCE_ALPHA:
                    ...
                    for (iImage = 0; iImage < m_uContinuationImageCount; iImage++)
                    {
                        ...
                        if (m_pContinuationFormats[iImage].m_uImageChannels & IFXIMAGECHANNEL_LUMINANCE)
                        {
                            SrcChannelIndex[iImage][DstChannelCount[iImage]] = 0;
                            DstChannelCount[iImage]++;
                        }
            ...
            for (iImage = 0; iImage < m_uContinuationImageCount; iImage++)
            {
                     ...
                     uIndex += src_bpp)
                {
                    ...
                    {
                        pBufColorChannels[iImage][uChannel] = pBufSourceImage[SrcChannelIndex[iImage][uChannel]];
                    }
    ...
    STextureSourceInfo* pImageInfo)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        { //  if( pChannelsSrc1 && pChannelsSrc2)
            // copy the color channel
            if (chanBit1 != (U8)IFXTextureObject::IFXIMAGECHANNEL_LUMINANCE && chanBit2 != (U8)IFXTextureObject::IFXIMAGECHANNEL_LUMINANCE)
            { // no luminance
                if (!(chanBit1 & (U8)IFXTextureObject::IFXIMAGECHANNEL_ALPHA) && !(chanBit2 & (U8)IFXTextureObject::IFXIMAGECHANNEL_ALPHA))
    ...
    STextureSourceInfo* pImageInfo)
{
    ...
    if (BlockType_ResourceTextureU3D == uBlockType)
    {
        ...
        {
            m_pContinuationFormats = new IFXContinuationImageFormat[m_uContinuationImageCount];
            ...
            for (i = 0; i < m_uContinuationImageCount; i++)
            {
                ...
                m_pContinuationFormats[i].m_bpp = CalculateBitsPerPixel(m_pContinuationFormats[i].m_uImageChannels);
                IFXASSERT(m_pContinuationFormats[i].m_bpp <= 4 && m_pContinuationFormats[i].m_bpp > 0);
                ...
                else
                {
                    ...
                    m_pContinuationFormats[i].m_pImageURLNames = new IFXString*[m_pContinuationFormats[i].m_uExtImageURLCount];
                         ...
                         ++count)
                    {
                        ...
                        m_pContinuationFormats[i].m_pImageURLNames[count] = new IFXString(&urlName);
                    }
                }
            }
        }
    ...
    U32& contInd, void** ppDecompressedImage)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        if (setjmp(m_envErrorRecovery) == 0)
        {
            ...
            if (pImageInfo->m_width != cinfo.output_width || pImageInfo->m_height != cinfo.output_height)
            {
                IFXTRACE_CUSTOM(IFXRESULT_COMPONENT_IMAGE_TOOLS, IFXDEBUG_WARNING, L"DecompressImageJPEG: JPEG image size (width and/or height) is "
            ...
            {
                IFXTRACE_CUSTOM(IFXRESULT_COMPONENT_IMAGE_TOOLS, IFXDEBUG_WARNING, L"DecompressImageJPEG: JPEG image channel number conflict could "
                                                                                   L"not be resolve.\n");
                iResult = IFX_E_UNSUPPORTED;
            }
            ...
            if (IFXSUCCESS(iResult))
            {
                pImageInfo->m_size = cinfo.output_width * cinfo.output_height * cinfo.output_components;
                // resize the buffer
                ...
                {
                    pOutput = IFXAllocate(
                        pImageInfo->m_width * pImageInfo->m_height * m_pContinuationFormats[contInd].m_bpp);
                }
                ...
                if (IFXSUCCESS(iResult) && bNeedImageResize)
                {
                    iResult = IFXTextureImageTools_ResizeImage(
                        (U8*)pDestBuffer, (U8*)pOutput, cinfo.num_components, FALSE, cinfo.image_width, cinfo.image_height, pImageInfo->m_width, pImageInfo->m_height);

                    if (IFXSUCCESS(iResult))
                    {
                        IFXDeallocate(pDestBuffer);
                        pImageInfo->m_size = pImageInfo->m_width * pImageInfo->m_height * m_pContinuationFormats[contInd].m_bpp;
    ...
    U32& contInd, void** ppCompressedData)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        OutputBufferSize = (JPEG_HEADER_BLOCK_MULTIPLIER * pImageInfo->m_height) + (pImageInfo->m_width * pImageInfo->m_height * 4) + JPEG_HEADER_BLOCK_MULTIPLIER;
    ...
    STextureSourceInfo* pImageInfo, U32& continuationIndex)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        pTexels = &pSourceImageBuffer[(pImageInfo->m_width * (pImageInfo->m_height - 1) * uPixelSize)];
        ...
        while (uNumRows != 0 && IFXSUCCESS(iResult))
        {
            ...
            else
            {
                ...
                IFXTRACE_CUSTOM(IFXRESULT_COMPONENT_IMAGE_TOOLS, IFXDEBUG_ERROR, L"CompressImage: unrecognized number of channels (not 1, 3, or 4).\n");
                iResult = IFX_E_UNSUPPORTED;
            }
            uNumRows--;
        }
    ...
    STextureSourceInfo* pImageInfo, U32& contInd)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        if (uDestinationPixelSize != 1 && uDestinationPixelSize != 3 && uDestinationPixelSize != 4)
        {
            IFXASSERT(0);
            iResult = IFX_E_UNSUPPORTED;
        }
        ...
        else
        {
            ...
            // output only trace messages in debug build...
            IFXTRACE_CUSTOM(IFXRESULT_COMPONENT_IMAGE_TOOLS, IFXDEBUG_ERROR, L"DecompressImage: unrecognized number of channels (not 1 or 3).\n");
...
{
    // output only trace messages in debug build...
    IFXTRACE_CUSTOM(IFXRESULT_COMPONENT_IMAGE_TOOLS, IFXDEBUG_ERROR, L"CIFXImageTools: error while decompressing image.\n");
...
void CIFXImageTools::ijg_emit_message(j_common_ptr cinfo, int msg_level)
{
    ...
    if (msg_level < 0)
    {
        if (msg_level == -1)
        {
            IFXTRACE_CUSTOM(IFXRESULT_COMPONENT_IMAGE_TOOLS, IFXDEBUG_ERROR, L"CIFXImageTools: IJG warning while decompressing image.\n");
        }
        else
        {
            IFXTRACE_CUSTOM(IFXRESULT_COMPONENT_IMAGE_TOOLS, IFXDEBUG_ERROR, L"CIFXImageTools: IJG error while decompressing image.\n");
        }
    }
    return;
}
...

{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        *ppCompressedData = new U8[OutputBufferSize];
        ...
        png_set_IHDR(
            png_ptr, info_ptr, pImageInfo->m_width, pImageInfo->m_height, ciBitDepth, iColorType, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_BASE, PNG_FILTER_TYPE_BASE);
        ...
        if (NULL == (ppbRowPointers = (png_bytepp) new png_bytep[pImageInfo->m_height]))
        {
            IFXASSERT(0);
        }
        ...
        for (i = 0; i < pImageInfo->m_height; i++)
        {
            ...
            ppbRowPointers[i] = (png_bytep)pSourceImage + (pImageInfo->m_height - i - 1) * ulRowBytes;
        ...
        pImageInfo->m_size = (*(png_bytepp)(png_get_io_ptr(png_ptr))) - (png_bytep)(*ppCompressedData);
        // and we're done
    ...
    U32& contInd, void** ppDecompressedImage)
{
    ...
    {
        // not PNG header
        IFXTRACE_CUSTOM(IFXRESULT_COMPONENT_IMAGE_TOOLS, IFXDEBUG_ERROR, L"DecompressImagePng: not PNG signature at image data block.\n");
        return IFX_E_UNDEFINED;
    }

    // create the two png(-info) structures
    png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL, (png_error_ptr)user_error_fn, (png_error_ptr)NULL);
    ...
    png_get_IHDR(
        png_ptr, info_ptr, &width_png, &height_png, &iBitDepth, &iColorType, NULL, NULL, NULL);
    IFXASSERT(iBitDepth == 8);
    IFXASSERT( // this is temp for debugging
        iColorType == PNG_COLOR_TYPE_RGB || iColorType == PNG_COLOR_TYPE_RGB_ALPHA || iColorType == PNG_COLOR_TYPE_GRAY || iColorType == PNG_COLOR_TYPE_GRAY_ALPHA);

    if (pImageInfo->m_width != width_png || pImageInfo->m_height != height_png)
    {
        IFXTRACE_CUSTOM(IFXRESULT_COMPONENT_IMAGE_TOOLS, IFXDEBUG_WARNING, L"DecompressImagePng: PNG image size (width and/or height) is different "
    ...
    {
        IFXTRACE_CUSTOM(IFXRESULT_COMPONENT_IMAGE_TOOLS, IFXDEBUG_WARNING, L"DecompressImagePng: PNG image channel number is different from one "
                                                                           L"specified in image declaration block. Trying to handle...\n");

        if (m_uContinuationImageCount == 1)
        {
            IFXTRACE_CUSTOM(IFXRESULT_COMPONENT_IMAGE_TOOLS, IFXDEBUG_WARNING, L"DecompressImagePng: Channel number and image type specified in "
            ...
            switch (iColorType)
            {
                case PNG_COLOR_TYPE_RGB:
                    if (pImageInfo->m_imageType == IFXTextureObject::IFXTEXTUREMAP_FORMAT_RGBA32)
                    {
                        pImageInfo->m_imageType = IFXTextureObject::IFXTEXTUREMAP_FORMAT_RGB24;
                    }
                    ...
                    m_pContinuationFormats[contInd].m_uImageChannels ^= IFXIMAGECHANNEL_ALPHA;
                    pImageInfo->m_blockChannels[contInd] ^= IFXIMAGECHANNEL_ALPHA;
                    break;
                case PNG_COLOR_TYPE_RGB_ALPHA:
                    if (pImageInfo->m_imageType == IFXTextureObject::IFXTEXTUREMAP_FORMAT_RGB24)
                    {
                        pImageInfo->m_imageType = IFXTextureObject::IFXTEXTUREMAP_FORMAT_RGBA32;
                    }
        ...
        {
            IFXTRACE_CUSTOM(IFXRESULT_COMPONENT_IMAGE_TOOLS, IFXDEBUG_ERROR, L"DecompressImagePng: channel number conflict could not be reslove.\n");
            iResult = IFX_E_UNDEFINED;
        }
    }
    ...
    if (IFXSUCCESS(iResult))
    {
        if (iColorType == PNG_COLOR_TYPE_RGB_ALPHA || iColorType == PNG_COLOR_TYPE_GRAY_ALPHA)
        {
            bHasAlpha = TRUE;
        }
        ...
        if ((pbImageData = (png_byte*)new png_byte[ulRowBytes * height_png]) == NULL)
        {
        ...
        if (IFXSUCCESS(iResult))
        {
            ...
            {
                *ppDecompressedImage = IFXReallocate(*ppDecompressedImage, pImageInfo->m_size);
            }
        }
        ...
        {
            iResult = IFXTextureImageTools_ResizeImage(
                pbImageData, (U8*)(*ppDecompressedImage), png_get_channels(png_ptr, info_ptr), bHasAlpha, width_png, height_png, pImageInfo->m_width, pImageInfo->m_height);
            if (!IFXSUCCESS(iResult))
            {
                IFXTRACE_CUSTOM(IFXRESULT_COMPONENT_IMAGE_TOOLS, IFXDEBUG_ERROR, L"DecompressImagePng: Error at re-sizing image.\n");
            }
        }
    }
...
IFXRESULT CIFXImageTools::EvaluateContinuationFormat(STextureSourceInfo* pImageInfo)
{
    ...
    switch (imageType)
    {
        ...
        case IFXIMAGE_TYPE_RGB:
            ...
            switch (m_uContinuationImageCount)
            {
                case 1:
                    IFXASSERT(
                        m_pContinuationFormats[0].m_uCompressionType == IFXTextureObject::TextureType_Png || m_pContinuationFormats[0].m_uCompressionType == IFXTextureObject::TextureType_Jpeg24);
                    // we can force it here because with 1 cont. image no other
                    // channel combinations possible
                    pImageInfo->m_blockChannels[0] = (U8)IFXTextureObject::IFXIMAGECHANNEL_RED | (U8)IFXTextureObject::IFXIMAGECHANNEL_GREEN | (U8)IFXTextureObject::IFXIMAGECHANNEL_BLUE;
                ...
                case 2:
                    IFXASSERT(
                        (m_pContinuationFormats[0].m_uCompressionType == IFXTextureObject::TextureType_Jpeg8 && m_pContinuationFormats[1].m_uCompressionType == IFXTextureObject::TextureType_Png) || (m_pContinuationFormats[0].m_uCompressionType == IFXTextureObject::TextureType_Png && m_pContinuationFormats[1].m_uCompressionType == IFXTextureObject::TextureType_Jpeg8) || (m_pContinuationFormats[0].m_uCompressionType == IFXTextureObject::TextureType_Png && m_pContinuationFormats[1].m_uCompressionType == IFXTextureObject::TextureType_Png));

                    m_pContinuationFormats[0].m_uImageChannels = pImageInfo->m_blockChannels[0];
                    m_pContinuationFormats[1].m_uImageChannels = pImageInfo->m_blockChannels[1];

                    m_pContinuationFormats[0].m_bpp = CalculateBitsPerPixel(pImageInfo->m_blockChannels[0]);
                    m_pContinuationFormats[1].m_bpp = CalculateBitsPerPixel(pImageInfo->m_blockChannels[1]);
                    IFXASSERT(
                        m_pContinuationFormats[0].m_bpp <= 2 && m_pContinuationFormats[0].m_bpp > 0);
                    IFXASSERT(
                        m_pContinuationFormats[1].m_bpp <= 2 && m_pContinuationFormats[1].m_bpp > 0);
                    break;

                case 3:
                    IFXASSERT(
                        m_pContinuationFormats[0].m_uCompressionType != IFXTextureObject::TextureType_Jpeg24 && m_pContinuationFormats[1].m_uCompressionType != IFXTextureObject::TextureType_Jpeg24 && m_pContinuationFormats[2].m_uCompressionType != IFXTextureObject::TextureType_Jpeg24);

                    m_pContinuationFormats[0].m_uImageChannels = pImageInfo->m_blockChannels[0];
                    m_pContinuationFormats[1].m_uImageChannels = pImageInfo->m_blockChannels[1];
                    m_pContinuationFormats[2].m_uImageChannels = pImageInfo->m_blockChannels[2];
        ...
        case IFXIMAGE_TYPE_RGBA:
            ...
            switch (m_uContinuationImageCount)
            {
                case 1:
                    IFXASSERT(
                        m_pContinuationFormats[0].m_uCompressionType == IFXTextureObject::TextureType_Png);
                    // we can force it here because with 1 cont. image no
                    // other channel combinations possible
                    pImageInfo->m_blockChannels[0] = (U8)IFXTextureObject::IFXIMAGECHANNEL_RED | (U8)IFXTextureObject::IFXIMAGECHANNEL_GREEN | (U8)IFXTextureObject::IFXIMAGECHANNEL_BLUE | (U8)IFXTextureObject::IFXIMAGECHANNEL_ALPHA;
                    m_pContinuationFormats[0].m_uImageChannels = pImageInfo->m_blockChannels[0];
                ...
                case 2:
                    ...
                    m_pContinuationFormats[1].m_bpp = CalculateBitsPerPixel(pImageInfo->m_blockChannels[1]);
                    IFXASSERT(
                        m_pContinuationFormats[0].m_bpp <= 3 && m_pContinuationFormats[0].m_bpp > 0);
                    IFXASSERT(
                        m_pContinuationFormats[1].m_bpp <= 3 && m_pContinuationFormats[1].m_bpp > 0);
                    break;
                ...
                case 3:
                    ...
                    m_pContinuationFormats[0].m_bpp = CalculateBitsPerPixel(pImageInfo->m_blockChannels[0]);
                    m_pContinuationFormats[1].m_bpp = CalculateBitsPerPixel(pImageInfo->m_blockChannels[1]);
                    m_pContinuationFormats[2].m_bpp = CalculateBitsPerPixel(pImageInfo->m_blockChannels[2]);
                    IFXASSERT(
                        m_pContinuationFormats[0].m_bpp <= 2 && m_pContinuationFormats[0].m_bpp > 0);
                    IFXASSERT(
                        m_pContinuationFormats[1].m_bpp <= 2 && m_pContinuationFormats[1].m_bpp > 0);
                    IFXASSERT(
                        m_pContinuationFormats[2].m_bpp <= 2 && m_pContinuationFormats[2].m_bpp > 0);
                    break;

                case 4:
                    IFXASSERT(
                        m_pContinuationFormats[0].m_uCompressionType != IFXTextureObject::TextureType_Jpeg24 && m_pContinuationFormats[1].m_uCompressionType != IFXTextureObject::TextureType_Jpeg24 && m_pContinuationFormats[2].m_uCompressionType != IFXTextureObject::TextureType_Jpeg24 && m_pContinuationFormats[3].m_uCompressionType != IFXTextureObject::TextureType_Jpeg24);

                    m_pContinuationFormats[0].m_uImageChannels = pImageInfo->m_blockChannels[0];
                    m_pContinuationFormats[1].m_uImageChannels = pImageInfo->m_blockChannels[1];
                    m_pContinuationFormats[2].m_uImageChannels = pImageInfo->m_blockChannels[2];
                    m_pContinuationFormats[3].m_uImageChannels = pImageInfo->m_blockChannels[3];
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        {
            U32 uFileNum = (pImageInfo->m_pURLFileNameList[iContImg]).GetNumberElements();
            ...
            if (uFileNum > 0)
            {
                ...
                m_pContinuationFormats[iContImg].m_pImageURLNames = new IFXString*[uFileNum];
                ...
                if (NULL != m_pContinuationFormats[iContImg].m_pImageURLNames)
                {
                    ...
                    {
                        IFXString*& pStr = (pImageInfo->m_pURLFileNameList[iContImg]).GetElement(iFile);
                        (m_pContinuationFormats[iContImg].m_pImageURLNames)[iFile] = new IFXString(pStr);
                    }
                }
                else
                {
...
void CIFXImageTools::SetCompressionProperties(STextureSourceInfo* pImageInfo)
{
    ...
    if (pImageInfo)
    {
        ...
        {
            m_pContinuationFormats[iCount].m_uCompressionType = pImageInfo->m_blockCompressionType[iCount];
...
CIFXImageTools::IFXContinuationImageFormat::IFXContinuationImageFormat()
{
    ...
    m_bpp = 0;
    m_uImageChannels = (U8)(IFXTextureObject::IFXIMAGECHANNEL_RED | IFXTextureObject::IFXIMAGECHANNEL_BLUE | IFXTextureObject::IFXIMAGECHANNEL_GREEN);
...
void* CIFXImageTools::SwapImageToRGB(STextureSourceInfo* pImageInfo, void* pSrcImage)
{
    ...
    if (pImageInfo && pSrcImage && (pImageInfo->m_imageType == IFXTextureObject::IFXTEXTUREMAP_FORMAT_BGR24 || pImageInfo->m_imageType == IFXTextureObject::IFXTEXTUREMAP_FORMAT_BGRA32))
    {
