...
IFXRESULT IFXAPI_CALLTYPE IFXTextureImageTools_ReformatImage(U8* pSrc, U8* pDst, U8 u8PixelSize, IFXenum eSrcFormat, IFXenum eSrcOrder, IFXenum eDstFormat, IFXenum eDstOrder, U32 uSrcX, U32 uSrcY, U32 uSrcPitch, U32 uDstX, U32 uDstY, U32 uDstPitch, U32 uWidth, U32 uHeight);
        ...
        {
            {
                ...
                else if (IFXTextureObject::IFXTEXTUREMAP_FORMAT_RGB24 == m_uFormat || IFXTextureObject::IFXTEXTUREMAP_FORMAT_BGR24 == m_uFormat)
                {
                    m_u8PixelSize = 3;
                ...
                else if (IFXTextureObject::IFXTEXTUREMAP_FORMAT_RGBA32 == m_uFormat || IFXTextureObject::IFXTEXTUREMAP_FORMAT_BGRA32 == m_uFormat)
                {
                    m_u8PixelSize = 4;
                    m_bHasAlpha = TRUE;
...
U32 CIFXTextureImageTools::ComputeBufferSize(U32 uWidth, U32 uHeight, U8 uFormat)
{
    ...
    else if (IFXTextureObject::IFXTEXTUREMAP_FORMAT_RGB24 == uFormat || IFXTextureObject::IFXTEXTUREMAP_FORMAT_BGR24 == uFormat)
    {
        uSize = uWidth * uHeight * 3;
    }
    else if (IFXTextureObject::IFXTEXTUREMAP_FORMAT_RGBA32 == uFormat || IFXTextureObject::IFXTEXTUREMAP_FORMAT_BGRA32 == uFormat)
    {
...
IFXRESULT IFXAPI_CALLTYPE CIFXTextureImageTools::ReformatImage(U8* pDst, IFXenum eSrcFormat, IFXenum eSrcOrder, IFXenum eDstFormat, IFXenum eDstOrder, U32 uSrcX, U32 uSrcY, U32 uSrcPitch, U32 uDstX, U32 uDstY, U32 uDstPitch, U32 uWidth, U32 uHeight)
{
    if (!((eSrcFormat == IFX_RGBA_8888) || (eSrcFormat == IFX_RGBA_8880)) || !((eDstFormat == IFX_RGBA_8888) || (eDstFormat == IFX_RGBA_8880)))
    {
        return IFXTextureImageTools_ReformatImage(
            m_pBuffer, pDst, m_u8PixelSize, eSrcFormat, eSrcOrder, eDstFormat, eDstOrder, uSrcX, uSrcY, uSrcPitch, uDstX, uDstY, uDstPitch, uWidth, uHeight);
    }
    else
    {
...
IFXRESULT IFXAPI_CALLTYPE CIFXTextureImageTools::MipMap(STextureOutputInfo* pSrcOutputInfo, STextureOutputInfo* pDstOutputInfo, U32 uSrcX, U32 uSrcY, U32 uDstX, U32 uDstY)
{
    IFXRESULT iResult = IFX_OK;
...
IFXRESULT CIFXTextureImageTools::ConvertToRenderFormat(STextureOutputInfo* pRenderImageInfo)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ReformatImage(pRenderImageInfo->m_pData, m_eCurRenderFormat, m_eCurOrder, pRenderImageInfo->eRenderFormat, pRenderImageInfo->eChannelOrder, 0, 0, m_uPitch, 0, 0, pRenderImageInfo->m_pitch, pRenderImageInfo->m_width, pRenderImageInfo->m_height);
...
IFXRESULT IFXAPI_CALLTYPE IFXTextureImageTools_ReformatImage(U8* pSrc, U8* pDst, U8 u8PixelSize, IFXenum eSrcFormat, IFXenum eSrcOrder, IFXenum eDstFormat, IFXenum eDstOrder, U32 uSrcX, U32 uSrcY, U32 uSrcPitch, U32 uDstX, U32 uDstY, U32 uDstPitch, U32 uWidth, U32 uHeight)
{
    ...
    else
    {
        ...
        I32 redOffset = redDstOffset - redSrcOffset + IFXTextureImageTools_CountBits(dstFrmt.dwRBitMask) - IFXTextureImageTools_CountBits(srcFrmt.dwRBitMask);
        I32 greenOffset = greenDstOffset - greenSrcOffset + IFXTextureImageTools_CountBits(dstFrmt.dwGBitMask) - IFXTextureImageTools_CountBits(srcFrmt.dwGBitMask);
        I32 blueOffset = blueDstOffset - blueSrcOffset + IFXTextureImageTools_CountBits(dstFrmt.dwBBitMask) - IFXTextureImageTools_CountBits(srcFrmt.dwBBitMask);
        I32 alphaOffset = alphaDstOffset - alphaSrcOffset + IFXTextureImageTools_CountBits(dstFrmt.dwRGBAlphaBitMask) - IFXTextureImageTools_CountBits(srcFrmt.dwRGBAlphaBitMask);
        U32 value;
                ...
                if (u8PixelSize == 4)
                {
                    for (sY = uSrcY, dY = uDstY; sY < (uHeight + uSrcY); sY++, dY++)
                    {
                        for (sX = uSrcX, dX = uDstX; sX < (uWidth + uSrcX); sX++, dX++)
                        {
                            ...
                            *pDstPixelLong = MakeTexel(pSrcPixelShort[u8RIndex + redOffset / 8], pSrcPixelShort[u8GIndex + greenOffset / 8], pSrcPixelShort[u8BIndex + blueOffset / 8], pSrcPixelShort[u8AIndex + alphaOffset / 8]);
                        }
                    }
                }
                else if (u8PixelSize == 3)
                {
                    ...
                    for (sY = uSrcY, dY = uDstY; sY < (uHeight + uSrcY); sY++, dY++)
                    {
                        for (sX = uSrcX, dX = uDstX; sX < (uWidth + uSrcX); sX++, dX++)
                        {
                            ...
                            pSrcPixelShort = (U8*)&(pSrc[uSrcIndex]);
                            pDstPixelLong = (U32*)&(pDst[uDstIndex]);
                            *pDstPixelLong = MakeTexel(pSrcPixelShort[u8RIndex + redOffset / 8], pSrcPixelShort[u8GIndex + greenOffset / 8], pSrcPixelShort[u8BIndex + blueOffset / 8], 0xFF);
                        }
                    }
                }
                else
                ...
                if (dBpp == 2)
                {
                    ...
                    for (sY = uSrcY, dY = uDstY; sY < (uHeight + uSrcY); sY++, dY++)
                    {
                        for (sX = uSrcX, dX = uDstX; sX < (uWidth + uSrcX); sX++, dX++)
                        {
                            ...
                            (*pDstPixelShort) &= ~(dstFrmt.dwRBitMask | dstFrmt.dwGBitMask | dstFrmt.dwBBitMask | dstFrmt.dwRGBAlphaBitMask);
                            uSrcPixelLong = MakeTexel(pSrcPixelShort[u8RIndex], pSrcPixelShort[u8GIndex], pSrcPixelShort[u8BIndex], 0xFF);
                            // Red component
    ...
    if (dBpp == 2)
    {
        ...
        for (sY = uSrcY, dY = uDstY; dY < (pDstOutputInfo->m_height + uSrcY); sY++, dY++)
        {
            ...
            for (sX = uSrcX, dX = uDstX; dX < (pDstOutputInfo->m_width + uSrcX); sX++, dX++)
            {
                ...
                (*pDstPixelShort) &= ~(dstFrmt.dwRBitMask | dstFrmt.dwGBitMask | dstFrmt.dwBBitMask | dstFrmt.dwRGBAlphaBitMask);
                if ((pDstOutputInfo->m_height < pSrcOutputInfo->m_height && pDstOutputInfo->m_width < pSrcOutputInfo->m_width))
                {
                    RValue = ((*(U16*)&(pSrc[uCurRow + sX * sBpp2])) & srcFrmt.dwRBitMask) + ((*(U16*)&(pSrc[uCurRow + (sX * 2 + 1) * sBpp])) & srcFrmt.dwRBitMask) + ((*(U16*)&(pSrc[uNxtRow + sX * sBpp2])) & srcFrmt.dwRBitMask) + ((*(U16*)&(pSrc[uNxtRow + (sX * 2 + 1) * sBpp])) & srcFrmt.dwRBitMask);
                    GValue = ((*(U16*)&(pSrc[uCurRow + sX * sBpp2])) & srcFrmt.dwGBitMask) + ((*(U16*)&(pSrc[uCurRow + (sX * 2 + 1) * sBpp])) & srcFrmt.dwGBitMask) + ((*(U16*)&(pSrc[uNxtRow + sX * sBpp2])) & srcFrmt.dwGBitMask) + ((*(U16*)&(pSrc[uNxtRow + (sX * 2 + 1) * sBpp])) & srcFrmt.dwGBitMask);
                    BValue = ((*(U16*)&(pSrc[uCurRow + sX * sBpp2])) & srcFrmt.dwBBitMask) + ((*(U16*)&(pSrc[uCurRow + (sX * 2 + 1) * sBpp])) & srcFrmt.dwBBitMask) + ((*(U16*)&(pSrc[uNxtRow + sX * sBpp2])) & srcFrmt.dwBBitMask) + ((*(U16*)&(pSrc[uNxtRow + (sX * 2 + 1) * sBpp])) & srcFrmt.dwBBitMask);
                    AValue = ((*(U16*)&(pSrc[uCurRow + sX * sBpp2])) & srcFrmt.dwRGBAlphaBitMask) + ((*(U16*)&(pSrc[uCurRow + (sX * 2 + 1) * sBpp])) & srcFrmt.dwRGBAlphaBitMask) + ((*(U16*)&(pSrc[uNxtRow + sX * sBpp2])) & srcFrmt.dwRGBAlphaBitMask) + ((*(U16*)&(pSrc[uNxtRow + (sX * 2 + 1) * sBpp])) & srcFrmt.dwRGBAlphaBitMask);
                    u8ShiftBits = 2;
                }
                else if (pDstOutputInfo->m_height < pSrcOutputInfo->m_height)
                {
                    RValue = ((*(U16*)&(pSrc[uCurRow + sX * sBpp2])) & srcFrmt.dwRBitMask) + ((*(U16*)&(pSrc[uNxtRow + sX * sBpp2])) & srcFrmt.dwRBitMask);
                    GValue = ((*(U16*)&(pSrc[uCurRow + sX * sBpp2])) & srcFrmt.dwGBitMask) + ((*(U16*)&(pSrc[uNxtRow + sX * sBpp2])) & srcFrmt.dwGBitMask);
                    BValue = ((*(U16*)&(pSrc[uCurRow + sX * sBpp2])) & srcFrmt.dwBBitMask) + ((*(U16*)&(pSrc[uNxtRow + sX * sBpp2])) & srcFrmt.dwBBitMask);
                    AValue = ((*(U16*)&(pSrc[uCurRow + sX * sBpp2])) & srcFrmt.dwRGBAlphaBitMask) + ((*(U16*)&(pSrc[uNxtRow + sX * sBpp2])) & srcFrmt.dwRGBAlphaBitMask);
                    u8ShiftBits = 1;
                }
                else if (pDstOutputInfo->m_width < pSrcOutputInfo->m_width)
                {
                    RValue = ((*(U16*)&(pSrc[uCurRow + sX * sBpp2])) & srcFrmt.dwRBitMask) + ((*(U16*)&(pSrc[uCurRow + (sX * 2 + 1) * sBpp])) & srcFrmt.dwRBitMask);
                    GValue = ((*(U16*)&(pSrc[uCurRow + sX * sBpp2])) & srcFrmt.dwGBitMask) + ((*(U16*)&(pSrc[uCurRow + (sX * 2 + 1) * sBpp])) & srcFrmt.dwGBitMask);
                    BValue = ((*(U16*)&(pSrc[uCurRow + sX * sBpp2])) & srcFrmt.dwBBitMask) + ((*(U16*)&(pSrc[uCurRow + (sX * 2 + 1) * sBpp])) & srcFrmt.dwBBitMask);
                    AValue = ((*(U16*)&(pSrc[uCurRow + sX * sBpp2])) & srcFrmt.dwRGBAlphaBitMask) + ((*(U16*)&(pSrc[uCurRow + (sX * 2 + 1) * sBpp])) & srcFrmt.dwRGBAlphaBitMask);
                    u8ShiftBits = 1;
                }
        ...
        for (sY = uSrcY, dY = uDstY; dY < (pDstOutputInfo->m_height + uSrcY); sY++, dY++)
        {
            ...
            if (pDstOutputInfo->m_height < pSrcOutputInfo->m_height && pDstOutputInfo->m_width < pSrcOutputInfo->m_width)
            {
                for (sX = uSrcX, dX = uDstX; dX < (pDstOutputInfo->m_width + uSrcX); sX++, dX++)
                {
                    ...
                    pDstPixelShort[0] = (pSrc[uCurRow + sX * sBpp2] + pSrc[uCurRow + (sX * 2 + 1) * sBpp] + pSrc[uNxtRow + sX * sBpp2] + pSrc[uNxtRow + (sX * 2 + 1) * sBpp]) >> 2;
                    pDstPixelShort[1] = (pSrc[uCurRow + sX * sBpp2 + 1] + pSrc[uCurRow + (sX * 2 + 1) * sBpp + 1] + pSrc[uNxtRow + sX * sBpp2 + 1] + pSrc[uNxtRow + (sX * 2 + 1) * sBpp + 1]) >> 2;
                    pDstPixelShort[2] = (pSrc[uCurRow + sX * sBpp2 + 2] + pSrc[uCurRow + (sX * 2 + 1) * sBpp + 2] + pSrc[uNxtRow + sX * sBpp2 + 2] + pSrc[uNxtRow + (sX * 2 + 1) * sBpp + 2]) >> 2;
                    pDstPixelShort[3] = (pSrc[uCurRow + sX * sBpp2 + 3] + pSrc[uCurRow + (sX * 2 + 1) * sBpp + 3] + pSrc[uNxtRow + sX * sBpp2 + 3] + pSrc[uNxtRow + (sX * 2 + 1) * sBpp + 3]) >> 2;
                }
            }
            else if (pDstOutputInfo->m_height < pSrcOutputInfo->m_height)
            {
