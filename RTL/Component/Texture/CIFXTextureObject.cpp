...
IFXRESULT CIFXTextureObject::SetTextureType(IFXenum uMapType)
{
    ...
    if (uMapType != IFX_TEXTURE_CUBE && uMapType != IFX_TEXTURE_2D)
    {
...
IFXRESULT CIFXTextureObject::GetCubeMapTexture(IFXenum ePosition, IFXTextureObject** ppTexture)
{
    ...
    if (ePosition < IFX_TEXTURE_CUBE_PX && ePosition > IFX_TEXTURE_CUBE_NZ)
    {
    ...
    if (m_pCubeMapTexture)
    {
        ...
        if (IFXSUCCESS(iResult))
        {
            iResult = punk->QueryInterface(IID_IFXTextureObject, (void**)ppTexture);
        }
    }
    else
    {
...
IFXRESULT CIFXTextureObject::Reallocate(U32 uWidth, U32 uHeight, U32 uFormat)
{
    ...
    if (m_pImageBuffer)
    {
        ...
        // information...
        IFXTRACE_CUSTOM(IFXRESULT_COMPONENT_TEXTURE_MANAGER, IFXDEBUG_MESSAGE, L"reallocated CIFXTextureMap(%p) to %u x %u (%u bpp)\n", this, uWidth, uHeight, uFormat * 8);
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        if (TRUE == m_bImageDirty)
        {
            ...
            else
            {
                ...
                {
                    iResult = IFXCreateComponent(CID_IFXImageCodec, IID_IFXImageCodec, (void**)&m_pImageCodec);
                ...
                if (IFXSUCCESS(iResult))
                {
                    ...
                    if (IFXSUCCESS(iResult))
                    {
                        iResult = m_pImageCodec->DecompressBlockQueueToImage(m_pDataBlockQueueX, &sImageInfo, &pImage);
                    }
    ...
    IFXDataBlockQueueX* pDataBlockQueueX)
{
    ...
    try
    {
        ...
        IFXCHECKX(m_pImageCodec->CompressImageToBlockQueue(
            pDataBlockQueueX, &m_sImageInfo, (void*)m_pImageBuffer, m_uPriority));
    }
        ...
        switch (eCompressionState)
        {
            ...
            case IFXTEXTURECOMPRESSIONSTATE_FORCECOMPRESSED:
                ...
                if (!m_bKeepDecompressed)
                {
                    ...
                    {
                        iResult = Reallocate(
                            1, 1, IFXTextureImageTools::IFXTEXTUREMAP_FORMAT_LUMINANCE);
    ...
    else
    {
        ...
        switch (m_sImageInfo.m_imageType)
        {
            case IFXTEXTUREMAP_FORMAT_ALPHA:
                ...
                {
                    if ((pB[0] == png || pB[0] == j8) && (pImageChannelFlagsList[0] == IFXTextureObject::IFXIMAGECHANNEL_ALPHA))
                    {
                        rc = IFX_OK;
                    }
                    else
            ...
            case IFXTEXTUREMAP_FORMAT_BGR24:
                ...
                if (uContImageCount == 1)
                {
                    ...
                    if ((pB[0] == j24 || pB[0] == png) && (pCh[0]) == (ch_r | ch_b | ch_g))
                    {
                        rc = IFX_OK;
                    }
                    else
            ...
            case IFXTEXTUREMAP_FORMAT_BGRA32:
                ...
                if (uContImageCount == 1)
                {
                    if ((pB[0] == png) && (pImageChannelFlagsList[0] == (IFXTextureObject::IFXIMAGECHANNEL_RED | IFXTextureObject::IFXIMAGECHANNEL_BLUE | IFXTextureObject::IFXIMAGECHANNEL_GREEN | IFXTextureObject::IFXIMAGECHANNEL_ALPHA)))
                    {
                        rc = IFX_OK;
                    }
                    else
                ...
                {
                    if ((pB[0] == png && pB[1] == j8) || (pB[0] == j8 && pB[1] == png) || (pB[0] == j24 && pB[1] == j8) || (pB[0] == j8 && pB[1] == j24) || (pB[0] == j24 && pB[1] == png) || (pB[0] == png && pB[1] == j24) || (pB[0] == png && pB[1] == png))
                    {
                        rc = IFX_OK;
                    }
                    else
            ...
            case IFXTEXTUREMAP_FORMAT_LUMINANCE:
                if (uContImageCount == 1)
                {
                    if ((pB[0] == png || pB[0] == j8) && (pImageChannelFlagsList[0] == IFXTextureObject::IFXIMAGECHANNEL_LUMINANCE))
                    {
                        rc = IFX_OK;
                    }
                    else
            ...
            case IFXTEXTUREMAP_FORMAT_LUMINANCE_ALPHA:
                ...
                {
                    if ((pB[0] != j24 && pB[1] != j24) && (!(pCh[0] | ch_r) && !(pCh[0] | ch_b) && !(pCh[0] | ch_g) && !(pCh[1] | ch_r) && !(pCh[1] | ch_b) && !(pCh[1] | ch_g)))
                    {
                        rc = IFX_OK;
                    }
                    else
    ...
    const U8* pBlockCompressionType)
{
    ...
    {
        if (pBlockCompressionType[i] == TextureType_Jpeg24 || pBlockCompressionType[i] == TextureType_Jpeg8 || pBlockCompressionType[i] == TextureType_Png)
        {
        }
        else
        {
    ...
    IFXArray<IFXString*>** ppExternalFileReferenceArrayList)
{
    ...
    if (IFXSUCCESS(rc))
    {
        ...
        for (i = 0; i < rContImageCount; i++)
        {
            ...
            pImageChannelFlagsList[i] = (IFXTextureObject::ChannelType)m_sImageInfo.m_blockChannels[i];
            ...
            if (ppExternalFileReferenceArrayList != NULL && pExternalFileReferenceList != NULL)
            {
                if (pExternalFileReferenceList[i])
                {
                    U32 uFileRefCount = m_sImageInfo.m_pURLFileNameList[i].GetNumberElements();
                    IFXArray<IFXString*>* pPtrStringArray = ppExternalFileReferenceArrayList[i];

                    pPtrStringArray->ResizeToExactly(uFileRefCount);
                    U32 indFile;
                    for (indFile = 0; indFile < uFileRefCount; indFile++)
                    {
                        IFXString*& pStr = m_sImageInfo.m_pURLFileNameList[i].GetElement(indFile);
    ...
    IFXArray<IFXString*>** ppExternalFileReferenceArrayList)
{
    ...
    if (IFXSUCCESS(rc))
    {
        ...
        {
            m_sImageInfo.m_blockCompressionType[count] = pBlockCompressionTypeList[count];
            m_sImageInfo.m_blockChannels[count] = pImageChannelFlagsList[count];

            if (pExternalFileReferenceList != NULL && ppExternalFileReferenceArrayList != NULL)
            {
                m_sImageInfo.m_URLFileRef[count] = pExternalFileReferenceList[count];
                if (pExternalFileReferenceList[count])
                {
                    U32 uFileRefCount = ppExternalFileReferenceArrayList[count]->GetNumberElements();
                    ...
                    for (indFile = 0; indFile < uFileRefCount; indFile++)
                    {
                        IFXString*& pStr = ppExternalFileReferenceArrayList[count]->GetElement(indFile);
