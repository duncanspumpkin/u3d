...
BOOL Pair::continuousTexCoords(IFXAuthorCLODMesh* imesh)
{
    ...
    if (m_Faces.Size() == 2)
    {
        ...
        if (texCoordsEqual(f1, f1Side, f2, clockwise[f2Side], imesh) && texCoordsEqual(f1, clockwise[f1Side], f2, f2Side, imesh))
        {
            return TRUE;
        }
        else
...
void Pair::computeCost(Matrix4x4* pQuadric)
{
    ...
    if ((v1->getBoundary() && !v2->getBoundary() && !v2->getBase()) || (v1->getTextureBoundary() && !v2->getTextureBoundary() && !v2->getBase()))
    {
        cost = v1cost;
        weight = 0.0f;
    }
    else if ((v2->getBoundary() && !v1->getBoundary() && !v1->getBase()) || (v2->getTextureBoundary() && !v1->getTextureBoundary() && !v1->getBase()))
    {
        cost = v2cost;
        weight = 1.0f;
    }
