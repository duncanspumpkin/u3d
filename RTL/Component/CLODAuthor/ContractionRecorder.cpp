...
ContractionRecorder::ContractionRecorder(CLODGenerator::Params* pParams)
{
    ...
    if (pMesh->GetMaxMeshDesc()->NumTexCoords || pMesh->GetMaxMeshDesc()->NumDiffuseColors || pMesh->GetMaxMeshDesc()->NumSpecularColors)
    {
...
U32 ContractionRecorder::recordVerticesInFaceUpdates(SmallPtrSet* updated, IFXAuthorFaceUpdate* faceUpdates, Vertex* keep, Vertex* remove)
{
...
UPTR ContractionRecorder::recordAttributesInFaceUpdates(FacePtrSet* deleted, SmallPtrSet* updated, IFXAuthorFaceUpdate* pFaceUpdate, Vertex* keep, Vertex* remove)
{
    ...
    while (pUpdatedFace)
    {
        ...
        diffuseDone = FALSE;
        specularDone = FALSE;
        for (i = 0; i < m_NumTexLayersUsed; i++)
        {
            layerDone[i] = FALSE;
        }
        ...
        if (pMat->m_uNumTextureLayers || pMat->m_uDiffuseColors || pMat->m_uSpecularColors)
        {
            ...
            while (pDeletedFace)
            {
                ...
                if (DelMatID == UpMatID)
                {
                    ...
                    if (Da == remove)
                    {
                        deletedRemoveCorner = 0;
                    }
                    if (Db == remove)
                    {
                        deletedRemoveCorner = 1;
                    }
                    if (Dc == remove)
                    {
                        deletedRemoveCorner = 2;
                    }
                    ...
                    for (layer = 0; layer < numLayers; layer++)
                    {
                        if (!layerDone[layer])
                        {
                            ...
                            attributeCode = (IFXAuthorFaceUpdate::AttributeType)(IFXAuthorFaceUpdate::Tex0 + layer);

                            layerDone[layer] = fillUpdateRecord(delFace, upFace, upFaceIndex, attributeCode, deletedKeepCorner, deletedRemoveCorner, updatedCorner, pFaceUpdate);
                    ...
                    if (pMat->m_uDiffuseColors && !diffuseDone)
                    {
                        ...
                        diffuseDone = fillUpdateRecord(delFace, upFace, upFaceIndex, attributeCode, deletedKeepCorner, deletedRemoveCorner, updatedCorner, pFaceUpdate);
                        if (diffuseDone)
                        {
                            pFaceUpdate++;
                        }
                    ...
                    if (pMat->m_uSpecularColors && !specularDone)
                    {
                        ...
                        specularDone = fillUpdateRecord(delFace, upFace, upFaceIndex, attributeCode, deletedKeepCorner, deletedRemoveCorner, updatedCorner, pFaceUpdate);
...
BOOL ContractionRecorder::fillUpdateRecord(IFXAuthorFace* delFace, IFXAuthorFace* upFace, U32 upFaceIndex, IFXAuthorFaceUpdate::AttributeType attribute, U32 deletedKeepCorner, U32 deletedRemoveCorner, U32 updatedCorner, IFXAuthorFaceUpdate* pFaceUpdate)
{
...
U32 ContractionRecorder::recordNormalUpdate(U32 faceIndex, U32 keepVertex, U32 normalIndex, IFXAuthorFaceUpdate* faceUpdate)
{
...
void ContractionRecorder::computeOtherVertices(SmallPtrSet* vertices, SmallPtrSet* updated, Vertex* keep)
{
    U32 SetCtx = 0;
    Face* pFace = (Face*)updated->Begin(SetCtx);
    while (pFace)
...
U32 ContractionRecorder::recordNormalsInFaceUpdates_NormalNeighborhood(Vertex* keep, SmallPtrSet* updatedFaces, IFXAuthorFaceUpdate* faceUpdates, BOOL smallNormalChange)
{
...
U32 ContractionRecorder::recordNormalsInFaceUpdates_CreaseAngle(Vertex* keep, SmallPtrSet* faceSet, float tolerance, IFXAuthorFaceUpdate* faceUpdates)
{
        ...
        if (faceExam->faceIndex != UNDEFINED_INDEX)
        {
            ...
            if ((faceExam->marked) && (fe != firstFace)) // end of a crease angle group
            {
                ...
                for (i = start; i < fe; i++)
                {
                    ...
                    {
                        numFaceUpdates += recordNormalUpdate(thisFe->faceIndex, VTXIDX(keep), normalIndex, &faceUpdates[numFaceUpdates]);
                    }
                }
 ...
 */
void ContractionRecorder::record(Vertex* keep, Vertex* remove, FacePtrSet* deleted, SmallPtrSet* updated)
{
    numVertexRemovals++;
    ...
    if (m_hasTexturesOrColors)
    {
        ...
        usedFaceUpdates += recordAttributesInFaceUpdates(deleted, updated, &pFaceUpdatePool[usedFaceUpdates], keep, remove);
    }
...
void ContractionRecorder::recordAttribChanges(Vertex* keep, SmallPtrSet* updated, BOOL smallNormalChange)
{
    if (normalsMode == CLODGenerator::TrackSurfaceChanges)
    {
        usedFaceUpdates += recordNormalsInFaceUpdates_NormalNeighborhood(keep, updated, &pFaceUpdatePool[usedFaceUpdates], smallNormalChange);
    }
