    ...
    for (unsigned long n = 0; n < numNormals; n++)
    {
        ...
        if (list->GetFirst())
        {
            do
            {
                ...
                if (normals[n].x == normals[index].x && normals[n].y == normals[index].y && normals[n].z == normals[index].z)
                {
...
void NormalMap::searchCell(float phi, float theta, int row, int col, float* closestDistance, unsigned long* closestIndex)
{
    float testDistance;
