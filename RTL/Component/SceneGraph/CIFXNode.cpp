...
IFXRESULT CIFXNode::Update(IFXSubject* pInSubject, U32 uInChangeBits, IFXREFIID rIType)
{
    ...
    else
    {
        ...
        {
            IFXParent& Element = m_Parents.GetElement(parentIndex);
            if ((Element.uParentsModChainsTransformAspectBit != (U32)-1) && (uInChangeBits & Element.uParentsModChainsTransformAspectBit))
            {
...
void CIFXNode::SetPriority(U32 uInPriority, BOOL bRecursive, BOOL bPromotionOnly)
{
    ...
    if (bRecursive)
    {
        ...
        for (i = 0; i < childCounter && IFXSUCCESS(rc); i++)
        {
            ...
            if (IFXSUCCESS(rc))
            {
                pMarker->SetPriority(uInPriority, bRecursive, bPromotionOnly);
...
IFXRESULT CIFXNode::AddSpatials(IFXSpatial** pInSpatials, U32 uInNumberOfSpatials, IFXSpatial::eType eInType)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT CIFXNode::RemoveSpatials(IFXSpatial** pInSpatials, U32 uInNumberOfSpatials, IFXSpatial::eType eInType)
{
    IFXRESULT result = IFX_OK;
    IFXDECLARELOCAL(IFXCollection, pCollection);
...
IFXRESULT CIFXNode::GetCollection(IFXREFCID rInCollectionCID, IFXCollection** ppOutCollection)
{
    IFXRESULT result = IFX_OK;
    ...
    if (uCollection == (U32)-1)
    {
        ...
        if (IFXSUCCESS(result))
        {
            result = IFXCreateComponent(
                rInCollectionCID, IID_IFXCollection, (void**)&m_pCollections[m_uCollections]);
        }
    ...
    IFXNode* pParentNode, U32 uParentInstance)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        while (
            (i < count - 1) && (pMyNode->GetParentNR(i) == pMyNode->GetParentNR(i + parentCount)))
        {
            parentCount++;
        }

        {
            ...
            if (uMyInstance < index2)
            {
                ...
                {
                    result = IsThisBranch(
                        pMyNode->GetParentNR(i), (myInstances - 1 - uMyInstance - index) / parentCount, pParentNode, uParentInstance);
                }
                break;
            }
            index = index2;
...
IFXRESULT CIFXNode::GetSubgraphBound(IFXVector4& rSphere, U32 rootInstance)
{
    ...
    {
        // Opaque models
        result = m_pCollections[0]->GetSpatials(pModels, uModels, IFXSpatial::OPAQUE_MODEL);
        ...
        if (IFXSUCCESS(result))
        {
            ...
            result = m_pCollections[0]->GetSpatials(
                pModels, uModels, IFXSpatial::TRANSLUCENT_MODEL);
        }
    ...
    IFXVector3& direction, IFXSimpleList** ppPickedObjectList)
{
    ...
    *ppPickedObjectList = NULL;
    ...
    if (uSpatials[0] || uSpatials[1])
    {
        ...
        {
            result = IFXCreateComponent(CID_IFXSimpleList, IID_IFXSimpleList, (void**)&m_pPickObjectList);
        }
        else
        {
            U32 uListSize;
        ...
        for (list = 0; list < 2; list++)
        {
            for (i = 0; i < uSpatials[list] && IFXSUCCESS(result); i++)
            {
                ...
                if (IFXSUCCESS(result) && pModel->GetPickable())
                {
                    ...
                    {
                        if (!HasParent(
                                (IFXNode*)pModel, instance, (IFXNode*)this, myInstance))
                        {
                            continue;
                        }
                                ...
                                vBoundingSphere, position, direction))
                        {
                            ...
                            if (IFXSUCCESS(result))
                            {
                                ...
                                if (IFXSUCCESS(result))
                                {
                                    ...
                                    pWorldTransform = &pAWorldTransform->GetElement(instance);
                                }
    ...
    for (i = 0; i < count; i++)
    {
...

        {
            ...
            if (uMyInstance < index2)
            {
                ...
                {
                    result = HasParent(
                        pMyNode->GetParentNR(i), (uMyInstance - index) / parentCount, pParentNode, uParentInstance);
                }
                break;
            }
            index = index2;
                ...
                for (i = 0; i < IFXSpatial::TYPE_COUNT && IFXSUCCESS(rc); i++)
                {
                    ...
                    if (IFXSUCCESS(rc) && nSpatials[i])
                    {
                        rc = pNewParentNode->AddSpatials(
                            pSpatials[i], nSpatials[i], (IFXSpatial::eType)i);
                    }
                }
            }
...
IFXRESULT CIFXNode::GenerateOutput(U32 inOutputDataElementIndex, void*& rpOutData, BOOL& rNeedRelease)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT CIFXNode::SetDataPacket(IFXModifierDataPacket* pInInputDataPacket, IFXModifierDataPacket* pInOutputDataPacket)
{
    IFXRESULT result = IFX_OK;
    ...
    if (pInOutputDataPacket && pInInputDataPacket && IFXSUCCESS(result))
    {
        result = pInOutputDataPacket->GetDataElementIndex(DID_IFXTransform, m_uMyDataPacketTransformIndex);
    }
...
IFXRESULT CIFXNode::Notify(IFXModifierMessage eInMessage, void* pMessageContext)
{
    return IFX_OK;
}
