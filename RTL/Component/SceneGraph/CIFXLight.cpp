...
void CIFXLight::Counter(EIFXNodeCounterType type, U32* pOutCount)
{
    if (pOutCount)
    {
        ...
        if ((type == IFX_NODE_COUNTER_NODES) || (type == IFX_NODE_COUNTER_LIGHTS))
        {
            (*pOutCount)++;
        }
    }
...
IFXRESULT CIFXLight::GetSpatialBound(IFXVector4& rOutSphere, U32 WorldInstance)
{
    ...
    if (IFXSUCCESS(result))
    {
        ...
        if (IFXSUCCESS(result))
        {
            result = pDataPacket->GetDataElement(m_uMyDataPacketTransformIndex, (void**)&pAResultantTransform);
    ...
    if (IFXSUCCESS(result))
    {
        ...
        if (IFXSUCCESS(result))
        {
            rOutSphere.Set(*(pResultantTransform->RawConst() + 12), *(pResultantTransform->RawConst() + 13), *(pResultantTransform->RawConst() + 14), pLR->GetRenderLight().GetRange());
        }
...
IFXRESULT CIFXLight::GetOutputs(IFXGUID**& rpOutOutputs, U32& rOutNumberOfOutputs, U32*& rpOutOutputDepAttrs)
{
...
IFXRESULT CIFXLight::GetDependencies(IFXGUID* pInOutputDID, IFXGUID**& rppOutInputDependencies, U32& rOutNumberInputDependencies, IFXGUID**& rppOutOutputDependcies, U32& rOutNumberOfOutputDependcies, U32*& rpOutOutputDepAttrs)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT CIFXLight::GenerateOutput(U32 inOutputDataElementIndex, void*& rpOutData, BOOL& rNeedRelease)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT CIFXLight::Notify(IFXModifierMessage eInMessage, void* pMessageContext)
{
    return IFX_OK;
}
