...
IFXRESULT CIFXModel::Mark(void)
{
    ...
    if (IFXSUCCESS(result))
    {
        ...
        result = m_pSceneGraph->GetPalette(IFXSceneGraph::GENERATOR, &pGeneratorPalette);

        if (IFXSUCCESS(result))
        {
            IFXMarker* pGenerator = NULL;

            result = pGeneratorPalette->GetResourcePtr(m_uResourceIndex, IID_IFXMarker, (void**)&pGenerator);
            if (IFXSUCCESS(result))
            {
                pGenerator->Mark();
                IFXRELEASE(pGenerator);
            }
            else if ((IFX_E_PALETTE_NULL_RESOURCE_POINTER == result) || (IFX_E_INVALID_RANGE == result))
            {
                result = IFX_OK;
            }
...
void CIFXModel::SetPriority(U32 uInPriority, BOOL bRecursive, BOOL bPromotionOnly)
{
    IFXRESULT rc = IFX_OK;
    ...
    if ((bRecursive == IFX_TRUE) && IFXSUCCESS(rc))
    {
        ...
        {
            rc = m_pSceneGraph->GetPalette(IFXSceneGraph::GENERATOR, &pGeneratorPalette);

            if (IFXSUCCESS(rc))
            {
                IFXMarker* pGenerator = NULL;
                rc = pGeneratorPalette->GetResourcePtr(m_uResourceIndex, IID_IFXMarker, (void**)&pGenerator);
                if (IFXSUCCESS(rc))
                {
                    pGenerator->SetPriority(uInPriority);
                }
                else if ((IFX_E_PALETTE_NULL_RESOURCE_POINTER == rc) || (IFX_E_INVALID_RANGE == rc))
                {
                    rc = IFX_OK;
                }
...
IFXRESULT CIFXModel::GetSpatialBound(IFXVector4& rOutSphere, U32 WorldInstance)
{
    ...
    if (IFXSUCCESS(result))
    {
        result = pDataPacket->GetDataElement(m_uMyDataPacketTransformIndex, (void**)&pAWorldTransform);
    }
...

{
    ...
    if ((type == IFX_NODE_COUNTER_NODES) || (type == IFX_NODE_COUNTER_MODELS))
    {
        (*pOutCount)++;
    }
...
IFXRESULT CIFXModel::GetOutputs(IFXGUID**& rpOutOutputs, U32& rOutNumberOfOutputs, U32*& rpOutOutputDepAttrs)
{
    ...
    U32*& rpOutOutputDepAttrs)
{
    ...
    else if ((pInOutputDID == &DID_IFXLightSet) || (pInOutputDID == &DID_IFXFrustum) || (pInOutputDID == &DID_IFXViewSize) || (pInOutputDID == &DTS_IFXRenderable) || (pInOutputDID == &DTS_IFXBound) || (pInOutputDID == &DID_IFXBoundFrame) || (pInOutputDID == &DID_IFXBoundFrameBounds))
    {
...
IFXRESULT CIFXModel::GenerateOutput(U32 inOutputDataElementIndex, void*& rpOutData, BOOL& rNeedRelease)
{
    IFXRESULT result = IFX_OK;
        ...
        if (!m_bProcessingBoundFrameBounds)
        {
            ...
            if (IFXSUCCESS(GetModifierChain(&pModifierChain)))
            {
                ...
                if (IFXSUCCESS(pModifierChain->GetDataPacket(pDP)))
                {
                    ...
                    if (IFXSUCCESS(pDP->GetIterator(IFX_DID_BOUND, &pRenderableBoundIter)))
                    {
                        IFXDECLARELOCAL(IFXBoundSphereDataElement, pBound);
                        IFXDECLARELOCAL(IFXUnknown, pBoundUnk);
...
IFXRESULT CIFXModel::SetDataPacket(IFXModifierDataPacket* pInInputDataPacket, IFXModifierDataPacket* pInOutputDataPacket)
{
    ...
    if (pInInputDataPacket && pInOutputDataPacket)
    {
        if (IFXSUCCESS(result))
        {
            result = pInOutputDataPacket->GetDataElementIndex(DID_IFXLightSet, m_uLightSetDataElementIndex);
        }
        if (IFXSUCCESS(result))
        {
            result = pInOutputDataPacket->GetDataElementIndex(DID_IFXFrustum, m_uFrustumDataElementIndex);
        }
        if (IFXSUCCESS(result))
        {
            result = pInOutputDataPacket->GetDataElementIndex(DID_IFXViewSize, m_uViewSizeDataElementIndex);
        }
        if (IFXSUCCESS(result))
        {
            result = pInOutputDataPacket->GetDataElementIndex(DID_IFXBoundFrame, m_uBoundFrameDataElementIndex);
        }
        if (IFXSUCCESS(result))
        {
            result = pInOutputDataPacket->GetDataElementIndex(DID_IFXBoundFrameBounds, m_uBoundFrameBoundsDataElementIndex);
        }
        if (IFXSUCCESS(result))
        {
...
IFXRESULT CIFXModel::Notify(IFXModifierMessage eInMessage, void* pMessageContext)
{
...
IFXRESULT CIFXModel::GetAssociatedSpatial(U32 uInSpatialIndex, IFXSpatial** ppOutSpatial, IFXSpatialAssociation::Type eInAssociationType)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT CIFXModel::SetAssociatedSpatials(SPATIALINSTANCE_LIST* pInSpatials, U32 uInSpatialCount, IFXSpatialAssociation::Type eInAssociationType)
{
    IFXRESULT result = IFX_OK;
    ...
    if (eInAssociationType == LIGHTS)
    {
        if (!m_pLightSet)
        {
            result = IFXCreateComponent(CID_IFXLightSet, IID_IFXLightSet, (void**)&m_pLightSet);
        }
        ...
        {
            result = m_pModifierDataPacket->InvalidateDataElement(m_uLightSetDataElementIndex);
        }
    }
    else
    {
...
IFXRESULT CIFXModel::AppendAssociatedSpatials(SPATIALINSTANCE_LIST* pInSpatials, U32 uInSpatialCount, IFXSpatialAssociation::Type eInAssociationType)
{
    ...
    if (eInAssociationType == LIGHTS)
    {
        ...
        {
            result = SetAssociatedSpatials(pInSpatials, uInSpatialCount, eInAssociationType);
        }
    }
    else
    {
...
IFXRESULT CIFXModel::AttachToModChainForWorldPositionChange()
{
    ...
    if (m_pMyDataPacketAsSubjectNR && m_pModifierDataPacket)
    {
        ...
        if (IFXSUCCESS(rc))
        {
            rc = m_pMyDataPacketAsSubjectNR->Attach(m_pMyIFXObserverNR, uBits, IID_IFXDataPacket);
        }
    }
...
IFXRESULT CIFXModel::Update(IFXSubject* pInSubject, U32 uInChangeBits, IFXREFIID rIType)
{
    ...
    if ((pInSubject == m_pMyDataPacketAsSubjectNR) && (uInChangeBits & m_uMyDataPacketTransformAspectBit))
    {
        m_pModifierDataPacket->InvalidateDataElement(m_uBoundFrameDataElementIndex);
        m_pModifierDataPacket->InvalidateDataElement(m_uBoundFrameBoundsDataElementIndex);
    ...
    else if ((pInSubject == m_pResourceSubjectNR) && (IFX_SUBJECT_PALETTE_REMOVE_RESOURCE & uInChangeBits))
    {
