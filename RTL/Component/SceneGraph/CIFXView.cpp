...
public:
    ...
    IFXRESULT Allocate()
    {
        ...
        if (NULL == m_ppShaders)
        {
            m_ppShaders = (IFXShader**)IFXAllocate(INITIAL_SIZE * sizeof(IFXShader*));
...
protected:
    IFXRESULT Reallocate()
    {
        ...
        if (NULL != m_ppShaders)
        {
            ...
            memset(&m_ppShaders[m_shadersUsed], 0, (m_shadersAllocated - m_shadersUsed) * sizeof(IFXShader*));
        }
        else
        {
            result = IFX_E_OUT_OF_MEMORY;
...
IFXRESULT CIFXView::GetOutputs(IFXGUID**& rpOutOutputs, U32& rOutNumberOfOutputs, U32*& rpOutOutputDepAttrs)
{
...
IFXRESULT CIFXView::GenerateOutput(U32 inOutputDataElementIndex, void*& rpOutData, BOOL& rNeedRelease)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT CIFXView::Notify(IFXModifierMessage eInMessage, void* pMessageContext)
{
    return IFX_OK;
}
...
IFXRESULT CIFXView::WindowToFilm(F32 x, F32 y, IFXVector3* pFilmPt)
{
    ...
    switch (m_View.GetProjectionMode())
    {
        ...
        case IFX_ORTHOGRAPHIC:
        {
            ...
            {
                worldUnitsPerPixel = m_View.GetOrthoHeight() / m_rcRenderViewport.m_Height;
            }
...
IFXRESULT CIFXView::FrustumToWindow(const IFXVector3* pVector, I32* const pX, I32* const pY) const
{
    ...
    if ((ZC <= -m_nearClip) && (ZC >= -m_farClip))
    {
        ...
        switch (m_View.GetProjectionMode())
        {
            case IFX_PERSPECTIVE3:
            {
                const F32 perspectiveScale = m_firstDistanceToProjectionPlane / -pVector->ZC();
            ...
            case IFX_ORTHOGRAPHIC:
            {
                const F32 pixelsPerWorldUnits = m_rcRenderViewport.m_Height / m_View.GetOrthoHeight();
        ...
        if ((0 <= ix) && (ix < m_rcRenderViewport.m_Width) && (0 <= iy) && (iy < m_rcRenderViewport.m_Height))
        {
            *pX = ix;
            *pY = iy;
        }
...
IFXRESULT CIFXView::RenderViewLayer(U32 uLayer, IFXRenderContext& rRenderContext, IFXRect* pViewport, F32 fScaleX, F32 fScaleY)
{
    ...
    while (pLayer && IFXSUCCESS(rc))
    {
        ...
        if (IFXSUCCESS(rc) && pLayer->m_bTextureDirty)
        {
            ...
            if (IFXSUCCESS(rc))
            {
                pLayer->m_fTexSizeScaleX = (F32)uVal;
            }
            IFXRUN(rc, pTexture->GetHeight(&uVal));
            if (IFXSUCCESS(rc))
            {
                pLayer->m_fTexSizeScaleY = (F32)uVal;
            }
                        ...
                        if (IFXSUCCESS(result))
                        {
                            if (IFXSUCCESS(pModel->GetModifierChain(&pModifierChain)))
                            {
                                ...
                                while (IFXSUCCESS(result) && pBound)
                                {
                                    ...
                                    {
                                        pRenderable = NULL;
                                        result = pDataPacket->GetDataElement(pBound->RenderableIndex(), IID_IFXRenderable, (void**)&pRenderable);
                                        if (IFXSUCCESS(result) && pRenderable && pRenderable->GetEnabled())
                                        {
                                                 ...
                                                 uMesh++)
                                            {
                                                     ...
                                                     uShader++)
                                                {
                                                    ...
                                                    if (IFXSUCCESS(result))
                                                    {
                                                        result = pShaderPalette->GetResourcePtr(uShaderID, IID_IFXShader, (void**)&pShader);
                                                    }
...
//
IFXRESULT CIFXView::GenerateRay(const F32 x, const F32 y, U32 ViewInstance, IFXVector3& pos, IFXVector3& dir)
{
...
{
    if ((m_attributes & IFX_PERSPECTIVE2) && (m_attributes & IFX_PERSPECTIVE1))
    {
        (*pProjVector).X() = 0.0f;
        (*pProjVector).Y() = 0.0f;
        (*pProjVector).Z() = 0.0f;
