...
IFXRESULT CIFXLightResource::SetAttenuation(F32* pAttenuation)
{
    ...
    if (pAttenuation)
    {
        ...
        if ((fConstant >= 0) && (fLinear >= 0) && (fQuadradic >= 0) && ((fConstant + fLinear + fQuadradic) > 0))
        {
...
void CIFXLightResource::CalculateRange(void)
{
    ...
    if (atten[QUADRADIC] > 0.0)
    {
        ...
        fExtent += (float)sqrt(fExtent * fExtent - 4.0f * m_scfAttenuationEpsilon * atten[QUADRADIC] * (m_scfAttenuationEpsilon * atten[CONSTANT] - 1.0f));
        fExtent /= (2 * m_scfAttenuationEpsilon * atten[QUADRADIC]);
    }
    else if (atten[LINEAR] > 0.0)
    {
        fExtent = (1.0f - m_scfAttenuationEpsilon * atten[CONSTANT]) / (m_scfAttenuationEpsilon * atten[LINEAR]);
    }
    else
    {
