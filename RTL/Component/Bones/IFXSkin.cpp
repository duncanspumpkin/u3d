...
******************************************************************************/
void IFXSkin::FindNeighbors(IFXMeshInterface* mesh, IFXArray<IFXMeshVertexMap>& neighbors)
{
...
void IFXSkin::FilterWeights(void)
{
            ...
            while ((mapping1 = map[meshid][vertex].PostIncrement(context1)) != NULL)
            {
                       ...
                       != NULL)
                {
                    ...
                    if (bone1 == bone2)
                    {
                        ...
                        IFXTRACE_GENERIC(L"%d ", m_vertexweights[*mapping1].GetVertexIndex());
                        IFXTRACE_GENERIC(L"%d", m_vertexweights[*mapping2].GetVertexIndex());
    ...
    for (m = 0; m < length; m++)
    {
        ...
        if (remove || m_vertexweights[m].GetBoneWeight() < IFXSKIN_FILTER_MIN || m_vertexweights[m].GetBoneIndex() < 0 || m_vertexweights[m].GetBoneIndex() >= m_character->GetBoneTableSize())
        {
#if IFXSKIN_FILTER_DEBUG
            IFXTRACE_GENERIC(L"remove %d %d:%d ", m, meshid, m_vertexweights[m].GetVertexIndex());
            IFXTRACE_GENERIC(L"of %d ", vertices);
            IFXTRACE_GENERIC(L"bone %d/%d weight %.6G\n", m_vertexweights[m].GetBoneIndex(), m_character->GetBoneTableSize(), m_vertexweights[m].GetBoneWeight());
#endif
                ...
                if (bonenode && (!use_joints || bonenode->IsInfluential()))
                {
                    ...
                    for (pass = 0; pass < 2; pass++)
                    {
                        ...
                        else
                        {
                            ...
                            if (!(bonenode->Displacement() == zero) && bonenode->Parent()->IsBone())
                            {
                                IFXBoneNode* parent = (IFXBoneNode*)bonenode->Parent();

                                IFXTransform implant = parent
                                                           ->StoredTransform();
                                implant.Translate(parent->GetLength(), 0.0f, 0.0f);
                                implant.Rotate(
                                ...
                                IFXCylinder& parentbounds = parent->CylindricalBounds();
                                IFXVector3& parentscale = parentbounds
                                                              .GetScale(1);
                                basejointsize = 0.5f * (parentscale[1] + parentscale[2]);
                        ...
                        if (result[0] > 0.0f)
                        {
                            ...
                            else
                            {
                                ...
                                if (bonelength > 0.0f)
                                {
                                    jointsize = basejointsize + (tipjointsize - basejointsize) * result[0] / bonelength;
                                }
                                else
                                {
                        ...
                        {
                            IFXTRACE_GENERIC(L"vert %d:%d bone %d len %.6G %.6G, %.6G = %.6G", meshid, vertex, boneid, bonelength, basejointsize, tipjointsize, jointsize);
...
void IFXSkin::RemoveRogueWeights(void)
{
        ...
        if (bonenode->BestVertexScore() >= 0.0f)
        {
            ...
            if (meshid != 0xffffffff && vertid != 0xffffffff)
            {
                ...
                while ((mapping = map[meshid][vertid]
                                      .PostIncrement(longcontext))
                       != NULL)
                {
                    if (mapping && m_vertexweights[*mapping].GetBoneIndex() == boneid)
                    {
    ...
    for (m = 0; m < length; m++)
    {
        ...
        if (rVertexWeight.Offset()[1] == 0.0f)
        {
            ...
            while ((neighborvertex = neighbors[meshid][vertid].PostIncrement(context)) != NULL)
            {
                       ...
                       != NULL)
                {
                    ...
                    {
                        FloodFill(neighbors, map, false, m, m_vertexweights[*mapping].GetBoneIndex());
                        break;
                    }
                }
...
******************************************************************************/
void IFXSkin::FloodFill(const IFXArray<IFXMeshVertexMap>& neighbors, const IFXArray<IFXLongListArray>& map, BOOL scanfirst, I32 weightid, I32 newboneid)
{
    ...
    for (pass = scanfirst; pass >= 0; pass--)
    {
        ...
        while (floodstack.GetNumberElements() > 0)
        {
            ...
            IFXVertexWeight& rVertexWeight = m_vertexweights[floodstack[level].weightindex];
            ...
            if ((neighborvertex = neighbors[meshid][vertid]
                                      .PostIncrement(floodstack[level].context))
                != NULL)
            {
                I32 neighmesh = neighborvertex->GetMeshIndex();
                I32 neighvert = neighborvertex->GetVertexIndex();
                       ...
                       != NULL)
                {
                    ...
                    if ((scanfirst || neighboneid == oldboneid) && (m_vertexweights[*mapping].Offset()[1] == 0.0f || (!pass && neighboneid < 0)))
                    {
#if IFXSKIN_ROGUEPATCH_DEBUG
                        ...
                        IFXASSERT((U32)neighvert < neighbors[neighmesh].GetNumberElements());
...
void IFXSkin::SmoothWeights(I32 iterations, F32 threshhold, F32 weldmax, F32 modelsize)
{
        ...
        for (meshid = 0; meshid < meshes; meshid++)
        {
            ...
            for (vertex = 0; vertex < vertices; vertex++)
            {
                       ...
                       != NULL)
                {
                    ...
                    while ((neighborvertex = neighbors[meshid][vertex].PostIncrement(context2))
                           != NULL)
                    {
                        I32 neighmesh = neighborvertex->GetMeshIndex();
                        I32 neighvert = neighborvertex->GetVertexIndex();
                        ...
                        difference.Subtract(m_inmesh->GetVertexConst(vertex), neighvector);
                        ...
                        {
#if IFXSKIN_SMOOTH_DEBUG
                            BOOL show = (meshid == 0 && vertex == 485) || (meshid == 9 && vertex == 1) || (neighmesh == 0 && neighvert == 485) || (neighmesh == 9 && neighvert == 1);

                            if (show)
                            {
                                IFXTRACE_GENERIC(L"vert %d:%d bone %d to %d:%d ", meshid, vertex, boneid, neighmesh, neighvert);
                            ...
                            if (lastpass)
                            {
                                ...
                                {
                                    m_vertexweights[found].BoneWeight() = m_vertexweights[*mapping].GetBoneWeight();
                                }
                                else
                                {
                                    m_vertexweights[*mapping].BoneWeight() = m_vertexweights[found].GetBoneWeight();
                                }
                            }
                            else
                            {
...
BOOL IFXSkin::GetMasterMeshVertex(U32 meshid, U32 vertex, U32* mmeshid, U32* mvertex)
{
    if (meshid >= m_mastervertices.GetNumberElements() || vertex >= m_mastervertices[meshid].GetNumberElements())
    {
        return false;
    }
...
void IFXSkin::RelinkWeights(void)
{
    ...
    for (m = 0; m < length; m++)
    {
        ...
        if (show)
        {
            IFXTRACE_GENERIC(L"bone %d %ls\n", bonenode->GetBoneIndex(), bonenode->NameConst().Raw8());
        }
#endif
        ...
        for (n = start; n < end; n++)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        parcompeteweight += m_vertexweights[n].GetBoneWeight();
                    }
                }
            }
        }
        ...
        for (n = 0; n < ancestors; n++)
        {
            ...
            for (j = 0; j < ancestors; j++)
            {
                if (j != n)
                {
                    ...
                    if (otherbone->HasAncestor(ansbone))
                    {
                        F32 est_fraction = (childweight) / (childweight + ancestorcompete[n]);
                        ancestoranticompete[j] += est_fraction * m_vertexweights[ancestor[n]].GetBoneWeight();
                    }
                }
            }
        }
        ...
        for (n = 0; n < ancestors; n++)
        {
            F32 fraction = (childweight + ancestoranticompete[n]) / (childweight + ancestoranticompete[n] + ancestorcompete[n]);

            ancestorweight += fraction * m_vertexweights[ancestor[n]].GetBoneWeight();

#if IFXSKIN_RELINK_DEBUG
            if (show)
            {
                IFXTRACE_GENERIC(L"ans %d: compete = %.6G anticompete = %.6G\n", m_vertexweights[ancestor[n]].GetBoneIndex(), ancestorcompete[n], ancestoranticompete[n]);
                IFXTRACE_GENERIC(L"  fraction = %.6G of %.6G adds %.6G\n", fraction, m_vertexweights[ancestor[n]].GetBoneWeight(), fraction * m_vertexweights[ancestor[n]].GetBoneWeight());
            }
#endif
        }
#endif
        ...
        F32 parcompetefraction = parcompeteweight / (parcompeteweight + childweight);

#if IFXSKIN_RELINK_DEBUG
        if (show)
        {
            IFXTRACE_GENERIC(L"vertex = %d:%d ", rVertexWeight.GetMeshIndex(), vertid);
#if IFXSKIN_RELINK_BY_WEIGHTS
            IFXTRACE_GENERIC(L"ancestors = %d ", ancestors);
        ...
        CalcLocalOffset(boneid, m_vertexweights[m].OffsetConst(), &offset);
        ...
        F32 subfraction = (childweight / totalweight - substart) * (F32)(numlinks + 1);
#else
        ...
        if (parententry)
        {
            ...
            F32 average = IFXSQRT(jointscale[1] * jointscale[1] + jointscale[2] * jointscale[2]);
...
BOOL IFXSkin::CalcLocalOffset(I32 boneid, const IFXVector3& world, IFXVector3* offset)
{
    IFXBoneNode* bonenode = m_character->LookupBoneIndex(boneid);
    if (!bonenode)
    {
...
void IFXSkin::CalculateJointCrossSections(BOOL updatecache, BOOL computetips)
{
    ...
    m_character->ForEachNodeTransformed(IFXPARENTFIRST, &IFXSkin::CalculateJointsForBone, &computetips);
}

BOOL IFXSkin::CalculateJointsForBone(IFXCoreNode& node, IFXTransform& transform, IFXVariant state)
{
    if (!node.IsBone())
    {
        return false;
    ...
    {
        char progresstext[128];
        sprintf(progresstext, "Calc Joints %d%%", 100 * index / bonenode.RootCharacter()->GetBoneTableSize());
        IFXCharacter::StateProgress(progresstext);
    }
...
void IFXSkin::CalculateJointForBone(IFXCoreNode& node, IFXTransform& transform, BOOL tip, BOOL computetips)
{
    ...
    if (length > 0)
    {
        for (m = 0; m < length; m++)
        {
...
#endif
            {
                ...
                skin->CalcLocalOffset(boneid, m_vertexweights[m].OffsetConst(), &offset);
#else
                IFXVector3& offset = offsets.CreateNewElement();
        ...
        for (meshid = 0; meshid < meshes; meshid++)
        {
            ...
            for (m = 0; m < length; m++)
            {
                ...
                for (n = 0; n < weights; n++)
                {
                    ...
                    {
                        IFXVector3& offset = offsets.CreateNewElement();
                        skin->CalcLocalOffset(boneid, pPackVertex->OffsetConst(), &offset);
    ...
    for (pass = 0; pass < passes; pass++)
    {
        ...
        for (cycle = 0; cycle < 2; cycle++)
        {
                ...
                for (n = 0; n < 2; n++)
                {
                    ...
                    if (scale[n + 1] > axis)
                    {
                        ...
                        else
                        {
                            ...
                            if (change > 0.0f)
                            {
                                delta[n] += (originaloffset[n + 1] > 0.0) ? -change : change;
                            }
                        }
                    }
                }
            ...
            for (j = 1; j < 3; j++)
            {
                ...
                {
                    center[j] *= scale[j] * IFXJOINT_MAXDISPLACE / (F32)fabs(center[j]);
                }
...
void IFXSkin::DeformMesh(BOOL renormalize, BOOL clod)
{
    ...
    if (!m_vertexweights.GetNumberElements() && !m_packweightarray.GetNumberElements())
    {
        return;
    }
    ...
    m_character->ForEachNodeTransformed(IFXPARENTFIRST | IFXSTORE_XFORM, &IFXSkin::ComputeStretchNormals);

    m_character->ForEachNodeTransformed(IFXCHILDFIRST, &IFXSkin::AccumulateWeightedVertices);

    m_character->ForEachNodeTransformed(IFXPARENTFIRST, &IFXSkin::TweakWeightedVertices);
#endif
...
void IFXSkin::UnpackVertexWeights(void)
{
    ...
    for (meshid = 0; meshid < meshes; meshid++)
    {
        ...
        {
            IFXTRACE_GENERIC(L"WARNING: %d of %d vertices valid on mesh %d\n", m_inmesh->GetNumberVertices(), m_inmesh->GetMaxNumberVertices(), meshid);
        }
        ...
        for (m = 0; m < numverts; m++)
        {
            ...
            for (n = 0; n < weights; n++)
            {
                ...
                rVertexWeight.Offset().Set(pPackVertex->OffsetConst()[0], pPackVertex->OffsetConst()[1], pPackVertex->OffsetConst()[2]);
...
void IFXSkin::ComputeDeformedVerticesPacked(BOOL clod)
{
    ...
    for (meshid = 0; meshid < meshes; meshid++)
    {
        ...
        const I32 numverts = (clod) ? m_outmesh->GetNumberVertices() : m_outmesh->GetMaxNumberVertices();
        ...
        for (m = 0; m < length; m++)
        {
            ...
            cbonecachearray.GetElementConst(pPackWeight->GetBoneIndex())
                ...
                .TransformVertexAndNormal(
                    ...
                    pPackVertex->OffsetConst(),
                    accumulator,
                    pPackVertex->NormalOffsetConst(),
                    normaccum);
            ...
            outputIndexer->SetVertexAndNormal(pPackVertex->GetVertexIndex(), accumulator, normaccum);
...
void IFXSkin::NormalizeOutputNormals(void)
{
    ...
    for (meshid = 0; meshid < meshes; meshid++)
    {
        ...
        for (index = 0; index < normals; index++)
        {
            ...
            mag2 = (*normal)[0] * (*normal)[0] + (*normal)[1] * (*normal)[1] + (*normal)[2] * (*normal)[2];

#if IFXSKIN_USE_SQRT_TABLE
            IFXASSERT((I32)(mag2 * (IFXSKIN_SQRT_SCALESIZE - 1)) < IFXSKIN_SQRT_TABLESIZE);
