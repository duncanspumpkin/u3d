...
IFXCharacter::IFXCharacter(IFXCharacter* pCloneOf)
...
void IFXCharacter::CopyBoneHierarchy(IFXCoreNode* pParent, IFXCoreNode* pNewParent)
{
...
IFXCoreNode* IFXCharacter::FindDescendentByName2(IFXCoreNode& parent, const IFXString& rName)
{
...
void IFXCharacter::ForEachNode(I32 flags, IFXForEachNodeCB callback, IFXVariant state)
{
    BOOL stop = false;
...
void IFXCharacter::ForEachNode2(I32 flags, IFXCoreNode& parent, IFXForEachNodeCB callback, IFXVariant state)
{
...
void IFXCharacter::ForEachNodeDual(IFXForEachNodeCB preCallback, IFXForEachNodeCB postCallback, IFXVariant state)
{
    BOOL stop = false;
    if (preCallback)
    {
...
void IFXCharacter::ForEachNodeDual2(IFXCoreNode& parent, IFXForEachNodeCB preCallback, IFXForEachNodeCB postCallback, IFXVariant state)
{
    IFXBoneNodeList& childlist = parent.Children();
    if (!childlist.GetNumberElements())
    {
...
*/
void IFXCharacter::ForEachNodeTransformed2(I32 flags, IFXCoreNode& rParent, IFXForEachNodeTransformedCB callback, IFXVariant state)
{
...
BOOL IFXCharacter::BlendBoneNode(IFXCoreNode& node, IFXVariant state)
{
    ...
    pos.Interpolate(*weight, node.BlendDisplacementConst(), node.DisplacementConst());
    rot.Interpolate(*weight, node.BlendRotationConst(), node.RotationConst());
    scale.Interpolate(*weight, node.BlendScaleConst(), node.ScaleConst());
...
BOOL IFXCharacter::StoreReferenceForBone(IFXCoreNode& node, IFXTransform& transform, IFXVariant state)
{
    if (!node.IsBone())
    {
        return false;
...
void IFXCharacter::ApplyIK(void)
{
    ...
    {
        ForEachNodeTransformed(IFXCHILDFIRST | IFXSTORE_XFORM, &IFXCharacter::KineBone, &ik_modes);
    }
...
BOOL IFXCharacter::KineBone(IFXCoreNode& node, IFXTransform& transform, IFXVariant state)
{
    if (!node.IsBone())
    {
        return false;
    ...
    if (world_relative)
    {
        ...
        global_rot.Multiply(parent->StoredTransform().QuaternionConst(), thisbone.RotationConst());
        ...
        IFXTRACE_GENERIC(L"parent Gcur %ls\n", parent->StoredTransform().QuaternionConst().Out(true).Raw());
    ...
    while ((node = childlist.PostIncrement(context)) != NULL)
    {
        ...
        if (lookback)
        {
            ...
            else
            {
                ...
                m_transforms.GetCurrent().ReverseTransformVector(child_location, localvector);
            }
        }
        else
        {
            ...
            if (grandchild)
            {
                ...
                m_transforms.GetCurrent().ReverseRotateVector(worldvector, localvector);
            }
            else
            {
                worldvector.Reset();
...
{
    ForEachNodeTransformed(IFXPARENTFIRST | IFXSTORE_XFORM, &IFXCharacter::CalculateImplantReferenceForBone);
}

BOOL IFXCharacter::CalculateImplantReferenceForBone(IFXCoreNode& node, IFXTransform& transform, IFXVariant state)
{
    if (!node.IsBone())
    {
        return false;
