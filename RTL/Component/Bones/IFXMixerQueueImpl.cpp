...
IFXRESULT IFXMixerQueueImpl::Queue(const IFXString& rName, F32* localOffset, F32* localStartTime, F32* localEndTime, F32* timeScale, BOOL* loop, BOOL* sync, BOOL delayMap)
{
    return Push(rName, localOffset, localStartTime, localEndTime, timeScale, loop, sync, delayMap, true);
}

IFXRESULT IFXMixerQueueImpl::Push(const IFXString& rName, F32* localOffset, F32* localStartTime, F32* localEndTime, F32* timeScale, BOOL* loop, BOOL* sync, BOOL delayMap, BOOL tail)
{
...
#if IFXMQ_QUEUEDEBUG
    IFXTRACE_GENERIC(L"Push/Queue \"%ls localOffset=%.6G localStartTime=%.6G localEndTime=%.6G\n timeScale=%.6G loop=%d sync=%d m_lastTime=%.6G worldTime=%.6G\n", rName.Raw(), localOffset ? *localOffset : 0.0f, localStartTime ? *localStartTime : 0.0f, localEndTime ? *localEndTime : 0.0f, timeScale ? *timeScale : 0.0f, loop ? *loop : 0, sync ? *sync : 0, m_lastTime, m_pBonesManager ? GetWorldTime() : 0);
#endif
    ...
    if (localOffset)
    {
        offset = *localOffset + (GetWorldTime() - m_lastTime) * (timeScale ? *timeScale : 1.0f);
    }
    ...
    if (!IFXSUCCESS(iResult))
    {
        ...
        if (oldwrap)
        {
            oldwrap->m_pMotionMixer->EventState() = IFXMotionMixer::IFXSX_Normal;
        }
    }
...
#if IFXMQ_QUEUEDEBUG
    IFXTRACE_GENERIC(L"Pop \"%ls\" %.6G\n", GetMotionMixerNR(0)->GetPrimaryMotionName().Raw(), m_lastTime);
#endif
    const IFXMixerWrap& oldwrap = (tail) ? m_queue.ElementFromTailConst(0) : m_queue.ElementFromHeadConst(0);
...
void IFXMixerQueueImpl::TransitionHead(bool push)
{
    ...
    if (GetNumberQueued())
    {
        ...
        iResult = GetMotionMixerNR(0)->CalcBonePositionAtTime(0, m_lastTime, &location, &rotation, &scale);
    }
    else
    {
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        if (GetNumberQueued())
        {
            IFXTRACE_GENERIC(L"Transition 0x%x %ls %.6G\n", m_concatenation, GetMotionMixerNR(0)->GetPrimaryMotionName().Raw(), m_lastTime);
            IFXTRACE_GENERIC(L"old %ls new %ls\n", old_location.Out().Raw(), location.Out().Raw());
        }
#endif
...
IFXRESULT IFXMixerQueueImpl::TransitionToMixer(IFXMotionMixer* pSkelMixer)
{
    ...
    IFXRESULT iResult = m_pBonesManager->BlendToMotionMixer(pMotionMixer, &blendparam);
    IFXRETURN(iResult);
#else
...
void IFXMixerQueueImpl::VerifyQueue(void)
{
    ...
    for (m = 0; m < length; m++)
    {
        ...
        {
            IFXTRACE_GENERIC(L"Warning: SkelMixer[%d]=NULL on SkelParam %p", m, this);
        }
    }
#endif
}
...
IFXRESULT IFXMixerQueueImpl::Advance()
{
    ...
    if (!AutoBlend())
    {
        m_pBonesManager->SetFloat(IFXBonesManagerImpl::BlendFraction, BlendFraction());
    }
    ...
    if (!m_noloop && !m_noadjust && GetNumberHistory())
    {
        ...
        while (GetNumberHistory())
        {
            ...
            {
                if (pSkelMixer->EventState() == IFXMotionMixer::IFXSX_WaitEnd && pSkelMixer->Started())
                {
    ...
    {
        F32 startTime = 0.0f;
        m_pBonesManager->GetFloat(IFXBonesManagerImpl::BlendStartTime, &startTime);
        startTime += timeDiff;
        m_pBonesManager->SetFloat(IFXBonesManagerImpl::BlendStartTime, startTime);
    }
    ...
    for (m = 0; m < length; m++)
    {
        ...
        //* scale adjustment
        IFXMotionMixer* pSkelMixer = (U32(m) < history) ? UseHistoricalMixer(m) : GetMotionMixerNR(m - history);
    ...
    {
        IFXMotionMixer* pSkelMixer = (m < 0) ? UseHistoricalMixer(history - 1) : GetMotionMixerNR(m);
        ...
        if (!m_noloop && !m_noadjust && playing && pSkelMixer->EventState() == IFXMotionMixer::IFXSX_WaitStart)
        {
...
#if IFXMQ_QUEUEDEBUG
            IFXTRACE_GENERIC(L"On Mixer %p->%p", pSkelMixer, pSkelMixer);
    ...
    for (m = 0; m < (I32)((history = GetNumberHistory()) + GetNumberQueued()); m++)
    {
        ...
        if (!m_noloop && !m_noadjust && (!GetNumberQueued() || pSkelMixer != pBlendMixer))
        {
            F32 mixerTime = 0.0f;
            pSkelMixer->GetTimeOffset(&mixerTime);
            // F32 localTime=worldTime-mixerTime;
...
void IFXMixerQueueImpl::CalcMotionLimits(IFXMotionMixer* pSkelMixer, F32* pMin, F32* pMax)
{
    F32 min = 0.0f, max = 0.0f;
...
void IFXMixerQueueImpl::AdjustForGap(I32 index, F32 time1, F32 time2)
{
    ...
    F32 offset = 0.0f;
    IFXMotionMixer* pSkelMixer = (index < 0) ? UseHistoricalMixer(GetNumberHistory() + index) : GetMotionMixerNR(index);

    pSkelMixer->GetTimeOffset(&offset);
    pSkelMixer->CalcBonePositionAtTime(0, offset + time1, &firstframe, &rotation[0], &scale[0]);

    pSkelMixer->CalcBonePositionAtTime(0, offset + time2, &lastframe, &rotation[1], &scale[1]);
...
#if IFXMQ_LOOPDEBUG
        IFXTRACE_GENERIC(L"Gap Translation %ls to %ls\n  (%ls to %ls)\n", change.Out().Raw8(), old_location.Out().Raw8(), firstframe.Out().Raw8(), lastframe.Out().Raw8());
#endif
    }
...
void IFXMixerQueueImpl::IFXMixerWrap::Set(const IFXString& rPrimaryName, const F32* pOffset, const F32* pLocalStartTime, const F32* pLocalEndTime, const F32* pTimeScale, const BOOL* pLoop, const BOOL* pSync)
{
...
void IFXMixerQueueImpl::IFXMixerWrap::Run(F32 worldTime, F32 fsync)
{
    ...
    {
#if IFXMQ_QUEUEDEBUG
        IFXTRACE_GENERIC(L"Run() \"%ls\" %.6G+%.6G\n", m_pMotionMixer->GetPrimaryMotionName().Raw(), m_pMotionMixer->QueueOffset(), worldTime);
#endif
    ...
    if (m_pMotionMixer->Sync())
    {
        ...
        motionmixer->SetTimeOffset(m_pMotionMixer->QueueOffset() + worldTime - (max - min) * fsync);
    }
}
    ...
    IFXPlaylistSummary* pPlaylistSummary)
{
    ...
    if (IFXSUCCESS(rc))
    {
        ...
        for (i = 0; (i < uNumMixers) && (i < pPlaylistSummary->m_uMixerCount); i++)
        {
            ...
            {
                pPlaylistSummary->m_MixerSummaryArray[i].m_name = pMixer->GetPrimaryMotionName();
                pPlaylistSummary->m_MixerSummaryArray[i].m_localOffset = pMixer->QueueOffset();
                pPlaylistSummary->m_MixerSummaryArray[i].m_localStartTime = 0.0f;
                pPlaylistSummary->m_MixerSummaryArray[i].m_localEndTime = -1.0f;
                pPlaylistSummary->m_MixerSummaryArray[i].m_timeScale = pMixer->TimeScale();
    ...
    const IFXPlaylistSummary* pPlaylistSummary)
{
    ...
    for (i = 0; (i < pPlaylistSummary->m_uMixerCount) && (IFXSUCCESS(rc)); i++)
    {
        ...
        rc = Queue(pPlaylistSummary->m_MixerSummaryArray[i].m_name, NULL, NULL, NULL, &pPlaylistSummary->m_MixerSummaryArray[i].m_timeScale, &pPlaylistSummary->m_MixerSummaryArray[i].m_loop, &pPlaylistSummary->m_MixerSummaryArray[i].m_sync);
    }
