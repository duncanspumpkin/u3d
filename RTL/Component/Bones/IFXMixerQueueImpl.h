...
public:
    ...
    virtual IFXRESULT IFXAPI Push(const IFXString& rName, F32* localOffset, F32* localStartTime, F32* localEndTime, F32* timeScale, BOOL* loop, BOOL* sync, BOOL delaymap = false, BOOL tail = false);
    virtual IFXRESULT IFXAPI Queue(const IFXString& rName, F32* localOffset, F32* localStartTime, F32* localEndTime, F32* timeScale, BOOL* loop, BOOL* sync, BOOL delayMap = false);
...
private:
    ...
    void CalcMotionLimits(IFXMotionMixer* pSkelMixer, F32* pMin, F32* pMax);
