...
void IFXSkin::ComputeDeformedVerticesPackedSSE(BOOL clod)
{
    ...
    for (meshid = 0; meshid < meshes; meshid++)
    {
        ...
        for (m = 0; m < length; m++)
        {
            ...
            cbonecachearray.GetElementConst(packweight->GetBoneIndex()).GetGlobalMatrixConst().TransformVertexAndNormal(packweight->GetBoneWeight(), packvertex->OffsetConst(), accumulator, packvertex->NormalOffsetConst(), normaccum);
            ...
            for (n = 1; n < weights; n++)
            {
                ...
                cbonecachearray.GetElementConst(packweight->GetBoneIndex()).GetGlobalMatrixConst().CumulativeTransformVertexAndNormal(packweight->GetBoneWeight(), packvertex->OffsetConst(), accumulator, packvertex->NormalOffsetConst(), normaccum);
