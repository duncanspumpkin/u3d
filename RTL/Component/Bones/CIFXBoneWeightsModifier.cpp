...
IFXRESULT IFXAPI_CALLTYPE CIFXBoneWeightsModifier_Factory(IFXREFIID interfaceId, void** ppInterface)
{
    IFXRESULT rc = IFX_OK;
    if (ppInterface)
    {
...
// IFXModifier
IFXRESULT CIFXBoneWeightsModifier::GetOutputs(IFXDID**& rpOutOutputs, U32& rOutNumberOfOutputs, U32*& rpOutOutputDepAttrs)
{
...
IFXRESULT CIFXBoneWeightsModifier::GetDependencies(IFXGUID* pInOutputDID, IFXGUID**& rppOutInputDependencies, U32& rOutNumberInputDependencies, IFXGUID**& rppOutOutputDependencies, U32& rOutNumberOfOutputDependencies, U32*& rpOutOutputDepAttrs)
{
...
IFXRESULT CIFXBoneWeightsModifier::GenerateOutput(U32 inOutputDataElementIndex, void*& rpOutData, BOOL& rNeedRelease)
{
    IFXRESULT rc = IFX_OK;
    ...
    IFXModifierDataPacket* pInDataPacket)
{
    ...
    if (IFXSUCCESS(rc))
    {
        rc = pInDataPacket->GetDataElementIndex(DID_IFXBoneWeights, m_BoneWeightsIndex);
    }

    IFXRETURN(rc);
}

IFXRESULT CIFXBoneWeightsModifier::Notify(IFXModifierMessage eInMessage, void* pMessageContext)
{
    return IFX_OK;
}
    ...
    IFXMeshMap** ppMeshMap)
{
    ...
    if (IFXFAILURE(pMod2->QueryInterface(IID_IFXAuthorLineSetResource, (void**)&pLS)))
    {
        ...
        if (IFXSUCCESS(rc))
        {
            rc = pUnk->QueryInterface(IID_IFXAuthorLineSetResource, (void**)&pLS);
        }
    }
    ...
    IFXMeshMap** ppMeshMap)
{
    ...
    if (IFXFAILURE(pMod2->QueryInterface(IID_IFXAuthorCLODResource, (void**)&pCLOD)))
    {
        ...
        if (IFXSUCCESS(rc))
        {
            rc = pUnk->QueryInterface(IID_IFXAuthorCLODResource, (void**)&pCLOD);
        }
    }
...
IFXRESULT CIFXBoneWeightsModifier::Initialize(void)
{
    ...
    if (IFXSUCCESS(rc) && m_uABWSize > 0)
    {
        ...
        if (IFXSUCCESS(rc))
        {
            ...
            if (IFXSUCCESS(rc))
            {
                rc = m_pInputDataPacket->GetDataElementIndex(DID_IFXBonesManager, m_BonesManagerIndex);
            }

            if (IFXSUCCESS(rc))
            {
                rc = m_pInputDataPacket->GetDataElement(m_BonesManagerIndex, (void**)&m_pBonesMgr);
            }
            ...
            if (IFXSUCCESS(rc))
            {
                ...
                if (IFXSUCCESS(rc))
                {
                    ...
                    if (IFXSUCCESS(rc))
                    {
                        m_pBoneWeights = &pCharacter->GetSkin()->GetPackVertexWeights(this);
                    ...
                    for (j = 0; j < uTotal; j++)
                    {
                        ...
                        if (IFXSUCCESS(rc))
                        {
                            if (uMeshIdx == 0)
                            {
                                ...
                                uVertCnt++;
                                uWeightCnt += m_pAuthorBoneWeights[i].GetWeightCnt();
                            }
                            else
                            {
            ...
            if (IFXSUCCESS(rc) && uMeshIdxMax > 0)
            {
                    ...
                    for (j = 0; j < uTotal; j++)
                    {
                        ...
                        if (IFXSUCCESS(rc))
                        {
                            if (uMeshIdx > 0)
                            {
                                ...
                                pVertCnt[uMeshIdx]++;
                                pWeightCnt[uMeshIdx] += m_pAuthorBoneWeights[i].GetWeightCnt();
                            }
                        }
                        else
                        {
                ...
                for (i = 1; i < uMeshIdxMax + 1; i++)
                {
                    if (IFXSUCCESS(rc))
                    {
                        rc = this->SetTotalBoneWeightCount(i, pVertCnt[i], pWeightCnt[i]);
                    }
                    else
                    {
                        i = uMeshIdxMax + 1;
                        ...
                        if (IFXSUCCESS(rc))
                        {
                            ...
                            if (uCnt)
                            {
                                ...
                                else
                                {
                                    ...
                                    rc = this->SetBoneWeights(j, i, uCnt, puIDs, pfW);
                                }
                            }
                        }
                        else
...
IFXRESULT CIFXBoneWeightsModifier::SetTotalBoneWeightCount(U32 uMeshIndex, U32 uTotalVertexCount, U32 uTotalWeightCount)
{
    IFXRESULT rc = IFX_OK;
    ...
    if (IFXSUCCESS(rc))
    {
        ...
        //      m_pBoneWeights->ResizeToAtLeast(uMeshIndex+1);
        (*m_pBoneWeights)[uMeshIndex].Allocate(uTotalVertexCount, uTotalWeightCount);
    }

    IFXRETURN(rc);
}

IFXRESULT CIFXBoneWeightsModifier::GetTotalBoneWeightCount(U32 meshIndex, U32* pTotalWeightCount)
{
    IFXRESULT rc = IFX_OK;
...
IFXRESULT CIFXBoneWeightsModifier::SetBoneWeights(U32 uVertexIndex, U32 uMeshIndex, U32 uWeightCount, U32* puBoneIDs, F32* pfBoneWeights)
{
    IFXRESULT rc = IFX_OK;
...
IFXRESULT CIFXBoneWeightsModifier::GotoVertex(U32 uVertexIndex, U32 uMeshIndex)
{
    ...
    if (uVertexIndex < m_lastGoodVertID[uMeshIndex] || m_lastReadOffset[uMeshIndex] != rPackWeights.GetReadOffset())
    {
    ...
    while (IFXSUCCESS(rc) && (!m_pLastVertex[uMeshIndex] || m_pLastVertex[uMeshIndex]->GetVertexIndex() < uVertexIndex))
    {
        {
            U8* pLastVertex = (U8*)(m_pLastVertex[uMeshIndex]);
            U8* pLastRead = (U8*)rPackWeights.GetBufferAddr() + m_lastReadOffset[uMeshIndex] - sizeof(IFXPackVertex);
        ...
        {
            m_lastGoodVertID[uMeshIndex] = m_pLastVertex[uMeshIndex]->GetVertexIndex();
        }

        //* if this ain't the one, skip over the associated weights
        if (IFXSUCCESS(rc) && (m_pLastVertex[uMeshIndex]->GetVertexIndex() < uVertexIndex))
        {
            U32 const wCount = m_pLastVertex[uMeshIndex]->GetNumWeights();
...
IFXRESULT CIFXBoneWeightsModifier::GetBoneWeightCount(U32 uVertexIndex, U32 uMeshIndex, U32* puWeightCount)
{
    IFXRESULT rc = IFX_OK;
...
IFXRESULT CIFXBoneWeightsModifier::GetBoneWeights(U32 uVertexIndex, U32 uMeshIndex, U32 uWeightCount, U32* puBoneIDs, F32* pfBoneWeights)
{
    IFXRESULT rc = IFX_OK;
    if (!puBoneIDs && !pfBoneWeights)
    {
    ...
    if (IFXSUCCESS(rc))
    {
        ...
        for (m = 0; m < uWeightCount; m++)
        {
            IFXPackBoneWeight const* pw = rPackWeights.NextPackBoneWeightForRead();
            m_lastReadOffset[uMeshIndex] = rPackWeights.GetReadOffset();
            if (!pw)
            {
...
IFXRESULT CIFXBoneWeightsModifier::SetBoneWeightsForAuthorMesh(U32 vertexIndex, U32 weightCount, U32* pBoneIDs, F32* pBoneWeights)
{
    IFXRESULT res = IFX_OK;
