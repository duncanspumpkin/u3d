...
IFXRESULT IFXMeshGroup_Character::AdoptMeshGroup(IFXMeshGroup* mg, IFXVertexMapGroup* vmg)
{
...
void IFXMeshGroup_Character::AllocOutMesh(void)
{
    ...
    m_share->m_outmeshgroup->TransferData(*m_share->m_inmeshgroup, ~IFXMeshAttributes() ^ (IFXMeshAttributes(IFX_MESH_POSITION) | IFXMeshAttributes(IFX_MESH_NORMAL)), IFXMeshAttributes(IFX_MESH_POSITION) | IFXMeshAttributes(IFX_MESH_NORMAL), TRUE);
}
...
******************************************************************************/
IFXRESULT IFXMeshGroup_Character::CleanupWeights(bool regenerate, bool removerogueweights, bool smooth, F32 threshhold, F32 weldmax, F32 modelsize)
{
    ...
    if (regenerate)
    {
        ...
        GetSkin()->SmoothWeights(IFX_MGCHAR_SMOOTH_ITERATIONS_REGEN, threshhold, weldmax, modelsize);
    ...
    if (smooth)
    {
        ...
        GetSkin()->SmoothWeights(IFX_MGCHAR_SMOOTH_ITERATIONS_FINAL, threshhold, weldmax, modelsize);
    }
...
******************************************************************************/
IFXRESULT IFXMeshGroup_Character::GetMasterMeshIndex(U32 mesh, U32 vertex, U32* mesh0, U32* vertex0)
{
...
IFXRESULT IFXMeshGroup_Character::AdoptVertexMap(void)
{
    ...
    for (map = 0; map < maps; map++)
    {
        ...
        for (original = 0; original < originals; original++)
        {
            ...
            for (n = 0; n < copies; n++)
            {
                IFXRESULT result = vertexmap->GetVertexCopy(original, n, &meshid2, &vertex2);
                IFXASSERT(result == IFX_OK);

                // replicants and mastervertices should be same size
                if (result != IFX_OK || meshid2 >= (U32)m_backmap.GetNumberElements() || vertex2 >= (U32)m_backmap[meshid2].GetNumberElements())
                {
                ...
                else
                {
                    ...
                    **(replist->Append(new IFXMeshVertex)) = mv;
                    ...
                    mastervertices[meshid2][vertex2].SetMeshVertex(meshid, vertex);
                }
            }
        }
    }
...
void IFXMeshGroup_Character::FindOverlap(F32 maxweld)
{
    ...
    {
        n = m + 1;
        while (n < length - 1 && (weights[n].Offset()[2] - weights[m].Offset()[2]) < maxweld)
        {
...
void IFXMeshGroup_Character::PrintIFXMeshGroup(IFXMeshGroup_Impl* img)
{
    ...
    for (mesh = 0; mesh < meshes; mesh++)
    {
        ...
        for (vert = 0; vert < verts; vert++)
        {
            ...
            IFXTRACE_GENERIC(L" position %.6G %.6G %.6G", img->GetVertexConst(vert)[0], img->GetVertexConst(vert)[1], img->GetVertexConst(vert)[2]);
            IFXTRACE_GENERIC(L" normal %.6G %.6G %.6G\n", img->GetNormalConst(vert)[0], img->GetNormalConst(vert)[1], img->GetNormalConst(vert)[2]);
        ...
        for (face = 0; face < faces; face++)
        {
            ...
            IFXTRACE_GENERIC(L" position %d %d %d\n", img->GetFaceVerticesConst(face)[0], img->GetFaceVerticesConst(face)[1], img->GetFaceVerticesConst(face)[2]);
        }
    }
}
