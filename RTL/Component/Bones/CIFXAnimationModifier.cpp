...
IFXRESULT CIFXAnimationModifier::QueryInterface(IFXREFIID interfaceId, void** ppInterface)
{
    IFXRESULT ir = IFX_OK;
...
// IFXModifier
IFXRESULT CIFXAnimationModifier::GetOutputs(IFXGUID**& rpOutOutputs, U32& rOutNumberOfOutputs, U32*& rpOutOutputDepAttrs)
{
    if (m_bIsKeyframe)
    {
    ...
    U32*& rpOutOutputDepAttrs)
{
    ...
    {
        IFXASSERTBOX(1, "CIFXAnimationModifier::GetDependencies() called with unknown output!");
...
IFXRESULT CIFXAnimationModifier::GenerateOutput(U32 inOutputDataElementIndex, void*& rpOutData, BOOL& rNeedRelease)
{
    ...
    if (m_pBonesManager)
    {
        ...
        else if (inOutputDataElementIndex == m_uMeshGroupDataElementIndex)
        {
            ...
            if (IFXSUCCESS(result))
            {
                result = pMeshGroupUnknown->QueryInterface(IID_IFXMeshGroup, (void**)&pInputMeshGroup);
            }

            if (m_uLastMeshGroupChangeCount != uMeshGroupChangeCount && IFXSUCCESS(result))
            {
            ...
            if (IFXSUCCESS(result))
            {
                ...
                result = m_pBonesManager->UpdateMesh(pInputMeshGroup, &m_pOutputMeshGroup);
...
IFXRESULT CIFXAnimationModifier::Notify(IFXModifierMessage eInMessage, void* pMessageContext)
{
    IFXRESULT result = IFX_OK;
...
void CIFXAnimationModifier::Setup(void)
{
    ...
    if (!m_motionManager.GetMotionPalette())
    {
        ...
        if (IFXSUCCESS(result))
        {
            result = m_pSceneGraph->GetPalette(IFXSceneGraph::MOTION, &pMotionResourcePalette);
            IFXASSERT(pMotionResourcePalette);

            result = m_pSceneGraph->GetPalette(IFXSceneGraph::MIXER, &pMixerResourcePalette);
            IFXASSERT(pMixerResourcePalette);

            m_motionManager.SetPalettes(pMotionResourcePalette, pMixerResourcePalette);
        }
    }
}
    ...
    I32 motionId, IFXMotion** ppMotion, IFXString* pSourcename)
{
    ...
    IFXRESULT result = m_pMotionResourcePalette->GetResourcePtr(motionId, IID_IFXMotionResource, (void**)&pMotionResource);
    ...
    const IFXString& rMixerName, IFXMotionMixer* pMotionMixer)
{
    ...
    for (m = 0; m < paletteSize; m++)
    {
        ...
        result = GetMotionPalette()->GetResourcePtr(m, IID_IFXMotionResource, (void**)&pResource);
        ...
        if (pMotion && (tracks = pMotion->GetNumberTracks()))
        {
            ...
            {
                IFXTRACE_GENERIC(L"Track %d:%d \"%ls\"\n", m, n, pMotion->GetTrack(n).GetName().Raw());
                IFXTRACE_GENERIC(L"\n");
            }
        }
    }
    ...
    for (m = 0; m < paletteSize; m++)
    {
        ...
        result = GetMixerPalette()->GetResourcePtr(m, IID_IFXMixerConstruct, (void**)&pMixCon);
    ...
    result = GetMixerPalette()->GetResourcePtr(mixConId, IID_IFXMixerConstruct, (void**)&pMixCon);
    ...
    IFXMixerConstruct* pMixCon, const IFXString& rBoneName)
{
    ...
    if (motionResource || !pConList->GetNumberElements())
    {
        ...
        IFXTRACE_GENERIC(L"find name=\"%ls\" resource=%p result==x%x id=%d\n", motionResource->GetMotionRef()->GetNameConst().Raw(), motionResource, result, motionId);
    ...
    IFXBoneNode* bonenode = (bonecore && bonecore->IsBone()) ? (IFXBoneNode*)bonecore : NULL;

    IFXListContext context;
    pConList->ToHead(context);
    while (IFXSUCCESS(result) && (pConMap = pConList->PostIncrement(context)) != NULL)
    {
        ...
        if (bonenode)
        {
            ...
            IFXBoneNode* othernode = (othercore && othercore->IsBone()) ? (IFXBoneNode*)othercore : NULL;
    ...
    F32 localTime)
{
    ...
    IFXCreateComponent(CID_IFXNotificationInfo, IID_IFXNotificationInfo, (void**)&pEvent);
    ...
    pCoreServices->GetNotificationManager(IID_IFXNotificationManager, (void**)&pNotificationManager);
    IFXASSERT(pNotificationManager);

#if TRUE
    IFXTRACE_GENERIC(L"SubmitEvent(%ls%ls,%.6G) at %.6G\n", startevent ? L"startAnim," : L"endAnim,  ", rName.Raw(), localTime, GetWorldTime());
#endif
