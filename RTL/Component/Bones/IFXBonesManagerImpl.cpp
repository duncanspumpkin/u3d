...
IFXMotionMixer* IFXBonesManagerImpl::MotionMixer(void)
{
    ...
    IFXASSERT(!m_pMotionMixer || 0 == m_pMotionMixer->Loop() || 1 == m_pMotionMixer->Loop());
...
{
    // check for corruption
    IFXASSERT(!m_pLastMixer || m_pLastMixer->Loop() == 0 || m_pLastMixer->Loop() == 1);
...
IFXRESULT IFXBonesManagerImpl::SetBool(BooleanProperty prop, bool set)
{
    ...
    if (IFXSUCCESS(res))
    {
        ...
        if (!m_joints_calced && set && (prop == IFXBonesManagerImpl::ShowJoints || prop == IFXBonesManagerImpl::ShowCylinders))
        {
            CalcJoints();
            m_joints_calced = true;
        }

        if (prop >= IFXBonesManagerImpl::AutoJoint && m_integers[IFXBonesManagerImpl::SelectBone] >= 0)
        {
...
IFXRESULT IFXBonesManagerImpl::SetInteger(IntegerProperty prop, I32 set)
{
    ...
    else if (prop == IFXBonesManagerImpl::SelectBone && (set < -1 || set >= m_pCharacter->GetBoneTableSize()))
    {
        res = IFX_E_INVALID_RANGE;
    }
    else
    {
        ...
        if (prop == IFXBonesManagerImpl::NumBoneLinks && m_integers[IFXBonesManagerImpl::SelectBone] >= 0)
        {
            IFXBoneNode* pSelectBone = GetBoneByIndex(
                m_integers[IFXBonesManagerImpl::SelectBone]);
...
IFXRESULT IFXBonesManagerImpl::SetFloat(FloatProperty prop, F32 set)
{
    ...
    if (prop >= IFXBonesManagerImpl::LinkLength && m_integers[IFXBonesManagerImpl::SelectBone] >= 0)
    {
        IFXBoneNode* pSelectBone = GetBoneByIndex(
            m_integers[IFXBonesManagerImpl::SelectBone]);
...
*/
void IFXBonesManagerImpl::TimeProgress(IFXCharacter* pCharacter, U32 index, bool on)
{
    ...
    if (index == IFXBonesManagerImpl::TimeFrame && !on)
    {
        ...
        if (pManager->m_sumtimer[index] > 1000.0f)
        {
            ...
            {
                pManager->m_lastperiod[m] = pManager->m_sumtimer[m] / (F32)pManager->m_frames;
...
IFXRESULT IFXBonesManagerImpl::InitMesh(IFXMeshGroup* pMeshGrp, IFXVertexMapGroup* pVertexMapGroup)
{
#if IFXBONESMGR_CHECKNULL
...
IFXRESULT IFXBonesManagerImpl::UpdateMesh(IFXMeshGroup* pIn, IFXMeshGroup** ppOut)
{
    ...
    {
        F32 difftime = m_floats[IFXBonesManagerImpl::Time] - m_blendparam.GetStartTimeConst();
        F32 weight = 1.0f;
...

        {
            ...
            if (!m_bools[IFXBonesManagerImpl::AutoRotationX] && !m_bools[IFXBonesManagerImpl::AutoRotationY] && !m_bools[IFXBonesManagerImpl::AutoRotationZ])
            {
                m_last_rotate.MakeIdentity();
                m_auto_rotate.MakeIdentity();
            }
            else if (!m_bools[IFXBonesManagerImpl::AutoRotationX] || !m_bools[IFXBonesManagerImpl::AutoRotationY] || !m_bools[IFXBonesManagerImpl::AutoRotationZ])
            {
            ...
            while ((rootbone = rootlist.PostIncrement(context)) != NULL)
            {
                ...
                else
                {
                    ...
                    xroot.MakeTRS(rootbone->DisplacementConst(), rootbone->RotationConst(), rootbone->ScaleConst());
                    xblend.MakeTRS(rootbone->BlendDisplacement(), rootbone->BlendRotation(), rootbone->BlendScale());

                    xauto.Multiply(xroot);
                    xlast.Multiply(xblend);
                    xauto.CalcTRS(&rootbone->Displacement(), &rootbone->Rotation(), &rootbone->Scale());
                    xlast.CalcTRS(&rootbone->BlendDisplacement(), &rootbone->BlendRotation(), &rootbone->BlendScale());
                }
            }
        }
    ...
    TIME_PROGRESS(m_pCharacter, IFXBonesManagerImpl::TimeMotion, true);
    if (m_bools[IFXBonesManagerImpl::RootClearTranslate] || m_bools[IFXBonesManagerImpl::RootClearRotate])
    {
        IFXListContext context;
    ...
    IFXMotionMixer* pSelect)
{
    ...
    if (pSelect)
    {
        if (MotionMixer())
        {
            ...
            MotionMixer()->CalcBonePositionAtTime(0, m_floats[IFXBonesManagerImpl::Time], &location, &rotation);
        }
        else
        {
            m_auto_translate.Reset();
...
IFXRESULT IFXBonesManagerImpl::BlendToMotionMixer(IFXMotionMixer* pNext, const IFXBlendParam* pParam)
{
    ...
    if (MotionMixer() && LastMixer())
    {
        ...
        MotionMixer()->CalcBonePositionAtTime(0, pParam->GetStartTimeConst(), &location2, &rotation2);

        LastMixer()->CalcBonePositionAtTime(0, m_lasttime + pParam->GetRealignmentConst(), &location, &rotation);
    }

    return IFX_OK;
}

IFXRESULT IFXBonesManagerImpl::SetAutoTranslate(const IFXVector3& rVector, bool last)
{
    IFXTRACE_GENERIC(L"AutoT%d %ls ->%ls\n", last, (last ? m_last_translate : m_auto_translate).Out().Raw(), rVector.Out().Raw());
...
IFXRESULT IFXBonesManagerImpl::SetAutoRotate(const IFXQuaternion& rQuaternion, bool last)
{
    if (last)
    {
...
IFXRESULT IFXBonesManagerImpl::SetAutoScale(const IFXVector3& rVector, bool last)
{
    if (last)
    {
...
IFXRESULT IFXBonesManagerImpl::AddToAutoTranslate(const IFXVector3& rAddVector, bool last)
{
    IFXVector3 rotated;
...
IFXRESULT IFXBonesManagerImpl::AddToAutoScale(const IFXVector3& rAddVector, bool last)
{
    if (last)
    {
...
IFXRESULT IFXBonesManagerImpl::CreateIKChain(I32 baseBoneId, I32 endBoneId)
{
    ...
    if (endBoneId < 0 || endBoneId >= m_pCharacter->GetBoneTableSize() || baseBoneId >= m_pCharacter->GetBoneTableSize())
    {
        res = IFX_E_INVALID_HANDLE;
    }
    else
