...
IFXRESULT IFXMotionMixerImpl::GetMotionReader(I32 boneid, IFXMotionReader** reader)
{
    IFXASSERT(boneid >= 0);
...
IFXRESULT IFXMotionMixerImpl::UseCustomMotionArray(I32 length, IFXMotion* motion)
{
#if IFXMOTIONMXR_CHECKNULL
    if (!motion)
    {
...
IFXRESULT IFXMotionMixerImpl::MapFullMotionToCharacter(I32 motionid)
{
    ...
    for (m = 0; m < number; m++)
    {
        ...
        */
        ...
        MapMotionTrackToBone(motionid, keytrack.GetNameConst(), ((IFXBoneNode*)node)->GetBoneIndex());
    }
...
IFXRESULT IFXMotionMixerImpl::SubMapMotionToCharacter(I32 motionid, I32 fromboneid)
{
    IFXASSERT(m_character);
    ...
    m_character->ForEachNode2(IFXPARENTFIRST, *fromnode, &IFXMotionMixerImpl::MapBone, &mapstate);
...
BOOL IFXMotionMixerImpl::MapBone(IFXCoreNode& node, IFXVariant state)
{
    ...
    result = mapstate->mixer->MapMotionTrackToBone(mapstate->motionid, bonenode->Name(), bonenode->GetBoneIndex());
...
IFXRESULT IFXMotionMixerImpl::MapMotionTrackToBone(I32 motionid, const IFXString& rTrackName, I32 boneid)
{
    IFXASSERT(m_character);
...
IFXRESULT IFXMotionMixerImpl::SetPositionExplicitly(I32 boneid, const IFXVector3* location, const IFXQuaternion* rotation, const IFXVector3* scale)
{
    IFXASSERT(m_character);
...
//* location, rotation, and/or scale may be NULL
IFXRESULT IFXMotionMixerImpl::GetBonePosition(I32 boneid, IFXVector3* location, IFXQuaternion* rotation, IFXVector3* scale)
{
    IFXASSERT(m_character);
...
IFXRESULT IFXMotionMixerImpl::GetAbsoluteBoneMatrix(I32 boneid, IFXMatrix4x4* pMatrix)
{
    IFXASSERT(m_character);
    ...
    IFXVector3* scale)
{
    ...
    if ((U32)boneid >= m_readerarray.GetNumberElements() || !(reader = &m_readerarray[boneid])->IsValid())
    {
