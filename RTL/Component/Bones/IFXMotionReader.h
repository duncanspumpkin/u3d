...
public:
    ...
    {
        return (m_motion != NULL && m_track < m_motion->GetNumberTracks());
    }
    ...
    bool Read(F32 time, IFXInstant* instant)
    {
        ...
        IFXASSERT(m_motion);
        IFXASSERT(m_track < m_motion->GetTracks().GetNumberElements());
        m_motion->CalcInstantConst(WarpTime(time), m_track, instant, &m_context);
        return true;
    }
