...
void IFXScreenSpaceMetric::EvaluateTriangle(IFXTQTTriangle* pTriangle, Action* pAction)
{
    ...
    **/
    ...
    pTriangle->GetVertices(&ppVertex[IFXTQTAddress::Left], &ppVertex[IFXTQTAddress::Base], &ppVertex[IFXTQTAddress::Right]);
    ...
    for (j = 0; j < 4; j++)
    {
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            if (dotprod >= m_frustumdist[j])
            {
                ...
                if (0.95 * dotprod >= m_frustumdist[j])
                {
                    tight_visibility[i]++;
                }
            } /* if */
    ...
    if (visible)
    {
        *pAction = ScreenSpace(pTriangle, ppVertex);
        ...
        if ((*pAction == IFXAdaptiveMetric::Subdivide) && ((tight_visibility[0] < 4) && (tight_visibility[1] < 4) && (tight_visibility[2] < 4)))
        {
            *pAction = IFXAdaptiveMetric::Sustain;
        }
    }
...
bool IFXScreenSpaceMetric::GetFaceNormal(IFXTQTTriangle* pTriangle, IFXTQTVertex** ppVertex, IFXVector3& faceNormal)
{
    // Get two unit vecs, u and v, then compute face normal:
    IFXVector3 u, v;
    v.Subtract(ppVertex[IFXTQTAddress::Right]->m_position, ppVertex[IFXTQTAddress::Base]->m_position);
    u.Subtract(ppVertex[IFXTQTAddress::Left]->m_position, ppVertex[IFXTQTAddress::Base]->m_position);
...
{
    center.X() = (ppVertex[IFXTQTAddress::Left]->m_position.X() + ppVertex[IFXTQTAddress::Base]->m_position.X() + ppVertex[IFXTQTAddress::Right]->m_position.X()) * 0.333333333f;
    center.Y() = (ppVertex[IFXTQTAddress::Left]->m_position.Y() + ppVertex[IFXTQTAddress::Base]->m_position.Y() + ppVertex[IFXTQTAddress::Right]->m_position.Y()) * 0.333333333f;
    center.Z() = (ppVertex[IFXTQTAddress::Left]->m_position.Z() + ppVertex[IFXTQTAddress::Base]->m_position.Z() + ppVertex[IFXTQTAddress::Right]->m_position.Z()) * 0.333333333f;
}
...
void IFXScreenSpaceMetric::EvaluateGeometricError(IFXTQTTriangle* pTriangle, IFXTQTVertex** ppVertex, IFXVector3& faceNormal, F32 delsquared, F32* pLeftmetric, F32* pRightmetric)
{
    // Average vertices together to find a centroid:
    IFXVector3 center;
    ...
    for (I32 i = 0; i < 3; i++)
    {
        ...
        if ((IFXTQTAddress::UnDefined == meshBoundary) && (address.LocalNeighbor(direction, &nbraddress)))
        {
            pTriangle->GetBaseTriangle()->GetBaseNeighbor(direction, &pBaseNeighbor, &orientation);
            if (!pBaseNeighbor)
            {
                meshBoundary = direction;
            }
        }
    }
...
IFXAdaptiveMetric::Action IFXScreenSpaceMetric::ScreenSpace(IFXTQTTriangle* pTriangle, IFXTQTVertex** ppVertex)
{
    ...
    if (backface > SLIGHT_BACKFACE)
    {
        if (backface > VERY_BACKFACE)
        {
            ...
            if (pParent)
            {
                ...
                pParent->GetVertices(&ppVertex[IFXTQTAddress::Left], &ppVertex[IFXTQTAddress::Base], &ppVertex[IFXTQTAddress::Right]);
    ...
    { // the triangle is a consolidation candidate.
...
void IFXScreenSpaceMetric::UpdateViewpoint(IFXVector3& pCameraX, IFXVector3& pCameraY, IFXVector3& pCameraZ, IFXVector3& pCameraPos, F32 fAspect, F32 fFieldOfView)
{
...
void IFXScreenSpaceMetric::SetPixelTolerance(F32 factor)
{
    ...
    m_fConstant = 4.0f * (m_fPixelTolerance * m_fPixelTolerance) / (fDLambda * fDLambda);
}
