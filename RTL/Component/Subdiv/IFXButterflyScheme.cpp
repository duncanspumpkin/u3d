...
IFXButterflyScheme::IFXButterflyScheme()
{
    ...
    m_pMaskLayout[BFCorner] = new IFXBFCornerLayout;
    if (!m_pMaskLayout[BFCorner])
    {
        return;
    }
    m_pMaskLayout[BFBoundary] = new IFXBFBoundaryLayout;
    if (!m_pMaskLayout[BFBoundary])
    {
        return;
    }
    m_pMaskLayout[BFNearBoundary] = new IFXBFNearBoundaryLayout;
    if (!m_pMaskLayout[BFNearBoundary])
    {
        return;
    }
    m_pMaskLayout[BFNearBoundaryJustAverage] = new IFXBFNearBoundaryJustAvgLayout;
    if (!m_pMaskLayout[BFNearBoundaryJustAverage])
    {
        return;
    }
    m_pMaskLayout[BFContinous] = new IFXBFContinuousLayout;
    if (!m_pMaskLayout[BFContinous])
    {
        return;
    }

    m_pMask[BFMask2D] = new IFXButterflyMask2D;
    if (!m_pMask[BFMask2D])
    {
        return;
    }
    m_pMask[BFMask3D] = new IFXButterflyMask3D;
    if (!m_pMask[BFMask3D])
    {
        return;
    }
    m_pMask[BFMask3DNormal] = new IFXButterflyMask3DNormal;
    if (!m_pMask[BFMask3DNormal])
    {
        return;
    }
}
...
void IFXButterflyScheme::GatherEdgeNeighborhood(IFXTQTTriangle* pCntrTriangle, IFXTQTAddress::Direction usDirection, IFXAttributeNeighborhood* pNeighHood)
{
