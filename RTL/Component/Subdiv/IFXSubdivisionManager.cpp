...
IFXSubdivisionManager::~IFXSubdivisionManager()
{
    ...
    if (m_pBaseTriangle)
    {
        delete[] m_pBaseTriangle;
    }
    if (m_ppBaseVertex)
    {
        delete[] m_ppBaseVertex;
    }
    m_pBaseTriangle = NULL;
    m_ppBaseVertex = NULL;
...
IFXRESULT IFXSubdivisionManager::AllocateFreeLists()
{
    ...
    m_pTriangleAllocator = new IFXTQTTriangleAllocator(m_puInteger[InitialTriangleAllocation], m_puInteger[GrowTriangleAllocation]);

    m_pVertexAllocator = new IFXVertexAllocator(m_puInteger[InitialVertexAllocation], m_puInteger[GrowVertexAllocation]);
...
{
    if (m_pTriangleAllocator)
    {
        delete m_pTriangleAllocator;
    }
    m_pTriangleAllocator = NULL;
...
IFXRESULT IFXSubdivisionManager::ReallocateOutputMesh()
{
    ...
    for (uMeshNum = 0; uMeshNum < m_uNumOutMeshes; uMeshNum++)
    {
        ...
        if (IFXSUCCESS(rc) && ((pMeshOut->GetNumVertices() == pMeshOut->GetMaxNumVertices()) || (pMeshOut->GetNumFaces() == pMeshOut->GetMaxNumFaces())))

        {
            ...
            if ((uNumFacesIncreased < uNumAllocatedFaces) && (uNumVertsIncreased < uNumAllocatedVertices))
            {
...
IFXRESULT IFXSubdivisionManager::InitMesh(IFXMeshGroup* pMeshGrp, IFXNeighborMesh* pNeighborMesh)
{
    ...
    for (uMeshNum = 0; uMeshNum < uNumberOfMeshes; uMeshNum++)
    {
        ...
        for (f = 0; f < uNumMeshFaces; f++)
        {
            ...
            m_pBaseTriangle[uFaceOffset + f].Init(&m_pBaseTriangle[uFaceOffset + f], 0, emptyAddress, IFXTQTTriangle::TipUp, m_ppBaseVertex[uVertexOffset + a], m_ppBaseVertex[uVertexOffset + b], m_ppBaseVertex[uVertexOffset + c]);

            // Grab the 3 neighbor indices:
            pNeighborFace = &pNeighborFaceArray[f];

            pNeighborFace->GetNeighbor(0, &uMeshIndex[0], &puNeighborFaceIndex[0], &ppNeighborFlags[0]);
            pNeighborFace->GetNeighbor(1, &uMeshIndex[1], &puNeighborFaceIndex[1], &ppNeighborFlags[1]);
            pNeighborFace->GetNeighbor(2, &uMeshIndex[2], &puNeighborFaceIndex[2], &ppNeighborFlags[2]);
            ...
            m_pBaseTriangle[uFaceOffset + f].SetBaseNeighbors(pNeighborTriangles[0], pNeighborTriangles[1], pNeighborTriangles[2]);
...
IFXRESULT IFXSubdivisionManager::AddRenderCrackFillTriangle(IFXTQTTriangle* pTriangle, IFXTQTVertex* ppCorner[3], IFXTQTTriangle::SametOrientation bSametOrientation)
{
    IFXVertexIter vertexIter;
    U32 uMeshIndex;
    IFXTQTBaseTriangle* pBaseTriangle = pTriangle->GetBaseTriangle();
