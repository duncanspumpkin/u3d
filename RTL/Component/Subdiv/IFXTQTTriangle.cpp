...
IFXTQTAddress::Direction IFXTQTTriangle::m_usOpposite[3] = { IFXTQTAddress::Right, IFXTQTAddress::Base, IFXTQTAddress::Left };
...
IFXTQTLocality IFXTQTTriangle::LocateNeighborTriangle(IFXTQTAddress::Direction usDirection, IFXTQTAddress* pNeighAddr, IFXTQTTriangle** ppNeighbor, IFXTQTAddress::Direction* pOrientation)
{
...
//
IFXTQTLocality IFXTQTTriangle::GetSubdivNeighbor(IFXSubdivisionManager* pSubdivMgr, IFXTQTAddress::Direction usDirection, IFXAttributeNeighborhood* pNeighHood, U32 uIndex)
{
...
#define INFINITE_LOOP 100
    ...
    if (pNeighbor && (NeighAddr.Length() == m_address.Length()) && (uReOrientedDirection != IFXTQTAddress::UnDefined))
    {
...
void IFXTQTTriangle::FindCracks(IFXSubdivisionManager* pSubdivMgr)
{
    ...
    {
        IFXASSERT(m_pVertex[IFXTQTAddress::Left] && m_pVertex[IFXTQTAddress::Base] && m_pVertex[IFXTQTAddress::Right]);
...
**/
void IFXTQTTriangle::AddCrackFillData(IFXSubdivisionManager* pSubdivMgr, IFXTQTAddress::Direction crackedSide, IFXTQTVertex* pCrackVertex, BOOL bDiscontinuity)
{
...
void IFXTQTTriangle::DetectCrack(IFXSubdivisionManager* pSubdivMgr)
{
    ...
    LocateNeighborTriangle(IFXTQTAddress::Left, &pAddress[IFXTQTAddress::Left], &pTriangle[IFXTQTAddress::Left], &reOrientation[IFXTQTAddress::Left]);
    LocateNeighborTriangle(IFXTQTAddress::Base, &pAddress[IFXTQTAddress::Base], &pTriangle[IFXTQTAddress::Base], &reOrientation[IFXTQTAddress::Base]);
    LocateNeighborTriangle(IFXTQTAddress::Right, &pAddress[IFXTQTAddress::Right], &pTriangle[IFXTQTAddress::Right], &reOrientation[IFXTQTAddress::Right]);

    // Compute relative subdivision depths:
    int leftDiff = 0, baseDiff = 0, rightDiff = 0;

    if ((pTriangle[IFXTQTAddress::Left]) && (reOrientation[IFXTQTAddress::Left] != IFXTQTAddress::UnDefined))
    {
        leftDiff = m_address.Length() - pTriangle[IFXTQTAddress::Left]->m_address.Length();
    }

    if ((pTriangle[IFXTQTAddress::Base]) && (reOrientation[IFXTQTAddress::Base] != IFXTQTAddress::UnDefined))
    {
        baseDiff = m_address.Length() - pTriangle[IFXTQTAddress::Base]->m_address.Length();
    }

    if ((pTriangle[IFXTQTAddress::Right]) && (reOrientation[IFXTQTAddress::Right] != IFXTQTAddress::UnDefined))
    {
    ...
    if (leftDiff == 1)
    {
        ...
        pTriangle[IFXTQTAddress::Left]->AddCrackFillData(pSubdivMgr, reOrientation[IFXTQTAddress::Left], pCrackVertex, bDiscontinuous);
    }
    ...
    if (baseDiff == 1)
    {
        ...
        pTriangle[IFXTQTAddress::Base]->AddCrackFillData(pSubdivMgr, reOrientation[IFXTQTAddress::Base], pCrackVertex, bDiscontinuous);
    }
    ...
    if (rightDiff == 1)
    {
        ...
        pTriangle[IFXTQTAddress::Right]->AddCrackFillData(pSubdivMgr, reOrientation[IFXTQTAddress::Right], pCrackVertex, bDiscontinuous);
    }
    ...
    if (!m_pChild[TQT_00])
    {
        if (!bAdaptiveEnabled || (bAdaptiveEnabled && (m_action == IFXAdaptiveMetric::Subdivide)))
        {
            ...
            */
            ...
            m_pChild[TQT_00]->Init(m_pBaseTriangle, this, childAddress[TQT_00], m_bSametOrientation, pRightMidpoint, m_pVertex[IFXTQTAddress::Base], pLeftMidpoint);

            m_pChild[TQT_01]->Init(m_pBaseTriangle, this, childAddress[TQT_01], m_bSametOrientation, pBaseMidpoint, pLeftMidpoint, m_pVertex[IFXTQTAddress::Right]);

            m_pChild[TQT_10]->Init(m_pBaseTriangle, this, childAddress[TQT_10], bMiddleTriangle, pRightMidpoint, pBaseMidpoint, pLeftMidpoint);

            m_pChild[TQT_11]->Init(m_pBaseTriangle, this, childAddress[TQT_11], m_bSametOrientation, m_pVertex[IFXTQTAddress::Left], pRightMidpoint, pBaseMidpoint);
    ...
    */
    ...
    else
    {
        if (m_action == IFXAdaptiveMetric::Consolidate)
        {
            ...
            /* pLocality[IFXTQTAddress::Left] = */ LocateNeighborTriangle(IFXTQTAddress::Left, &pAddress[IFXTQTAddress::Left], &pTriangle[IFXTQTAddress::Left], &reOrientation[IFXTQTAddress::Left]);
            /* pLocality[IFXTQTAddress::Base] = */ LocateNeighborTriangle(IFXTQTAddress::Base, &pAddress[IFXTQTAddress::Base], &pTriangle[IFXTQTAddress::Base], &reOrientation[IFXTQTAddress::Base]);
            /* pLocality[IFXTQTAddress::Right] = */ LocateNeighborTriangle(IFXTQTAddress::Right, &pAddress[IFXTQTAddress::Right], &pTriangle[IFXTQTAddress::Right], &reOrientation[IFXTQTAddress::Right]);
...
void IFXTQTTriangle::ResetMarkers(U32 depth)
{
    ...
    IFXASSERT(m_pVertex[IFXTQTAddress::Left] && m_pVertex[IFXTQTAddress::Right] && m_pVertex[IFXTQTAddress::Base]);
