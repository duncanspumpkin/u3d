...
IFXRESULT CIFXSubdivModifier::GetOutputs(IFXGUID**& rpOutOutputs, U32& rOutNumberOfOutputs, U32*& rpOutOutputDepAttrs)
{
...
IFXRESULT CIFXSubdivModifier::GenerateOutput(U32 inOutputDataElementIndex, void*& rpOutData, BOOL& rNeedRelease)
{
    IFXRESULT result = IFX_OK;
    ...
    if (inOutputDataElementIndex == m_uMeshGroupDataElementIndex)
    {
        ...
        result = m_pInputDataPacket->GetDataElement(m_uMeshGroupDataElementIndex, IID_IFXMeshGroup, (void**)&pInputMeshGroup);

        if (IFXSUCCESS(result))
        {
            result = m_pInputDataPacket->GetDataElement(m_uFrustumDataElementIndex, (void**)&pFrustum);
        }
        ...
        if (IFXSUCCESS(result))
        {
            if (!m_bEnabled)
            {
                result = m_pInputDataPacket->GetDataElement(m_uMeshGroupDataElementIndex, &rpOutData);
                rNeedRelease = TRUE;
            }
            else
            {
                ...
                if (m_pSubdivMgr == NULL)
                {
                    ...
                    result = m_pInputDataPacket->GetDataElement(m_uNeighborMeshDataElementIndex, IID_IFXNeighborMesh, (void**)&pInputNeighborMesh);
                    if (IFXSUCCESS(result))
                    {
                ...
                if (IFXSUCCESS(result) && m_pSubdivMgr)
                {
                    ...
                    if (IFXSUCCESS(result) && bAdaptive)
                    {
                        ...
                        m_pScreenSpaceMetric->UpdateViewpoint(cameraX, cameraY, cameraZ, cameraPosition, fAspect, fFOV);
                    }
                }
                    ...
                    if (bUpdated && IFXSUCCESS(result))
                    {
                        ...
                        for (i = 0; i < pOutputMeshGroup->GetNumMeshes(); i++)
                        {
                            ...
                            if (pMeshNew && pMeshOld)
                            {
                                ...
                                result = pInputMeshGroup->QueryInterface(IID_IFXRenderable, (void**)&pRenderableOld);
                                ...
                                if (IFXSUCCESS(result))
                                {
                                    result = pOutputMeshGroup->QueryInterface(IID_IFXRenderable, (void**)&pRenderableNew);
                                }
                                // Set the shaderID
                                if (IFXSUCCESS(result))
                                {
    ...
    IFXModifierDataPacket* pInDataPacket)
{
    ...
    if (IFXSUCCESS(result))
    {
        result = pInInputDataPacket->GetDataElementIndex(DID_IFXRenderableGroup, m_uMeshGroupDataElementIndex);
    }

    if (IFXSUCCESS(result))
    {
        result = pInInputDataPacket->GetDataElementIndex(DID_IFXNeighborMesh, m_uNeighborMeshDataElementIndex);
    }

    if (IFXSUCCESS(result))
    {
        result = pInInputDataPacket->GetDataElementIndex(DID_IFXTransform, m_uTransformDataElementIndex);
    }

    if (IFXSUCCESS(result))
    {
        result = pInInputDataPacket->GetDataElementIndex(DID_IFXFrustum, m_uFrustumDataElementIndex);
    }

    return result;
}

IFXRESULT CIFXSubdivModifier::Notify(IFXModifierMessage eInMessage, void* pMessageContext)
...
IFXRESULT CIFXSubdivModifier::GetTension(F32* pTension)
{
    ...
    {
        rc = m_pSubdivMgr->GetFloat(IFXSubdivisionManagerInterface::SurfaceTension, pTension);
    }
...
IFXRESULT CIFXSubdivModifier::GetDepth(U32* pDepth)
{
    ...
    {
        rc = m_pSubdivMgr->GetInteger(IFXSubdivisionManagerInterface::MaxComputeDepth, pDepth);
    }
...
U32 CIFXSubdivModifier::CalculateSafeDepth()
{
    ...
    if (m_pInputDataPacket)
    {
        ...
        if (IFXSUCCESS(result))
        {
            ...
            while ((safeDepth < IFX_SDS_MAX_ALLOWED_SUBDIVISION_DEPTH) && (numVertices < uMaxAllowedVertices) && ((numTriangles *= 4) < uMaxAllowedTriangles))
            {
                safeDepth++;
...
IFXRESULT CIFXSubdivModifier::SetDepth(U32 depth)
{
    ...
    else
    {
        ...
        if (IFXSUCCESS(rc))
        {
            rc = m_pSubdivMgr->SetInteger(IFXSubdivisionManagerInterface::MaxComputeDepth, depth);
        }

        if (IFXSUCCESS(rc))
        {
            rc = m_pSubdivMgr->SetInteger(IFXSubdivisionManagerInterface::MaxRenderDepth, depth);
        }
    }
...
IFXRESULT CIFXSubdivModifier::GetAdaptive(BOOL* pbAdaptive)
{
    ...
    {
        rc = m_pSubdivMgr->GetBool(IFXSubdivisionManagerInterface::Adaptive, pbAdaptive);
    }
...
IFXRESULT CIFXSubdivModifier::SetAdaptive(BOOL bAdaptive)
{
    ...
    {
        rc = m_pSubdivMgr->SetBool(IFXSubdivisionManagerInterface::Adaptive, bAdaptive);
    }

    return rc;
}

IFXRESULT CIFXSubdivModifier::InitializeSubdiv(IFXMeshGroup* pMeshGroup, IFXNeighborMesh* pNeighborMesh)
{
    IFXRESULT result = IFX_OK;
