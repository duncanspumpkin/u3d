...
void CIFXPointSetEncoder::CalculatePredictedNormalAtSplitPosX(U32 currPosInd, U32 splitPosInd, IFXVector3& vPredictedNormal)
{
    vPredictedNormal = IFXVector3(0.f, 0.f, 0.f);
    IFXASSERT(m_pPointSetDescription);

    if (m_pPointSetDescription->m_numPoints == 0 || m_pPointSetDescription->m_numPositions == 0)
    {
        return;
    }
    ...
    if (currPosInd == 0)
    {
        return; // no Points deifed yet - so predicted is just zero normal
    }
...
void CIFXPointSetEncoder::CalculatePredictedColorAtSplitPosX(BOOL bDiffuseColor, U32 currPosInd, U32 splitPosInd, IFXVector4& v4PredictedVertColor)
{
    ...
    if (currPosInd == 0)
    {
        return; // no Points defined yet - so predicted is just zero color
    }
...
void CIFXPointSetEncoder::CalculatePredictedTexCoordAtSplitPosX(U32 texInd, U32 currPosInd, U32 splitPosInd, IFXVector4& v4PredictedTexCoord)
{
    IFXASSERT(m_pPointSetDescription);
    ...
    if (texInd > m_pPointSetDescription->m_numTexCoords)
    {
        return;
    }
...
void CIFXPointSetEncoder::QuantizeTexCoordForWrite(IFXVector4& texCoord, U8& u8Signs, U32& udR, U32& udG, U32& udB, U32& udA)
{
...
void CIFXPointSetEncoder::GetPointAttribIndexes(U32 PointInd, CIFXPointSetEncoder::AttribType attrib, U32& attrInd)
{
...
void CIFXPointSetEncoder::QuantizeColorForWrite(BOOL bDiffuseColor, IFXVector4& color, U8& u8Signs, U32& udR, U32& udG, U32& udB, U32& udA)
{
    F32 fQuantVertColor;
    if (bDiffuseColor)
    {
