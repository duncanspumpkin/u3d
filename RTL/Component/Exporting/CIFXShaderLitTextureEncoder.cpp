...
IFXRESULT CIFXShaderLitTextureEncoder::QueryInterface(IFXREFIID interfaceId, void** ppInterface)
{
    IFXRESULT rc = IFX_OK;
...
void CIFXShaderLitTextureEncoder::EncodeX(IFXString& rName, IFXDataBlockQueueX& rDataBlockQueue, F64 units)
{
    ...
    try
    {
        ...
        U32 uFlags = 0;
        if (pShaderLitTexture->GetLightingEnabled())
        {
            uFlags |= 0x00000001;
        }
        if (pShaderLitTexture->GetAlphaTestEnabled())
        {
            uFlags |= 0x00000002;
        }
        if (pShaderLitTexture->GetRenderMaterial().GetUseVertexColors())
        {
            uFlags |= 0x00000004;
        }
