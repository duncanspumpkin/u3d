...
void CIFXLineSetEncoder::CalculatePredictedNormalAtSplitPosX(U32 currPosInd, U32 splitPosInd, IFXVector3& vPredictedNormal)
{
    IFXASSERT(m_pLineSetDescription);
    IFXASSERT(m_pLineSetAnalyzer);
    IFXASSERT(m_pLineSetAnalyzer->IsInitialized());

    if (m_pLineSetDescription->m_numLines == 0 || m_pLineSetDescription->m_numPositions == 0)
    {
        return;
    }
    ...
    if (currPosInd == 0)
    {
        return; // no lines deifed yet - so predicted is juat zero normal
    }
    ...
    if (numLines == 0)
    {
        return; // just stand alone position = no lines connected w/ it
    }
    ...
    if (linesForPredict == 0)
    {
        return; // no lines connected to split wrtitten so far -> use zero normal as prdiction
    }
...
void CIFXLineSetEncoder::CalculatePredictedColorAtSplitPosX(BOOL bDiffuseColor, U32 currPosInd, U32 splitPosInd, IFXVector4& v4PredictedVertColor)
{
    IFXASSERT(m_pLineSetDescription);
    IFXASSERT(m_pLineSetAnalyzer);
    IFXASSERT(m_pLineSetAnalyzer->IsInitialized());
    ...
    if (currPosInd == 0)
    {
        return; // no lines defined yet - so predicted is just zero normal
    }

    if ((m_uCurrentNumDiffuseColors == 0 && bDiffuseColor) || (m_uCurrentNumSpecularColors == 0 && (!bDiffuseColor)))
    {
        return;
    }
    ...
    if (linesForPredict == 0)
    {
        return; // no lines connected to split written so far -> use zero normal as prediction
    }
...
void CIFXLineSetEncoder::CalculatePredictedTexCoordAtSplitPosX(U32 texInd, U32 currPosInd, U32 splitPosInd, IFXVector4& v4PredictedTexCoord)
{
    IFXASSERT(m_pLineSetDescription);
    IFXASSERT(m_pLineSetAnalyzer);
    IFXASSERT(m_pLineSetAnalyzer->IsInitialized());
    ...
    if (texInd >= m_pLineSetDescription->m_numTexCoords)
    {
        return;
    }
...
void CIFXLineSetEncoder::GetLineAttribIndexes(U32 lineInd, U32 splitPos, CIFXLineSetEncoder::AttribType attrib, U32& splitInd, U32& endInd)
{
...
void CIFXLineSetEncoder::GetLineTexCoord(U32 texLayer, U32 lineInd, U32 currPos, U32& currInd, U32& endInd)
{
...
void CIFXLineSetEncoder::QuantizeColorForWrite(BOOL bDiffuseColor, IFXVector4& color, U8& u8Signs, U32& udR, U32& udG, U32& udB, U32& udA)
{
    F32 fQuantVertColor;
    if (bDiffuseColor)
    {
...
void CIFXLineSetEncoder::QuantizeTexCoordForWrite(IFXVector4& texCoord, U8& u8Signs, U32& udR, U32& udG, U32& udB, U32& udA)
{
    u8Signs = (texCoord.R() < 0.0) | ((texCoord.G() < 0.0) << 1) | ((texCoord.B() < 0.0) << 2) | ((texCoord.A() < 0.0) << 3);
...
#if _DEBUG
void CIFXLineSetEncoder::ReconstructPosition(U8 u8Signs, U32 udX, U32 udY, U32 udZ, IFXVector3& vReconstructedPosition)
{
    if (u8Signs & 1)
    {
        ...
        for (iPos = 0; iPos < 2; iPos++)
        {
            ...
            if (currMaterial.m_uDiffuseColors)
            {
                ...
                if (u8DupAttribLineIndexFlag == 0)
                {
                    ...
                    QuantizeColorForWrite(TRUE, v4VertColorDifference, u8Signs, udR, udG, udB, udA);
                    pBitStreamX->WriteCompressedU8X(uACContextNormalDiffSigns, u8Signs);
            ...
            if (currMaterial.m_uSpecularColors)
            {
                ...
                {
                    v4VertColorDifference.Subtract(v4VertColor, v4PredictedVertColor);
                    QuantizeColorForWrite(FALSE, v4VertColorDifference, u8Signs, udR, udG, udB, udA);
                    pBitStreamX->WriteCompressedU8X(uACContextNormalDiffSigns, u8Signs);
                    pBitStreamX->WriteCompressedU32X(uACContextVertexColorDiffMagR, udR);
                    pBitStreamX->WriteCompressedU32X(uACContextVertexColorDiffMagG, udG);
                    pBitStreamX->WriteCompressedU32X(uACContextVertexColorDiffMagB, udB);
