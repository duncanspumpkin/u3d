...
//---------------------------------------------------------------------------
IFXRESULT IFXAPI_CALLTYPE CIFXWriteManager_Factory(IFXREFIID interfaceId, void** ppInterface)
{
    IFXRESULT rc = IFX_OK;
...
//---------------------------------------------------------------------------
IFXRESULT CIFXWriteManager::QueryInterface(IFXREFIID interfaceId, void** ppInterface)
{
    IFXRESULT rc = IFX_OK;
...
IFXRESULT CIFXWriteManager::SetQualityFactor(U32 uQualityFactor, IFXExportOptions ePalettes)
{
    IFXRESULT rc = IFX_OK;
...
void CIFXWriteManager::SetQualityOnPalette(U32 uQualityFactor, IFXSceneGraph::EIFXPalette ePalette)
{
    IFXRESULT rc = IFX_OK;
...
*/
void CIFXWriteManager::ComponentIsMarkedForWrite(IFXUnknown* pInUnknown, BOOL* pbOutMarked)
{
...
void CIFXWriteManager::CreateNewObjectDeclarationBlockX(U32* blockTypes, IFXExtensionData* pExtData, IFXDataBlockQueueX& rDataBlockQueue)
{
            ...
            if ((i != 0) && bMarked) // Don't write the default object (index 0)
            {
                ...
                if (pEncoder)
                {
                    ...
                    if (IFXSUCCESS(rc))
                    {
                        ...
                        if (ePalette == IFXSceneGraph::NODE)
                        {
                            uChainType = IFXModifierChain::NODE;
                        }
                        if (ePalette == IFXSceneGraph::GENERATOR)
                        {
                            uChainType = IFXModifierChain::RESOURCE;
                        }
                        if (ePalette == IFXSceneGraph::TEXTURE)
                        {
                            uChainType = IFXModifierChain::TEXTURE;
                        }
