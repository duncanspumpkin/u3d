...
IFXRESULT CIFXMeshGroup::QueryInterface(IFXREFIID interfaceId, void** ppInterface)
{
...
IFXRESULT IFXAPI CIFXMeshGroup::TransferData(IFXMeshGroup& in_rMeshGroup, IFXMeshAttributes in_uTransferAttr, IFXMeshAttributes in_uCopyAttr, BOOL bCopyShaders)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT CIFXMeshGroup::TransferData(IFXRenderable& in_rSrc, BOOL bCopyElements, BOOL bCopyShaders)
{
    IFXRESULT result = IFX_OK;
        ...
        if (m_pBoundHierarchy)
        {
            ...
            if (puPositionCounts && puFaceCounts)
            {
                ...
                for (i = 0; i < m_uNumElements && IFXSUCCESS(result); i++)
                {
                    ...
                    if (IFXSUCCESS(result))
                    {
                        if ((uPositionCount != puPositionCounts[i]) || (uFaceCount != puFaceCounts[i]))
                        {
                            bRebuild = TRUE;
                            break;
                        }
