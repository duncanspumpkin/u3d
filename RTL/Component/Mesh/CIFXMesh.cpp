...
IFXRESULT CIFXMesh::QueryInterface(IFXREFIID interfaceId, void** ppInterface)
{
    ...
    if (ppInterface)
    {
        ...
        else if (interfaceId == CID_IFXMesh)
        {
...
IFXRESULT CIFXMesh::Allocate(IFXVertexAttributes vaAttribs, U32 uNumVertices, U32 uNumFaces)
{
    IFXRESULT rc = IFX_OK;
...
IFXRESULT CIFXMesh::Copy(IFXMesh& rSrcMesh)
{
    ...
    // do a fast copy.
    if (m_vaAttribs.m_uAllData == rSrcMesh.GetAttributes().m_uAllData || m_uNumAllocatedVertices == 0 || m_uNumAllocatedFaces == 0)
    {
        // If mesh is too small, re-allocate
        if (m_uNumAllocatedVertices < rSrcMesh.GetMaxNumVertices() || m_uNumAllocatedFaces < rSrcMesh.GetMaxNumFaces())
        {
            rc = Allocate(rSrcMesh.GetAttributes(), rSrcMesh.GetMaxNumVertices(), rSrcMesh.GetMaxNumFaces());
        }

        if (IFXSUCCESS(rc))
        {
            rc = FastCopy(rSrcMesh, 0, rSrcMesh.GetMaxNumVertices(), 0, rSrcMesh.GetMaxNumFaces());
        }
...
// this mesh.
IFXRESULT IFXAPI CIFXMesh::FastCopy(IFXMesh& rSrcMesh, U32 uStartVertex, U32 uNumVertices, U32 uStartFace, U32 uNumFaces)
{
    IFXRESULT rc = IFX_OK;
    ...
    if (IFXSUCCESS(rc))
    {
        ...
        for (i = 0; i < IFX_MESH_FACE && IFXSUCCESS(rc); i++)
        {
            ...
            {
                rc = m_pspMeshData[i]->CopyData(*(pMesh->m_pspMeshData[i].GetPointerNR()), uStartVertex, uNumVertices);
            }
        }
    }
...
IFXRESULT CIFXMesh::TransferData(IFXMesh& rSrcMesh, IFXMeshAttributes uTransferAttr, IFXMeshAttributes uCopyAttr)
{
    IFXRESULT rc = IFX_OK;
...
IFXRESULT CIFXMesh::IncreaseSize(U32 uNumNewFaces, U32 uNumNewVerts)
{
    ...
    else
    {
        ...
        rc = IFXCreateComponent(CID_IFXInterleavedData, IID_IFXInterleavedData, (void**)&pNewFaces);
        ...
        if (IFXSUCCESS(rc))
        {
            ...
            if (IFXSUCCESS(rc))
            {
                if ((m_vaAttribs.m_uData.m_b32BitIndices && (uIndexSize == 32)) || ((!m_vaAttribs.m_uData.m_b32BitIndices) && (uIndexSize == 16)))
                {
                    // We can do a memcpy
                    rc = pNewFaces->CopyData(*(m_pspMeshData[IFX_MESH_FACE].GetPointerNR()), 0, m_uMaxNumFaces);
                }
                else
                {
        ...
        for (i = 0; (i < IFX_MESH_FACE) && IFXSUCCESS(rc); i++)
        {
            if (m_pspMeshData[i].IsValid() && m_pspMeshData[i]->GetStride())
            {
                ...
                if (uNumVertices < uTotalVerts)
                {
                    ...
                    rc = IFXCreateComponent(CID_IFXInterleavedData, IID_IFXInterleavedData, (void**)&pData);
...
IFXRESULT CIFXMesh::InterleaveData(IFXMeshAttributes eDirtyAttribs, IFXMeshAttributes eAllAttribs)
{
    ...
    if ((eDirtyAttribs == eAllAttribs) && (eAllAttribs == IFXMeshAttributes(IFX_MESH_POSITION)))
    {
        ...
        m_spInterleavedData->CopyData(*m_pspMeshData[IFX_MESH_POSITION].GetPointerNR(), 0, m_uNumVertices);
...
{
    IFXRESULT rc = IFX_OK;
    IFXASSERTBOX(eMeshAttribute < IFX_MESH_NUM_ATTRIBUTES, "Invalid Mesh Attribute access.");
...
IFXRESULT CIFXMesh::SetMaxNumVertices(U32 uNumVertices)
{
    ...
    IFXASSERTBOX(uNumVertices <= m_uNumAllocatedVertices, "Can't set number of vertices higher than amount allocated!");
...
IFXRESULT CIFXMesh::SetMaxNumFaces(U32 uNumFaces)
{
    ...
    IFXASSERTBOX(uNumFaces <= GetNumAllocatedFaces(), "Can't set number of faces higher than amount allocated!");
...
IFXRESULT CIFXMesh::SetMaxNumLines(U32 uNumLines)
{
    ...
    IFXASSERTBOX(uNumLines <= GetNumAllocatedLines(), "Can't set number of lines higher than amount allocated!");
...
IFXRESULT CIFXMesh::SetNumVertices(U32 uNumVertices)
{
    ...
    IFXASSERTBOX(uNumVertices <= m_pspMeshData[IFX_MESH_POSITION]->GetNumVertices(), "Can't set number of vertices higher than maximum!");
...
IFXRESULT CIFXMesh::SetNumFaces(U32 uNumFaces)
{
    ...
    IFXASSERTBOX(uNumFaces <= m_pspMeshData[IFX_MESH_FACE]->GetNumVertices(), "Can't set number of faces higher than maximum!");
...
IFXRESULT CIFXMesh::SetNumLines(U32 uNumLines)
{
    ...
    IFXASSERTBOX(uNumLines <= m_pspMeshData[IFX_MESH_LINE]->GetNumVertices(), "Can't set number of faces higher than maximum!");
    ...
    for (i = 0; i < IFX_MAX_TEXUNITS; i++)
    {
        if (m_eRenderTCsUsed[IFX_MESH_RENDER_TC0 + i] && (i < m_uNumTCsUsed))
        {
            if (m_pspMeshData[IFX_MESH_RENDER_TC0 + i].IsValid())
            {
                ...
                {
                    m_pVersionsInterleaved[IFX_MESH_TC0 + i] = m_pspMeshData[IFX_MESH_RENDER_TC0 + i]->GetVersionWord(0);
        ...
        else
        {
            if (i < m_uNumTCsUsed && m_pspMeshData[IFX_MESH_TC0 + i].IsValid())
            {
                ...
                {
                    m_pVersionsInterleaved[IFX_MESH_TC0 + i] = m_pspMeshData[IFX_MESH_TC0 + i]->GetVersionWord(0);
                    eVal |= IFX_MESH_TC0 + i;
                }
