...
IFXRESULT CIFXShaderLitTexture::Construct()
{
    ...
    for (i = 0; i < IFX_MAX_TEXUNITS && IFXSUCCESS(rc); i++)
    {
        ...
        rc = IFXCreateComponent(CID_IFXUVGenerator, IID_IFXUVGenerator, (void**)(&m_pUVMapper[i]));
...
IFXRESULT CIFXShaderLitTexture::Mark(void)
{
    ...
    if ((IFXSUCCESS(rc)) && (m_pSceneGraph != NULL))
    {
        ...
        if (m_uMaterialID)
        {
            ...
            rc = m_pSceneGraph->GetPalette(IFXSceneGraph::MATERIAL, &pPalette);
            if (IFXSUCCESS(rc))
            {
                IFXMarker* pMarker = NULL;
                rc = pPalette->GetResourcePtr(m_uMaterialID, IID_IFXMarker, (void**)&pMarker);
        ...
        for (i = 0; i < IFX_MAX_TEXUNITS; i++)
        {
            ...
            if (m_uTextureID[i])
            {
                ...
                {
                    IFXPalette* pPalette = NULL;
                    rc = m_pSceneGraph->GetPalette(IFXSceneGraph::TEXTURE, &pPalette);
                    if (IFXSUCCESS(rc))
                    {
                        IFXMarker* pMarker = NULL;
                        rc = pPalette->GetResourcePtr(m_uTextureID[i], IID_IFXMarker, (void**)&pMarker);
...
IFXRESULT CIFXShaderLitTexture::RenderShaderLitTexture(IFXRenderContext& rInRenderer)
{
    ...
    if (m_uFlags & MATERIAL)
    {
        ...
        if (IFXSUCCESS(result))
        {
            result = pPalette->GetResourcePtr(m_uMaterialID, IID_IFXMaterialResource, (void**)&pMaterialResource);
            if (IFXFAILURE(result))
            {
                result = pPalette->GetResourcePtr(0, IID_IFXMaterialResource, (void**)&pMaterialResource);
            }
        }
    ...
    for (i = 0; i < IFX_MAX_TEXUNITS; i++)
    {
        if (m_uChannels & (1 << i))
        {
            ...
            if (IFXSUCCESS(result))
            {
                result = pPalette->GetResourcePtr(m_uTextureID[i], IID_IFXTextureObject, (void**)&pTextureObject);
            }
            ...
            if (IFXSUCCESS(result))
            {
                ...
                if (result == IFX_W_READING_NOT_COMPLETE)
                {
                ...
                if (IFXFAILURE(result))
                {
                    IFXTRACE_CUSTOM(IFXRESULT_COMPONENT_SCENEGRAPH, IFXDEBUG_MESSAGE, L"UpdateAndMakeCurrent failed in IFXShaderLitTexture:(%p)\n", this);
                    result = IFX_OK;
    ...
    for (passNum = 0; passNum < uNumPasses && IFXSUCCESS(result); passNum++)
    {
        ...
        {
            if (IFX_OK == pElement->pDataPacket->GetDataElement(pElement->u16DataElementIndex, IID_IFXMeshGroup, (void**)&pMeshGroup))
            {
                if (IFX_OK == pMeshGroup->GetMesh(pElement->uRenderableElementIndex, pMesh))
                {
                        ...
                        || (pMesh->GetNumLines() && m_bDrawLines) || (pMesh->GetNumVertices() && m_bDrawPoints))
                    {
                        ...
                        {
                            result = m_pUVMapper[i]->Generate(*pMesh, &m_sUVMapperParams[i], &renderTransform, &mFrustumInverse, pElement->pLightSet);
                        }
                    ...
                    if (pMesh->GetNumVertices() && m_bDrawPoints && (pMesh->GetNumFaces() == 0) && (pMesh->GetNumLines() == 0)
                        && IFXSUCCESS(result))
                    {
...
IFXRESULT CIFXShaderLitTexture::SetTextureMode(U32 uInLayer, TextureMode eInMode)
{
    IFXRESULT result = IFX_OK;
    ...
    {
        result = IFXCreateComponent(CID_IFXUVGenerator, IID_IFXUVGenerator, (void**)(&m_pUVMapper[uInLayer]));
    }
...
IFXRESULT CIFXShaderLitTexture::GetBlendFunction(U32 uInLayer, BlendFunction* peOut)
{
    if (uInLayer > (IFX_MAX_TEXUNITS - 1))
    {
        return IFX_E_INVALID_RANGE;
...
IFXRESULT CIFXShaderLitTexture::SetBlendFunction(U32 uInLayer, BlendFunction eInFn)
{
    if (uInLayer > (IFX_MAX_TEXUNITS - 1))
    {
        return IFX_E_INVALID_RANGE;
...
IFXRESULT CIFXShaderLitTexture::GetBlendSource(U32 uInLayer, BlendSource* peOut)
{
    if (uInLayer > (IFX_MAX_TEXUNITS - 1))
    {
        return IFX_E_INVALID_RANGE;
...
IFXRESULT CIFXShaderLitTexture::SetBlendSource(U32 uInLayer, BlendSource eIn)
{
    if (uInLayer > (IFX_MAX_TEXUNITS - 1))
    {
        return IFX_E_INVALID_RANGE;
...
IFXRESULT CIFXShaderLitTexture::GetBlendConstant(U32 uInLayer, F32* pfOutConstant)
{
    if (uInLayer > (IFX_MAX_TEXUNITS - 1))
    {
        return IFX_E_INVALID_RANGE;
...
IFXRESULT CIFXShaderLitTexture::SetBlendConstant(U32 uInLayer, F32 fInConstant)
{
    if (uInLayer > (IFX_MAX_TEXUNITS - 1))
    {
        return IFX_E_INVALID_RANGE;
...
IFXRESULT CIFXShaderLitTexture::GetTextureMode(U32 uInLayer, TextureMode* peOut)
{
    if (uInLayer > (IFX_MAX_TEXUNITS - 1))
    {
        return IFX_E_INVALID_RANGE;
...
IFXRESULT CIFXShaderLitTexture::GetTextureTransform(U32 uInLayer, IFXMatrix4x4** ppmOutTextureTransform)
{
...
IFXRESULT CIFXShaderLitTexture::SetTextureTransform(U32 uInLayer, IFXMatrix4x4* pmInTextureTransform)
{
    m_pTexTransforms[uInLayer] = *pmInTextureTransform;
    return IFX_OK;
}

IFXRESULT CIFXShaderLitTexture::GetWrapTransform(U32 uInLayer, IFXMatrix4x4** ppmOutWrapTransform)
{
...
IFXRESULT CIFXShaderLitTexture::SetWrapTransform(U32 uInLayer, IFXMatrix4x4* pmInWrapTransform)
{
    m_sUVMapperParams[uInLayer].mWrapTransformMatrix = *pmInWrapTransform;
    return IFX_OK;
}

IFXRESULT CIFXShaderLitTexture::GetTextureRepeat(U32 uInLayer, U8* puOutTextureRepeat)
{
    if (uInLayer > (IFX_MAX_TEXUNITS - 1))
    {
        return IFX_E_INVALID_RANGE;
...
IFXRESULT CIFXShaderLitTexture::SetTextureRepeat(U32 uInLayer, U8 uInTextureRepeat)
{
    if (uInLayer > (IFX_MAX_TEXUNITS - 1))
    {
        return IFX_E_INVALID_RANGE;
