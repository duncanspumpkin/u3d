...
IFXRESULT CIFXShadingModifier::GetOutputs(IFXGUID**& rpOutOutputs, U32& rOutNumberOfOutputs, U32*& rpOutOutputDepAttrs)
{
...
IFXRESULT CIFXShadingModifier::GetDependencies(IFXGUID* pInOutputDID, IFXGUID**& rppOutInputDependencies, U32& rOutNumberInputDependencies, IFXGUID**& rppOutOutputDependencies, U32& rOutNumberOfOutputDependencies, U32*& rpOutOutputDepAttrs)
{
    IFXRESULT result = IFX_OK;
            ...
            if (IFXSUCCESS(result))
            {
                ...
                if (IFXSUCCESS(result))
                {
                    for (i = 0; i < Size; i++)
                    {
                        ...
                        if ((type == IFXRENDERABLE_ELEMENT_TYPE_MESH && m_attributes & MESH) || (type == IFXRENDERABLE_ELEMENT_TYPE_LINESET && m_attributes & LINE) || (type == IFXRENDERABLE_ELEMENT_TYPE_POINTSET && m_attributes & POINT) || (type == IFXRENDERABLE_ELEMENT_TYPE_GLYPH && m_attributes & GLYPH))
                        {
...
IFXRESULT CIFXShadingModifier::SetDataPacket(IFXModifierDataPacket* pInInputDataPacket, IFXModifierDataPacket* pInDataPacket)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT CIFXShadingModifier::Notify(IFXModifierMessage eInMessage, void* pMessageContext)
{
    return IFX_OK;
}
