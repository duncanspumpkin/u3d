...
IFXRESULT CIFXMetaData::QueryInterface(IFXREFIID interfaceId, void** ppInterface)
{
    ...
    if (ppInterface)
    {
        if ((interfaceId == IID_IFXMetaDataX) || (interfaceId == IID_IFXUnknown))
        {
    ...
    for (i = 0; i < l; i++)
    {
        ...
        if (!tmpMD->Subattributes.GetElement(i).NoValue)
        {
            ...
            for (j = 0; j < len; j++)
            {
                IFXCHAR ch;
                value0.GetAt(j, &ch);
                if (ch == L' ' || (ch >= 0x09 && ch <= 0x0D) || ch == L'#' || ch == L'"')
                {
                    quoted = true;
                }
                if (ch == L'"')
...
IFXRESULT CIFXMetaData::GetSubattributeIndex(U32 uIndex, const IFXString& rSubattributeName, U32& rSubattributeIndex)
{
    ...
    if (uIndex >= m_uMDCount)
    {
        return IFX_E_OUT_OF_METADATA;
    }
...
void CIFXMetaData::UnpackKey(IFXString& rKey, IFXSubattributes& rSubattributes)
{
    enum EParsingState
    {
        PARSING_STATE_SEARCHING = 0,
        PARSING_STATE_NAME = 1,
        PARSING_STATE_VALUE = 2
    };

    U32 uFirstAttribute = 0;
    rSubattributes.Clear();
    IFXString key = rKey;
    if (IFXFAILURE(key.FindSubstring(L"#", &uFirstAttribute)))
    {
        return;
    }
