...
CIFXCoreServices::Initialize(U32 uProfile, F64 units)
{
    ...
    if (!m_bInitialized)
    {
        ...
        if (IFXSUCCESS(IFXCreateComponent(CID_IFXCoreServicesRef, IID_IFXCoreServicesRef, (void**)&pCSRef)) && IFXSUCCESS(pCSRef->QueryInterface(IID_IFXCoreServices, (void**)&pWeakCS)))
        {
            IFXCoreServices* pCS;
...
CIFXCoreServices::~CIFXCoreServices()
{
    ...
    if (m_bInitialized)
    {
        ...
        IFXASSERT(m_pWeakCS);
        result = m_pWeakCS->QueryInterface(IID_IFXCoreServicesRef, (void**)&pCoreServicesRef);
...
CIFXCoreServices::CreateComponents()
{
    ...
    if (IFXSUCCESS(rc))
    {
        rc = IFXCreateComponent(CID_IFXDidRegistry, IID_IFXDidRegistry, (void**)&m_pDidRegistry);
    }
    ...
    {
        IFXRESULT rc = IFX_OK;
        rc = IFXCreateComponent(CID_IFXScheduler, IID_IFXScheduler, (void**)&m_pScheduler);
        if (IFXSUCCESS(rc))
        {
    ...
    if (IFXSUCCESS(rc))
    {
        rc = IFXCreateComponent(CID_IFXSceneGraph, IID_IFXSceneGraph, (void**)&m_pSceneGraph);
    }
    ...
    {
        IFXRESULT rc = IFX_OK;
        rc = IFXCreateComponent(CID_IFXNameMap, IID_IFXNameMap, (void**)&m_pNameMap);
        if (IFXSUCCESS(rc))
        {
    ...
    if (IFXSUCCESS(rc))
    {
        rc = IFXCreateComponent(CID_IFXPalette, IID_IFXPalette, (void**)&m_pFileReferencePalette);
    }
