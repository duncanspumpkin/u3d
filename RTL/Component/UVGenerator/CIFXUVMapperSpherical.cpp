...
/**
        @file	CIFXSphericalMap.h

                        Class implementation file for the spherical texture coordinate
                        generator class.
*/

#include "CIFXUVMapperSpherical.h"

CIFXUVMapperSpherical::CIFXUVMapperSpherical()
    : CIFXUVMapperNone(TRUE)
{
}
...
// Factory method
IFXRESULT IFXAPI_CALLTYPE CIFXUVMapperSpherical_Factory(IFXREFIID interfaceId, void** ppInterface)
{
    IFXRESULT result;
    ...
    if (ppInterface)
    {
        ...
        // upon destruction.
        CIFXUVMapperSpherical* pComponent = new CIFXUVMapperSpherical;
...
BOOL CIFXUVMapperSpherical::NeedToMap(IFXMesh& rMesh, IFXUVMapParameters* pParams)
{
    BOOL bRet = FALSE;
    ...
    if (pParams)
    {
        ...
        else if (!(IFX_UV_SPHERICAL == rUVMapperParams.eWrapMode))
        {
            bRet = TRUE;
        }
...
IFXRESULT CIFXUVMapperSpherical::Map(IFXMesh& rMesh, IFXUVMapParameters* pParams, IFXMatrix4x4* pModelMatrix, IFXMatrix4x4* pViewMatrix, const IFXLightSet* pLightSet)
{
    IFXRESULT rc = IFX_OK;
    ...
    if (IFXSUCCESS(rc))
    {
        ...
        if (IFXSUCCESS(rc))
        {
            ...
            for (uIndex = 0; uIndex < uNumVertex; uIndex++)
            {
                ...
                dZ = vTransformedPoint.Z();

                // calculate the spherical coordinate
                dTheta = (atan2(dX, dY)) / (2 * IFX_UV_PI);

                t1 = sqrt(dX * dX + dY * dY + dZ * dZ);
                t2 = asin(dZ / t1);
                dPhi = t2 / (2 * IFX_UV_PI) + 0.5;

                pvTexCoord->Set(1.0f - (F32)dTheta, (F32)dPhi);
            }
