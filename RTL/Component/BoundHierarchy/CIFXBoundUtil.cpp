...
U32 CIFXBoundUtil::ComputeLongestAxis(IFXVector3* pMin, IFXVector3* pMax)
{
    ...
    if (fLength[1] > fLength[uAxisIndex])
    {
        uAxisIndex = 1;
    }
    if (fLength[2] > fLength[uAxisIndex])
    {
        uAxisIndex = 2;
    }
...
IFXRESULT CIFXBoundUtil::InitFaceNode(CIFXBoundFace** ppFaceList, U32* pNumFaces, IFXVector3** ppVertexList, U32* pNumVerts, IFXMeshGroup* pMeshGroup)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT CIFXBoundUtil::SplitFaceList_Midpoint(CIFXBoundFace* pFaceList, CIFXBoundFace** ppLeftFaces, CIFXBoundFace** ppRightFaces, U32 uNumFaces, U32* pNumLeftFaces, U32* pNumRightFaces, IFXVector3* pvMin, IFXVector3* pvMax)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT CIFXBoundUtil::SplitFaceList_SortedMedian(CIFXBoundFace* pFaceList, U32 uNumFaces, IFXVector3* pvMin, IFXVector3* pvMax, U32* pMedian)
{
    IFXRESULT result = IFX_OK;
    ...
    if (pFaceList && pvMin && pvMax && pMedian)
    {
        ...
        if (IFXSUCCESS(result))
        {
            ...
            for (i = 1; i < uNumFaces; i++)
            {
                ...
                while ((j >= 0) && (nextFace.GetCentroidComponent(uAxisIndex) < pFaceList[j].GetCentroidComponent(uAxisIndex)))
                {
