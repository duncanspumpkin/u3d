...
IFXRESULT CIFXAABBHierarchyBuilder::Build(IFXBoundHierarchy** ppBoundHierarchy, U32 uType, IFXMeshGroup* pMeshGroup)
{
    ...
    if (pMeshGroup && ppBoundHierarchy)
    {
        ...
        if (IFXSUCCESS(result))
        {
            result = IFXCreateComponent(CID_IFXBoundHierarchy, IID_IFXBoundHierarchy, (void**)ppBoundHierarchy);
        }
