...
IFXRESULT CIFXBoundHierarchy::QueryInterface(IFXREFIID interfaceId, void** ppInterface)
{
...
void CIFXBoundHierarchy::GetMatrixComponents(U32 uIndex, IFXVector3& mTranslation, IFXVector3& mScaleFactor, IFXMatrix4x4& mTargetMatrix)
{
...
void CIFXBoundHierarchy::GetWorldMatrix(U32 uIndex, IFXMatrix4x4& mWorldMatrix)
{
    IFXASSERT(uIndex == 0 || uIndex == 1);
...
IFXRESULT CIFXBoundHierarchy::InitHierarchy(IFXMeshGroup* pMeshGroup, IFXVector3** ppVertexList)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT CIFXBoundHierarchy::IntersectBoxSphere(IFXBoundHierarchy* pHierarchy, F32 fRadiusSquared, IFXVector3& vSphereCentroid, IFXVector3& vContactPoint, IFXVector3 vContactNormal[2])
{
    IFXRESULT result = IFX_OK;
    ...
    if (pHierarchy)
    {
        ...
        if (pRootNode)
        {
            ...
            result = bTree.IntersectBoxSphere(m_pRoot, pRootNode, fRadiusSquared, vSphereCentroid, vContactPoint, vContactNormal);
        }
        else
        {
            result = IFX_E_NOT_INITIALIZED;
...
IFXRESULT CIFXBoundHierarchy::IntersectHierarchyQuick(IFXBoundHierarchy* pHierarchy, IFXVector3 vMin[2], IFXVector3 vMax[2])
{
    IFXRESULT result = IFX_OK;
    ...
    if (pHierarchy)
    {
        ...
        if (pRootNode)
        {
            ...
            result = bTree.IntersectTraverseQuick(m_pRoot, pRootNode, vMin, vMax);
        }
        else
        {
            result = IFX_E_NOT_INITIALIZED;
...
IFXRESULT CIFXBoundHierarchy::IntersectRay(IFXVector3& vOrigin, IFXVector3& vDirection, U32 uPickType, CIFXCollisionResult** ppResultPointer)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT CIFXBoundHierarchy::SetResult(U32 uModelIndex, U32 uMeshID, U32 uFaceID, IFXVector3& vIntersectPoint, IFXVector3& vIntersectNormal)
{
    IFXRESULT result = IFX_OK;
    ...
    {
        if ((pCollisionResult->GetMeshID() == uMeshID) && (pCollisionResult->GetFaceID() == uFaceID))
        {
            bFound = TRUE;
            break;
        }
...
IFXRESULT CIFXBoundHierarchy::SetResult(U32 uMeshID, U32 uFaceID, IFXVector3 vVerts[3], F32 fU, F32 fV, F32 fT)
{
    IFXRESULT result = IFX_OK;
