...
void CIFXPrimitiveOverlap::ClosestPointToBox(IFXVector3 vVertsBoxA[10], IFXVector3 vVertsBoxB[10], IFXVector3& vClosestPoint, IFXVector3& vContactNormal)
{
    ...
    F32 fDistance[] = { FLT_MAX, FLT_MAX, FLT_MAX, FLT_MAX, FLT_MAX, FLT_MAX };
    U32 uIndex = 0;
    ...
    ComputeVertexEdgeNormal(vVertsBoxA, vNormalA, vClosestPoint, vNewNormal[2], fDistance[2]);
    vNewNormal[2].Negate();

    ComputeVertexEdgeNormal(vVertsBoxB, vNormalB, vClosestPoint, vNewNormal[3], fDistance[3]);
...
void CIFXPrimitiveOverlap::ComputeTriangleNormal(IFXVector3& vP1, IFXVector3& vP2, IFXVector3& vP3, IFXVector3& vNormal)
{
    IFXVector3 vE1;
    IFXVector3 vE2;
...
IFXRESULT CIFXPrimitiveOverlap::Intersection2DLineSegment(IFXVector2* pOrigin1, IFXVector2* pDirection1, IFXVector2* pOrigin2, IFXVector2* pDirection2, IFXVector2* pIntersectionPoint)
{
...
BOOL CIFXPrimitiveOverlap::Intersection3DLineSegment(IFXVector3& vP1, IFXVector3& vP2, IFXVector3& vP3, IFXVector3& vP4, IFXVector3& vApproachPoint, F32& fDistance)
{
    IFXVector3 vD43;
    vD43.Subtract(vP4, vP3);

    if (fabsf(vD43[0]) < IFX_EPSILON && fabsf(vD43[1]) < IFX_EPSILON && fabsf(vD43[2]) < IFX_EPSILON)
    {
        return FALSE;
    }
    ...
    if (fabsf(vD21[0]) < IFX_EPSILON && fabsf(vD21[1]) < IFX_EPSILON && fabsf(vD21[2]) < IFX_EPSILON)
    {
        return FALSE;
    }
...
IFXRESULT CIFXPrimitiveOverlap::ObbObb(IFXMatrix4x4& mRotationBtoA, IFXVector3& vHalfWidthB, IFXVector3& vHalfWidthA)
{
    ...
    fLeftSide = fabsf(vTranslateBtoA[0] * mRotationBtoA[0] + vTranslateBtoA[1] * mRotationBtoA[1] + vTranslateBtoA[2] * mRotationBtoA[2]);
    fRightSide = vHalfWidthB[0] + (vHalfWidthA[0] * MatrixABS[0]) + (vHalfWidthA[1] * MatrixABS[4]) + (vHalfWidthA[2] * MatrixABS[8]);
    if (fLeftSide > fRightSide)
    {
        // m_bMarked = FALSE;
        // pBoxB->SetMarked(FALSE);
    ...
    fLeftSide = fabsf(vTranslateBtoA[0] * mRotationBtoA[4] + vTranslateBtoA[1] * mRotationBtoA[5] + vTranslateBtoA[2] * mRotationBtoA[6]);
    fRightSide = vHalfWidthB[1] + (vHalfWidthA[0] * MatrixABS[1]) + (vHalfWidthA[1] * MatrixABS[5]) + (vHalfWidthA[2] * MatrixABS[9]);
    if (fLeftSide > fRightSide)
    {
        // m_bMarked = FALSE;
        // pBoxB->SetMarked(FALSE);
    ...
    fLeftSide = fabsf(vTranslateBtoA[0] * mRotationBtoA[8] + vTranslateBtoA[1] * mRotationBtoA[9] + vTranslateBtoA[2] * mRotationBtoA[10]);
    fRightSide = vHalfWidthB[2] + (vHalfWidthA[0] * MatrixABS[2]) + (vHalfWidthA[1] * MatrixABS[6]) + (vHalfWidthA[2] * MatrixABS[10]);
    if (fLeftSide > fRightSide)
    {
        // m_bMarked = FALSE;
        // pBoxB->SetMarked(FALSE);
...
BOOL CIFXPrimitiveOverlap::RaySphereIntersection(IFXVector4& vBoundingSphere, IFXVector3& vOrigin, IFXVector3& vDirection)
{
...
IFXRESULT CIFXPrimitiveOverlap::RayTriangle(IFXVector3 vTriangle[3], IFXVector3 vOrigin, IFXVector3 vDirection, U32 uPickType, F32* pfU, F32* pfV, F32* pfT)
{
    IFXRESULT result = IFX_TRUE;
...
IFXRESULT CIFXPrimitiveOverlap::ThreePlaneIntersection(IFXVector3* pP1, IFXVector3* pN1, IFXVector3* pP2, IFXVector3* pN2, IFXVector3* pP3, IFXVector3* pN3, IFXVector3* pIPoint)
{
    IFXRESULT result = IFX_OK;

    if (pP1 && pN1 && pP2 && pN2 && pP3 && pN3 && pIPoint)
    {
        F32 fDeterminant = pN1->X() * (pN2->Y() * pN3->Z() - pN3->Y() * pN2->Z()) - pN1->Y() * (pN2->X() * pN3->Z() - pN3->X() * pN2->Z()) + pN1->Z() * (pN2->X() * pN3->Y() - pN3->X() * pN2->Y());
...
IFXRESULT CIFXPrimitiveOverlap::TriangleTriangle(IFXOverlapTriangle TriA, IFXOverlapTriangle TriB, IFXVector3 vIntersectPoint[2], IFXVector3 vIntersectNormal[2])
{
    ...
    if (fabsf(du[0]) < IFX_EPSILON)
    {
        du[0] = 0;
    }
    if (fabsf(du[1]) < IFX_EPSILON)
    {
        du[1] = 0;
    }
    if (fabsf(du[2]) < IFX_EPSILON)
    {
        du[2] = 0;
    }
    ...
    if (fabsf(dv[0]) < IFX_EPSILON)
    {
        dv[0] = 0.0f;
    }
    if (fabsf(dv[1]) < IFX_EPSILON)
    {
        dv[1] = 0.0f;
    }
    if (fabsf(dv[2]) < IFX_EPSILON)
    {
        dv[2] = 0.0f;
    }
...
IFXRESULT CIFXPrimitiveOverlap::TriangleTriangleERIT(IFXVector3 vTriA[3], IFXVector3 vTriB[3], IFXVector3* pvIntersectPoint, IFXVector3* pvIntersectNormal, F32* pfPenetrationDistance)
{
    IFXRESULT result = IFX_TRUE;
    ...
    if (pvIntersectPoint && pvIntersectNormal && pfPenetrationDistance)
    {
        ...
        if ((fduABS[0] = fabsf(du[0])) < IFX_EPSILON)
        {
            du[0] = 0.0f;
        }
        if ((fduABS[1] = fabsf(du[1])) < IFX_EPSILON)
        {
            du[1] = 0.0f;
        }
        if ((fduABS[2] = fabsf(du[2])) < IFX_EPSILON)
        {
            du[2] = 0.0f;
        }
        ...
        if (result == IFX_TRUE)
        {
            ...
            U32 uIndex = 0;
            U32 uOther[] = { 1, 2 };
            if (fabsf(vTriBNormal[1]) > fabsf(vTriBNormal[uIndex]))
            {
                uIndex = 1;
                uOther[0] = 0;
                uOther[1] = 2;
            }
            if (fabsf(vTriBNormal[2]) > fabsf(vTriBNormal[uIndex]))
            {
                uIndex = 2;
                uOther[0] = 0;
                uOther[1] = 1;
            ...
            else if (pInside && !qInside)
            {
                ...
                for (i = 0; i < 3; i++)
                {
                    result = Intersection3DLineSegment(iPointP, iPointQ, vTriB[i], vTriB[(i + 1) % 3], *pvIntersectPoint, fDistance);
                    if (result == IFX_TRUE)
                    {
            ...
            else if (!pInside && qInside)
            {
                ...
                for (i = 0; i < 3; i++)
                {
                    result = Intersection3DLineSegment(iPointQ, iPointP, vTriB[i], vTriB[(i + 1) % 3], *pvIntersectPoint, fDistance);
                    if (result == IFX_TRUE)
                    {
...
void CIFXPrimitiveOverlap::ComputeBoxNormals(IFXVector3 vVertsBox[10], IFXVector3 vNormal[6])
{
    IFXVector3 vE1, vE2;
...
BOOL CIFXPrimitiveOverlap::ComputeIntervals(F32 fProjVert[3], F32 fDist[3], F32 fPosNeg[2], F32 fTmps[3], F32 fIsect[2])
{
    if (fPosNeg[0] > 0.0f)
    {
        ISECT(fProjVert[2], fProjVert[0], fProjVert[1], fDist[2], fDist[0], fDist[1], fIsect[0], fIsect[1]);
    }
    else if (fPosNeg[1] > 0.0f)
    {
        ISECT(fProjVert[1], fProjVert[0], fProjVert[2], fDist[1], fDist[0], fDist[2], fIsect[0], fIsect[1]);
    }
    else if (((fDist[1] * fDist[2]) > 0.0f) || fDist[0] != 0.0f)
    {
        ISECT(fProjVert[0], fProjVert[1], fProjVert[2], fDist[0], fDist[1], fDist[2], fIsect[0], fIsect[1]);
    }
...
void CIFXPrimitiveOverlap::ComputeVertexEdgeNormal(IFXVector3 vVerts[10], IFXVector3 vNormalsBox[6], IFXVector3& vPoint, IFXVector3& vNewNormal, F32& fDistance)
{
...
void CIFXPrimitiveOverlap::ComputeVertexNormal(IFXVector3 vNormalsBox[6], IFXVector3& vNewNormal, U32 uVertex)
{
    switch (uVertex)
    {
        case 0:
...
IFXRESULT CIFXPrimitiveOverlap::CoplanarTriangles(IFXVector3* pvNormal, IFXVector3 vTriA[3], IFXVector3 vTriB[3])
{
    IFXRESULT result = IFX_OK;
    ...
    if (pvNormal)
    {
        ...
        if (IFX_TRUE == ProjectedEdgeOverlap(&vTriA[0], &vTriB[0], &vTriB[1], fAx, fAy, uI0, uI1))
        {
            return IFX_TRUE;
        }
        if (IFX_TRUE == ProjectedEdgeOverlap(&vTriA[0], &vTriB[1], &vTriB[2], fAx, fAy, uI0, uI1))
        {
            return IFX_TRUE;
        }
        if (IFX_TRUE == ProjectedEdgeOverlap(&vTriA[0], &vTriB[2], &vTriB[0], fAx, fAy, uI0, uI1))
        {
            return IFX_TRUE;
        }
        ...
        if (IFX_TRUE == ProjectedEdgeOverlap(&vTriA[1], &vTriB[0], &vTriB[1], fAx, fAy, uI0, uI1))
        {
            return IFX_TRUE;
        }
        if (IFX_TRUE == ProjectedEdgeOverlap(&vTriA[1], &vTriB[1], &vTriB[2], fAx, fAy, uI0, uI1))
        {
            return IFX_TRUE;
        }
        if (IFX_TRUE == ProjectedEdgeOverlap(&vTriA[1], &vTriB[2], &vTriB[0], fAx, fAy, uI0, uI1))
        {
            return IFX_TRUE;
        }
        ...
        if (IFX_TRUE == ProjectedEdgeOverlap(&vTriA[2], &vTriB[0], &vTriB[1], fAx, fAy, uI0, uI1))
        {
            return IFX_TRUE;
        }
        if (IFX_TRUE == ProjectedEdgeOverlap(&vTriA[2], &vTriB[1], &vTriB[2], fAx, fAy, uI0, uI1))
        {
            return IFX_TRUE;
        }
        if (IFX_TRUE == ProjectedEdgeOverlap(&vTriA[2], &vTriB[2], &vTriB[0], fAx, fAy, uI0, uI1))
        {
            return IFX_TRUE;
        }

        if (IFX_TRUE == PointInTri(&vTriA[0], vTriB, uI0, uI1))
        {
            return IFX_TRUE;
        }
        if (IFX_TRUE == PointInTri(&vTriB[0], vTriA, uI0, uI1))
        {
            return IFX_TRUE;
        }
...
IFXRESULT CIFXPrimitiveOverlap::ProjectedEdgeOverlap(IFXVector3* pvVA0, IFXVector3* pvVB0, IFXVector3* pvVB1, F32 fAx, F32 fAy, U32 uI0, U32 uI1)
{
    ...
    if (pvVA0 && pvVB0 && pvVB1)
    {
        ...
        if ((fF > 0.0f && fD >= 0.0f && fD <= fF) || (fF < 0.0f && fD <= 0.0f && fD >= fF))
        {
...
BOOL CIFXPrimitiveOverlap::PointInTriangle(IFXVector2 triangle[4])
{
    ...
    for (k = 0; k < 4; k++)
    {
        if (((triangle[k].Y() >= 0) && (triangle[k + 1].Y() < 0)) || ((triangle[k].Y() < 0) && (triangle[k + 1].Y() >= 0)))
        {
            ...
            else if (((triangle[k].X() >= 0) && (triangle[k + 1].X() < 0)) || ((triangle[k].X() < 0) && (triangle[k + 1].X() >= 0)))
            {
                F32 tmp_val = (triangle[k + 1].X() - triangle[k].X()) / (triangle[k + 1].Y() - triangle[k].Y());
