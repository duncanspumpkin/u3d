    ...
    if (m_pFaceList)
    {
        ...
        if (IFXSUCCESS(result))
        {
            ...
            for (i = 0; i < m_uNumFaces; i++)
            {
                ...
                if (!uIndices[uIndex])
                {
                    if (pVertexList[uIndex][0] > m_vMax[0])
                    {
                        m_vMax[0] = pVertexList[uIndex][0];
                    }
                    if (pVertexList[uIndex][1] > m_vMax[1])
                    {
                        m_vMax[1] = pVertexList[uIndex][1];
                    }
                    if (pVertexList[uIndex][2] > m_vMax[2])
                    {
                        m_vMax[2] = pVertexList[uIndex][2];
                    }

                    if (pVertexList[uIndex][0] < m_vMin[0])
                    {
                        m_vMin[0] = pVertexList[uIndex][0];
                    }
                    if (pVertexList[uIndex][1] < m_vMin[1])
                    {
                        m_vMin[1] = pVertexList[uIndex][1];
                    }
                    if (pVertexList[uIndex][2] < m_vMin[2])
                    {
                        m_vMin[2] = pVertexList[uIndex][2];
                    }
...
IFXRESULT CIFXAxisAlignedBBox::IntersectBoxSphere(IFXBoundVolume* pBound, F32 fSphereRadiusSquared, IFXVector3& vSphereCentroid, IFXVector3& vContactPoint, IFXVector3 vContactNormal[2])
{
    IFXRESULT result = IFX_OK;
    ...
    if (pBound)
    {
        ...
        else
        {
            ...
            vContactNormal[1].Set(-vContactNormal[0].X(), -vContactNormal[0].Y(), -vContactNormal[0].Z());
            result = IFX_TRUE;
        }
    }
    else
...
IFXRESULT CIFXAxisAlignedBBox::IntersectRay(IFXVector3 vOrigin, IFXVector3 vDirection, U32 uPickType)
{
...
IFXRESULT CIFXAxisAlignedBBox::IntersectRayTriangle(IFXVector3 vOrigin, IFXVector3 vDirection, U32 uPickType)
{
    IFXRESULT result = IFX_OK;
    ...
    if (m_pBoundHierarchy && m_pFaceList)
    {
        ...
        if (pVertexList)
        {
            ...
            for (i = 0; i < m_uNumFaces; i++)
            {
                ...
                result = overlap.RayTriangle(vTFVerts, vOrigin, vDirection, uPickType, &fU, &fV, &fT);
                ...
                if (result == IFX_TRUE)
                {
                    ...
                    m_pBoundHierarchy->SetResult(m_pFaceList[i].GetMeshID(), m_pFaceList[i].GetFaceID(), vTFVerts, fU, fV, fT);
                }
            }
        }
        else
        ...
        if (IFXSUCCESS(result))
        {
            ...
            for (i = 0; i < m_uNumFaces && IFXSUCCESS(result); i++)
            {
                ...
                for (j = 0; j < uNumFacesB && IFXSUCCESS(result); j++)
                {
                    ...
                    result = overlap.TriangleTriangle(triangleA, pTriangleB[j], vIntersectPoint, vIntersectNormal);
                    if (result == IFX_TRUE)
                    {
                        m_pFaceList[i].SetIntersect(IFX_TRUE);
                        pFaceListB[j].SetIntersect(IFX_TRUE);

                        result = m_pBoundHierarchy->SetResult(0, m_pFaceList[i].GetMeshID(), m_pFaceList[i].GetFaceID(), vIntersectPoint[0], vIntersectNormal[0]);
                        if (IFXSUCCESS(result))
                        {
                            result = pBoundHierarchyB->SetResult(1, pFaceListB[j].GetMeshID(), pFaceListB[j].GetFaceID(), vIntersectPoint[1], vIntersectNormal[1]);
                        }
                    }
                }
            }
...
IFXRESULT CIFXAxisAlignedBBox::SplitBound(CIFXAxisAlignedBBox** ppLeftBound, CIFXAxisAlignedBBox** ppRightBound)
{
    IFXRESULT result = IFX_OK;
        ...
        if (m_pBoundHierarchy)
        {
            switch (m_pBoundHierarchy->GetSplitType())
            {
                case IFXBoundHierarchy::MIDPOINT:
                    result = boundUtil.SplitFaceList_Midpoint(m_pFaceList, &pTmpLeftList, &pTmpRightList, m_uNumFaces, &uNumLeftFaces, &uNumRightFaces, &m_vMin, &m_vMax);
                    break;
                ...
                case IFXBoundHierarchy::SORTED_MEDIAN:
                    result = boundUtil.SplitFaceList_SortedMedian(m_pFaceList, m_uNumFaces, &m_vMin, &m_vMax, &uMedian);
                    if (IFXSUCCESS(result))
                    {
                        uNumLeftFaces = uMedian;
                        uNumRightFaces = m_uNumFaces - uMedian;
