...
IFXRESULT CIFXMeshCompiler::PreviewCompile(IFXAuthorCLODMesh* pInputMesh)
{
    ...
    if (IFXSUCCESS(result))
    {
        ...
        {
            m_pMeshSizes[i].VertexAttributes.m_uData.m_bHasDiffuseColors = m_pMaterials[i].m_uDiffuseColors;
            m_pMeshSizes[i].VertexAttributes.m_uData.m_bHasSpecularColors = m_pMaterials[i].m_uSpecularColors;
            m_pMeshSizes[i].VertexAttributes.m_uData.m_uNumTexCoordLayers = m_pMaterials[i].m_uNumTextureLayers;
            m_pMeshSizes[i].VertexAttributes.m_uData.m_bHasPositions = TRUE;

            m_pMeshSizes[i].VertexAttributes.m_uData.m_bHasNormals = m_pAM->GetMaxMeshDesc()->NumNormals > 0 ? TRUE : FALSE;

            // take into account the number of dimentions of texture coordinates
            m_pMeshSizes[i].VertexAttributes.m_uData.m_uTexCoordSize0 = m_pMaterials[i].m_uTexCoordDimensions[0];
            m_pMeshSizes[i].VertexAttributes.m_uData.m_uTexCoordSize1 = m_pMaterials[i].m_uTexCoordDimensions[1];
            m_pMeshSizes[i].VertexAttributes.m_uData.m_uTexCoordSize2 = m_pMaterials[i].m_uTexCoordDimensions[2];
            m_pMeshSizes[i].VertexAttributes.m_uData.m_uTexCoordSize3 = m_pMaterials[i].m_uTexCoordDimensions[3];
            m_pMeshSizes[i].VertexAttributes.m_uData.m_uTexCoordSize4 = m_pMaterials[i].m_uTexCoordDimensions[4];
            m_pMeshSizes[i].VertexAttributes.m_uData.m_uTexCoordSize5 = m_pMaterials[i].m_uTexCoordDimensions[5];
            m_pMeshSizes[i].VertexAttributes.m_uData.m_uTexCoordSize6 = m_pMaterials[i].m_uTexCoordDimensions[6];
            m_pMeshSizes[i].VertexAttributes.m_uData.m_uTexCoordSize7 = m_pMaterials[i].m_uTexCoordDimensions[7];
            ...
            */
            ...
            m_pMeshSizes[i].NumResolutionChanges = m_pMeshSizes[i].NumFaces * 4 + 100;

            // over estimate updates
            m_pMeshSizes[i].NumFaceUpdates = m_pMeshSizes[i].NumResolutionChanges * 18 + 100;
        }
    }
    ...
    for (r = m_LastResolution + 1; r <= m_pAM->GetMaxResolution(); r++)
    {
        ...
        for (m = 0; m < m_numMaterials; m++)
        {
            if (m_MaterialUpdated[m])
            {
                ...
                IFXASSERT(
                    m_pUG->GetUpdates(m)->numResChanges <= m_pMeshSizes[m].NumResolutionChanges);
            }
        }
    }
    ...
    U32* pOutIFXIndex)
{
    ...
    if (pMatDesc->m_uNormals)
    {
        ...
        else
        {
            ...
            pQ->pAttributes[attributeIndex++] = m_pNormalFaces[authorFaceIndex].corner[corner];
        }
    }

    for (layer = 0; layer < pMatDesc->m_uNumTextureLayers; layer++)
    {
        pQ->pAttributes[attributeIndex++] = m_pTextureFaces[layer][authorFaceIndex].corner[corner];
    }

    if (pMatDesc->m_uDiffuseColors)
    {
        pQ->pAttributes[attributeIndex++] = m_pDiffuseFaces[authorFaceIndex].corner[corner];
    }

    if (pMatDesc->m_uSpecularColors)
    {
        pQ->pAttributes[attributeIndex++] = m_pSpecularFaces[authorFaceIndex].corner[corner];
    }
    ...
    if (m_Stream)
    {
        ...
        if (pMatch && pMatDesc->m_uNormals && pMatch->Normal.DotProduct(pQ->Normal) < m_CosMaxNormalError)
        {
            pMatch = NULL;
        }
    }
...
IFXRESULT CIFXMeshCompiler::commonInit()
{
    ...
    if (IFXSUCCESS(result))
    {
        ...
        result = IFXCreateComponent(CID_IFXMeshMap, IID_IFXMeshMap, (void**)&m_pMeshMap);
    }
...
IFXRESULT CIFXMeshCompiler::allocateQueryVerts()
{
    ...
    for (i = 0; i < numMats; i++)
    {
        ...
        numAttributes = 1 + (pMat->m_uNormals ? 1 : 0) + pMat->m_uNumTextureLayers + (pMat->m_uDiffuseColors ? 1 : 0) + (pMat->m_uSpecularColors ? 1 : 0);
...
{
    if (fCosMaxNormalError > 1.0f)
    {
        fCosMaxNormalError = 1.0f;
    }
    if (fCosMaxNormalError < -1.0f)
    {
        fCosMaxNormalError = -1.0f;
