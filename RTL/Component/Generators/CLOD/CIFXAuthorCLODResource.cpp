...
IFXRESULT CIFXAuthorCLODResource::GetOutputs(IFXGUID**& rpOutOutputs, U32& rOutNumberOfOutputs, U32*& rpOutOutputDepAttrs)
{
    IFXRESULT result = IFX_OK;
    ...
    U32*& rpOutOutputDepAttrs)
{
    ...
    else if (pInOutputDID == &DID_IFXCLODController)
    {
        ...
        ruOutNumberOfOutputDependencies = sizeof(s_scpCLODControllerOutputDepDIDs) / sizeof(IFXGUID*);
...
IFXRESULT CIFXAuthorCLODResource::GenerateOutput(U32 inOutputDataElementIndex, void*& rpOutData, BOOL& rNeedRelease)
{
    IFXRESULT result = IFX_OK;
    ...
    IFXModifierDataPacket* pInDataPacket)
{
    ...
    if (pInInputDataPacket && pInDataPacket)
    {
        ...
        if (IFXSUCCESS(result))
        {
            m_pBoundSphereDataElement->RenderableIndex() = m_meshGroupDataElementIndex;
        }
...
IFXRESULT CIFXAuthorCLODResource::Notify(IFXModifierMessage eInMessage, void* pMessageContext)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT CIFXAuthorCLODResource::BuildMeshGroup()
{
    ...
    { // this means we are starting a stream or or are doing a static compile
        IFXRELEASE(m_pMeshCompiler);
        result = IFXCreateComponent(CID_IFXMeshCompiler, IID_IFXMeshCompiler, (void**)&m_pMeshCompiler);
        if (IFXSUCCESS(result))
        {
            ...
            {
                result = m_pMeshCompiler->InitCompile(m_pAuthorMesh, m_NumMeshSizes, m_pMeshSizes);
                if (IFXSUCCESS(result))
                {
...
IFXRESULT CIFXAuthorCLODResource::BuildCLODController()
{
    ...
    if (IFXSUCCESS(result))
    {
        IFXRELEASE(m_pCLODController);
        result = IFXCreateComponent(
            CID_IFXCLODManager, IID_IFXCLODManager, (void**)&m_pCLODController);
    }
