...
IFXRESULT IFXAPI_CALLTYPE CIFXAuthorMeshScrub_Factory(IFXREFIID interfaceId, void** ppInterface)
{
    IFXRESULT result;
...
IFXRESULT CIFXAuthorMeshScrub::QueryInterface(IFXREFIID interfaceId, void** ppInterface)
{
    ...
    IFXProgressCallback* pInProgressCallback)
{
    ...
    */
    ...
    if (IFXSUCCESS(result))
    {
        ...
        {
            result = pInAuthorMesh->Copy(IID_IFXAuthorMesh, (void**)&m_pScrubMesh);
        }
    }
    ...
    if (IFXSUCCESS(result))
    {
        result = IFXCreateComponent(CID_IFXAuthorMeshMap, IID_IFXAuthorMeshMap, (void**)&m_pMeshMap);
    }
    ...
    if (IFXSUCCESS(result))
    {
        m_TempBufferSize = sizeof(U32) * (IFXMAX(m_ScrubbedMeshDesc.NumFaces, IFXMAX(m_ScrubbedMeshDesc.NumPositions, IFXMAX(m_ScrubbedMeshDesc.NumNormals, IFXMAX(m_ScrubbedMeshDesc.NumSpecularColors, IFXMAX(m_ScrubbedMeshDesc.NumDiffuseColors, IFXMAX(m_ScrubbedMeshDesc.NumTexCoords, IFXMAX(m_ScrubbedMeshDesc.NumMaterials, m_ScrubbedMeshDesc.NumBaseVertices))))))));
    ...
    if (IFXSUCCESS(result) && m_pParams->bUnusedAttributes)
    {
        ...
        if (IFXSUCCESS(result) && m_pParams->RemoveUnusedDiffuseColors && m_ScrubbedMeshDesc.NumDiffuseColors)
        {
            result = RemoveUnusedDiffuseColors();
        }

        if (IFXSUCCESS(result) && m_pParams->RemoveUnusedSpecularColors && m_ScrubbedMeshDesc.NumSpecularColors)
        {
            result = RemoveUnusedSpecularColors();
        }

        if (IFXSUCCESS(result) && m_pParams->RemoveUnusedTextureCoordinates && m_ScrubbedMeshDesc.NumTexCoords)
        {
