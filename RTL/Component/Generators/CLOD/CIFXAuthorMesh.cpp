...
IFXRESULT IFXAPI_CALLTYPE CIFXAuthorMesh_Factory(IFXREFIID interfaceId, void** ppInterface)
{
    IFXRESULT result;
...
IFXRESULT CIFXAuthorMesh::QueryInterface(IFXREFIID interfaceId, void** ppInterface)
{
    ...
    if (ppInterface)
    {
        if (interfaceId == IID_IFXAuthorMesh || interfaceId == IID_IFXUnknown)
        {
...
IFXRESULT CIFXAuthorMesh::Reallocate(const IFXAuthorMeshDesc* pInMeshDesc)
{
    ...
    if (IFXSUCCESS(ir) && m_MaxMeshDesc.NumPositions != pInMeshDesc->NumPositions)
    {
        ...
        if (m_pUpdates)
        {
            IFXAuthorVertexUpdate* pNewUpdates = new IFXAuthorVertexUpdate[pInMeshDesc->NumPositions];
            if (!pNewUpdates)
            {
                ir = IFX_E_OUT_OF_MEMORY;
            }
            ...
            {
                U32 NumCpy = pInMeshDesc->NumPositions < m_MaxMeshDesc.NumPositions ? pInMeshDesc->NumPositions : m_MaxMeshDesc.NumPositions;
                U32 i;
                for (i = 0; i < NumCpy; ++i)
                {
    ...
    if (IFXSUCCESS(ir) && m_MaxMeshDesc.NumDiffuseColors != pInMeshDesc->NumDiffuseColors)
    {
        ReallocDataBlock(
            m_pDiffuseColors,
    ...
    if (IFXSUCCESS(ir) && m_MaxMeshDesc.NumSpecularColors != pInMeshDesc->NumSpecularColors)
    {
        ReallocDataBlock(
            m_pSpecularColors,
...
IFXRESULT CIFXAuthorMesh::Copy(IFXREFIID interfaceID, void** ppOutNewAuthorMesh)
{
    ...
    if (IFXSUCCESS(ir))
    {
        ir = IFXCreateComponent(
            CID_IFXAuthorMesh, IID_IFXAuthorCLODMesh, (void**)&pNewAuthorMesh);
    }
...

    {
        ...
        if (IFXSUCCESS(ir) && m_pMaterials)
        {
            ...
            {
                memcpy(
                    pTmpMat, m_pMaterials, sizeof(IFXAuthorMaterial) * m_MaxMeshDesc.NumMaterials);
            }
        }
        ...
        if (IFXSUCCESS(ir) && m_pPositionFaces)
        {
            ...
            {
                memcpy(
                    pTmpFace, m_pPositionFaces, sizeof(IFXAuthorFace) * m_MaxMeshDesc.NumFaces);
            }
        }
        ...
        if (IFXSUCCESS(ir) && m_pNormalFaces)
        {
            ...
            {
                memcpy(
                    pTmpFace, m_pNormalFaces, sizeof(IFXAuthorFace) * m_MaxMeshDesc.NumFaces);
            }
        }
        ...
        if (IFXSUCCESS(ir) && m_pDiffuseFaces)
        {
            ...
            {
                memcpy(
                    pTmpFace, m_pDiffuseFaces, sizeof(IFXAuthorFace) * m_MaxMeshDesc.NumFaces);
            }
        }
        ...
        if (IFXSUCCESS(ir) && m_pSpecularFaces)
        {
            ...
            {
                memcpy(
                    pTmpFace, m_pSpecularFaces, sizeof(IFXAuthorFace) * m_MaxMeshDesc.NumFaces);
            }
        }
        ...
        for (i = 0; i < j; ++i)
        {
            if (IFXSUCCESS(ir) && m_pTexCoordFaces[i].IsValid())
            {
                ...
                {
                    memcpy(
                        pTmpFace, m_pTexCoordFaces[i], sizeof(IFXAuthorFace) * m_MaxMeshDesc.NumFaces);
                }
            }
        }
        ...
        if (IFXSUCCESS(ir) && m_pFaceMaterials)
        {
            ...
            {
                memcpy(
                    pTmpU32, m_pFaceMaterials, sizeof(U32) * m_MaxMeshDesc.NumFaces);
            }
        }
        ...
        if (IFXSUCCESS(ir) && m_pBaseVertices)
        {
            ...
            {
                memcpy(
                    pTmpU32, m_pBaseVertices, sizeof(U32) * m_MaxMeshDesc.NumBaseVertices);
            }
        }
        ...
        if (IFXSUCCESS(ir) && m_pPositions)
        {
            ...
            {
                memcpy(
                    pTmpV3, m_pPositions, sizeof(IFXVector3) * m_MaxMeshDesc.NumPositions);
            }
        }
        if (IFXSUCCESS(ir) && m_pNormals)
        {
            ...
            {
                memcpy(
                    pTmpV3, m_pNormals, sizeof(IFXVector3) * m_MaxMeshDesc.NumNormals);
            }
        }
        ...
        if (IFXSUCCESS(ir) && m_pDiffuseColors)
        {
            ...
            {
                memcpy(
                    pTmpV4, m_pDiffuseColors, sizeof(IFXVector4) * m_MaxMeshDesc.NumDiffuseColors);
            }
        }
        if (IFXSUCCESS(ir) && m_pSpecularColors)
        {
            ...
            {
                memcpy(
                    pTmpV4, m_pSpecularColors, sizeof(IFXVector4) * m_MaxMeshDesc.NumSpecularColors);
            }
        }
        if (IFXSUCCESS(ir) && m_pTexCoords)
        {
            ...
            {
                memcpy(
                    pTmpV4, m_pTexCoords, sizeof(IFXVector4) * m_MaxMeshDesc.NumTexCoords);
            }
        }
...
IFXRESULT CIFXAuthorMesh::GetTexFaces(U32 index, IFXAuthorFace** ppOutTexFaces)
{
    ...
    if (IFXSUCCESS(ir) && m_pTexCoordFaces[index].IsNull())
    {
        ...
        {
            memset(
                m_pTexCoordFaces[index], 0, m_MaxMeshDesc.NumFaces * sizeof(IFXAuthorFace));
        }
    }
...
IFXRESULT CIFXAuthorMesh::GetTexCoord(U32 index, IFXVector4* pOutVector4)
{
    IFXRESULT ir = IFX_OK;
...
IFXRESULT CIFXAuthorMesh::SetTexCoord(U32 index, const IFXVector4* pInVector4)
{
    IFXRESULT ir = IFX_OK;
...
U32 CIFXAuthorMesh::SetResolution(U32 r)
{
    ...
    while (r > m_CurMeshDesc.NumPositions)
    {
        ...
        while (nfu--)
        {
            ...
            if (((U32)pfu->Attribute >= (U32)IFXAuthorFaceUpdate::Tex0) && ((U32)pfu->Attribute < ((U32)IFXAuthorFaceUpdate::Tex0 + IFX_MAX_TEXUNITS)))
            {
            ...
            else
            {
                switch (pfu->Attribute)
                {
                    case IFXAuthorFaceUpdate::Position:
                        m_pPositionFaces[pfu->FaceIndex].corner[pfu->Corner] = pfu->IncrValue;
                        break;
                    case IFXAuthorFaceUpdate::Normal:
                        m_pNormalFaces[pfu->FaceIndex].corner[pfu->Corner] = pfu->IncrValue;
                        break;
                    case IFXAuthorFaceUpdate::Diffuse:
                        m_pDiffuseFaces[pfu->FaceIndex].corner[pfu->Corner] = pfu->IncrValue;
                        break;
                    case IFXAuthorFaceUpdate::Specular:
                        m_pSpecularFaces[pfu->FaceIndex].corner[pfu->Corner] = pfu->IncrValue;
                        break;
                    default:
                        IFXASSERT(0);
                }
    ...
    while (r < m_CurMeshDesc.NumPositions)
    {
        ...
        while (nfu--)
        {
            ...
            else
            {
                ...
                {
                    case IFXAuthorFaceUpdate::Position:
                        m_pPositionFaces[pfu->FaceIndex].corner[pfu->Corner] = pfu->DecrValue;
                        break;
                    case IFXAuthorFaceUpdate::Normal:
                        m_pNormalFaces[pfu->FaceIndex].corner[pfu->Corner] = pfu->DecrValue;
                        break;
                    case IFXAuthorFaceUpdate::Diffuse:
                        m_pDiffuseFaces[pfu->FaceIndex].corner[pfu->Corner] = pfu->DecrValue;
                        break;
                    case IFXAuthorFaceUpdate::Specular:
                        m_pSpecularFaces[pfu->FaceIndex].corner[pfu->Corner] = pfu->DecrValue;
                        break;
                    default:
                        IFXASSERT(0);
                }
...
IFXRESULT CIFXAuthorMesh::GenSmoothNormals()
{
    ...
    if (IFXSUCCESS(result))
    {
        ...
        m_CurMeshDesc.NumNormals = IFXMIN(m_CurMeshDesc.NumNormals, m_MaxMeshDesc.NumNormals);
    }
