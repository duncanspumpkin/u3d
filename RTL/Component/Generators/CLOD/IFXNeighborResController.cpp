...
BOOL IFXNeighborResController::CheckForDistalMerge(U32 other, U32 child, U32 parent)
{
    ...
    do
    {
        ...
        faceIter.JumpAcross();
    } while ((faceIter.GetFaceIndex() != face1) || (faceIter.GetMeshIndex() != mesh1));
    ...
    do
    {
        ...
        faceIter.JumpAcross();
    } while ((faceIter.GetFaceIndex() != face2) || (faceIter.GetMeshIndex() != mesh2));
    ...
    U32 resolution, U32 other, U32 child, U32 parent)
{
    ...
    BOOL found =
#endif
        m_pEdgeMap->FindEdge(other, child, &(pMerge->m_meshA), &(pMerge->m_faceA), &(pMerge->m_cornerA));
    IFXASSERT(found);

#ifdef _DEBUG
    found =
#endif
        m_pEdgeMap->FindEdge(other, parent, &(pMerge->m_meshB), &(pMerge->m_faceB), &(pMerge->m_cornerB));
    IFXASSERT(found);
...
void IFXNeighborResController::FindNonDeletedFaceForEdge(U32 v1, U32 v2)
{
    ...
    do
    {
        ...
        faceIter.JumpAcross();
    } while ((faceIter.GetFaceIndex() != face) || (faceIter.GetMeshIndex() != mesh));
    ...
    U32 meshIndex, U32 faceIndex, U32 cornerIndex)
{
    ...
    faceIter.JumpAcross();
    while ((faceIter.GetFaceIndex() != faceIndex) || (faceIter.GetMeshIndex() != meshIndex))
    {
...
{
    return !(m_pNeighborMesh->GetNeighborFaceArray(meshIndex)
                 [faceIndex]
                     .GetFaceFlags()
                     ->duplicateFlag);
}
...
IFXNeighborResController::ClassifyEdge(U32 meshIndex, U32 faceIndex, U32 cornerIndex)
{
    ...
    m_pNeighborMesh->GetCornerIter(meshIndex, faceIndex, cornerIndex, faceIter);
    faceIter.JumpAcross();
    while ((faceIter.GetMeshIndex() != meshIndex) || (faceIter.GetFaceIndex() != faceIndex))
    {
...
BOOL IFXNeighborResController::FindCollapseEdge(U32 meshIndex, U32* collapseFaceIndex, U32* collapseCornerIndex)
{
    ResolutionState* state = &(m_pMeshStates[meshIndex]);
    U32 prevNumFaces = state->prevNumFaces;
    U32 faceIndex;
...
BOOL IFXNeighborResController::FindEdgeEdge(U32 meshIndex, U32* collapseFaceIndex, U32* collapseCornerIndex)
{
    ResolutionState* state = &(m_pMeshStates[meshIndex]);
...
static void SetCollapseIndex(IFXCornerIter* pFaceIter)
{
    ...
    IFXASSERT(pFlags->collapseIndex == NO_COLLAPSE_INDEX || pFlags->collapseIndex == collapseIndex);
    ...
    for (faceIndex = state->numFaces; faceIndex < prevNumFaces; ++faceIndex)
    {
        if (IsNotDuplicate(meshIndex, faceIndex))
        {
            ...
            if (collapseFound)
            {
                ...
                } while ((faceIter.GetFaceIndex() != collapseFace) || (faceIter.GetMeshIndex() != meshIndex));
            }
        }
    }
}
...
void IFXNeighborResController::RemoveFace(U32 meshIndex, U32 faceIndex, U32 xCornerIndex)
{
    ...
    do
    {
        ...
        walkerA.JumpAcross();
    } while ((walkerA.GetFaceIndex() != faceIndex) || (walkerA.GetMeshIndex() != meshIndex));
    ...
    do
    {
        ...
        walkerB.JumpAcross();
    } while ((walkerB.GetFaceIndex() != faceIndex) || (walkerB.GetMeshIndex() != meshIndex));
    ...
    BOOL isEpEdge = (walkerB.GetMeshIndex() == meshIndex && walkerB.GetFaceIndex() == faceIndex);

    // Check if Ec is an edge (if T is F)
    BOOL isEcEdge = (walkerA.GetMeshIndex() == meshIndex && walkerA.GetFaceIndex() == faceIndex);
...
void IFXNeighborResController::AddFace(U32 meshIndex, U32 faceIndex, U32 xCornerIndex)
{
    ...
    do
    {
        ...
        walkerA.JumpAcross();
    } while ((walkerA.GetFaceIndex() != faceIndex1) || (walkerA.GetMeshIndex() != meshIndex1));
    ...
    do
    {
        ...
        walkerB.JumpAcross();
    } while ((walkerB.GetFaceIndex() != faceIndex2) || (walkerB.GetMeshIndex() != meshIndex2));
