...
static const U32 s_pMeshGroupUnchangedAttrs[] = {
    IFX_MESHGROUP_CB_ALL ^ (IFX_MESHGROUP_CB_FACES | IFX_MESHGROUP_CB_FACECOUNT | IFX_MESHGROUP_CB_VERTEXCOUNT),
    0
};
    ...
    if (IFXSUCCESS(result))
    {
            ...
            if (IFXSUCCESS(result))
            {
                ...
                for (i = 0; i < numElem; i++)
                {
                    ...
                    if (pFrustrum->GetProjectionMode() == IFX_FRUSTUM_PERSPECTIVE_PROJECTION)
                    {
                        ...
                        fNewRes = (pBounds->Bound().Radius() * ViewportHeight * g_fCLODMultiplier * m_fCLODScreenSpaceBias * LOD_PERSPECTIVE_SCALE_FACTOR) / (sqrtf(viewToModel.DotProduct(viewToModel)) * ProjectionDegrees);
                    }
                    else
                    {
                        ...
                        fNewRes = (pBounds->Bound().Radius() * ViewportHeight * g_fCLODMultiplier * m_fCLODScreenSpaceBias * LOD_ORTHO_SCALE_FACTOR / OrthoHeight);
                    }
        ...
        else
        {
            m_pCLODManager->SetResolution(
                (U32)(m_fCLODLevel * m_pCLODManager->GetMaxResolution()));
        }
    }
