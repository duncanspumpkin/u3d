...
IFXRESULT CIFXContourExtruder::QueryInterface(IFXREFIID interfaceId, void** ppInterface)
{
    ...
    if (ppInterface)
    {
        if (interfaceId == IID_IFXContourExtruder || interfaceId == IID_IFXUnknown)
        {
...
//---------------------------------------------------------------------------
IFXRESULT IFXAPI_CALLTYPE CIFXContourExtruder_Factory(IFXREFIID interfaceId, void** ppInterface)
{
    IFXRESULT result;
...
IFXRESULT CIFXContourExtruder::Extrude(SIFXExtruderProperties* pExtruderProperties, IFXSimpleList* pGlyphList, IFXMeshGroup** ppMeshGroup)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        uPathIndex = 0;
        if (pExtruderProperties->eFacing == IFXGlyphOutFacing || pExtruderProperties->eFacing == IFXGlyphBothFacing)
        {
        ...
        U32 uPathIndex2 = 0;
        if (pExtruderProperties->eFacing == IFXGlyphInFacing || pExtruderProperties->eFacing == IFXGlyphBothFacing)
        {
            //			pExtruderProperties->bWindClockWise=IFX_TRUE;
            //			pExtruderProperties->eNormalOrientation=IFXGlyphInverseNormals;
            pExtruderProperties->bWindClockWise = IFX_FALSE;
...
IFXRESULT CIFXContourExtruder::Extrude(SIFXExtruderProperties* pExtruderProperties, IFXContour* pContour, IFXMesh** ppMesh)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        if (IFXSUCCESS(iResult))
        {
            ...
            if (IFXSUCCESS(iResult))
            {
                ...
                {
                    iResult = pMesh->Allocate(vertexAttributes, uVertexCount * 2 * (2 + 1 /*pExtruderProperties->uDepthSteps*/), uFaceCount * (2 + 1 /*pExtruderProperties->uDepthSteps*/));
                }
                else
                {
                    iResult = pMesh->Allocate(vertexAttributes, uVertexCount * 2 * (1 /*pExtruderProperties->uDepthSteps*/), uFaceCount * (1 /*pExtruderProperties->uDepthSteps*/));
                }
            }
        }
    }
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        if (bBevel)
        {
            ...
            if (IFXGlyphBevelFaceFront == pExtruderProperties->eBevelFace || IFXGlyphBevelFaceBoth == pExtruderProperties->eBevelFace)
            {
                bBevelFront = IFX_TRUE;
            }

            if (IFXGlyphBevelFaceBack == pExtruderProperties->eBevelFace || IFXGlyphBevelFaceBoth == pExtruderProperties->eBevelFace)
            {
                bBevelBack = IFX_TRUE;
            }
        }
