...
IFXRESULT CIFXContourTessellator::QueryInterface(IFXREFIID interfaceId, void** ppInterface)
{
    ...
    if (ppInterface)
    {
        if (interfaceId == IID_IFXContourTessellator || interfaceId == IID_IFXUnknown)
        {
...
//---------------------------------------------------------------------------
IFXRESULT IFXAPI_CALLTYPE CIFXContourTessellator_Factory(IFXREFIID interfaceId, void** ppInterface)
{
    IFXRESULT result = IFX_OK;
...
static IFXRESULT TessellateMeshBuilder(
    ...
    IFXMeshGroup** ppMeshGroup)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        if (pTessellatorProperties->eFacing == IFXGlyphOutFacing || pTessellatorProperties->eFacing == IFXGlyphBothFacing)
        {
        ...
        if (pTessellatorProperties->eFacing == IFXGlyphInFacing || pTessellatorProperties->eFacing == IFXGlyphBothFacing)
        {
    ...
    IFXMeshGroup** ppMeshGroupBack)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        if (cdtAlgo.m_pPointArray != NULL && cdtAlgo.m_triList.GetNumberElements() > 0)
        {
