...
IFXRESULT CIFXContourGenerator::QueryInterface(IFXREFIID interfaceId, void** ppInterface)
{
    ...
    if (ppInterface)
    {
        if (interfaceId == IID_IFXContourGenerator || interfaceId == IID_IFXUnknown)
        {
...
//---------------------------------------------------------------------------
IFXRESULT IFXAPI_CALLTYPE CIFXContourGenerator_Factory(IFXREFIID interfaceId, void** ppInterface)
{
    IFXRESULT result;
...
IFXRESULT CIFXContourGenerator::StartPath()
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        if (IFXSUCCESS(iResult))
        {
            iResult = IFXCreateComponent(CID_IFXContour, IID_IFXContour, (void**)&m_pCurrentPath);
...
IFXRESULT CIFXContourGenerator::LineTo(F64 fX, F64 fY)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        F64 eps = 1e-4;
        if (fabs(vPoint.x - m_vLastPoint.x) < eps && fabs(vPoint.y - m_vLastPoint.y) < eps)
        {
            IFXRETURN(iResult);
        }
...
IFXRESULT CIFXContourGenerator::CurveTo(F64 fCx1, F64 fCy1, F64 fCx2, F64 fCy2, F64 fAx, F64 fAy, U32 uNumberOfSteps)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        F64 eps = 1e-4;
        if (fabs(fAx1 - fAx) < eps && fabs(fAy1 - fAy) < eps && fabs(fCx1 - fCx2) < eps && fabs(fCy1 - fCy2) < eps)
        {
            // IFXASSERT(0);
...
{
    if (fX < m_pBoundingBoxMin[0])
    {
        m_pBoundingBoxMin[0] = fX;
    }
    if (fY < m_pBoundingBoxMin[1])
    {
        m_pBoundingBoxMin[1] = fY;
    }

    if (fX > m_pBoundingBoxMax[0])
    {
        m_pBoundingBoxMax[0] = fX;
    }
    if (fY > m_pBoundingBoxMax[1])
    {
        m_pBoundingBoxMax[1] = fY;
    }
}
...
{
    IFXRESULT iResult = IFX_OK;
    iResult = IFXCreateComponent(CID_IFXSimpleList, IID_IFXSimpleList, (void**)&m_pGlyphList);
