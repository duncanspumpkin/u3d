...
IFXRESULT CIFXGlyph3DGenerator::QueryInterface(IFXREFIID interfaceId, void** ppInterface)
{
    ...
    if (ppInterface)
    {
        if (interfaceId == IID_IFXGlyph3DGenerator || interfaceId == IID_IFXUnknown)
        {
...
IFXRESULT IFXAPI_CALLTYPE CIFXGlyph3DGenerator_Factory(IFXREFIID interfaceId, void** ppInterface)
{
    IFXRESULT iResult;
...
IFXRESULT CIFXGlyph3DGenerator::BuildMesh(SIFXGlyphMeshParams* pInitInfo)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        {
            iResult = CollapseFinalMeshGroup(pBuildMeshGroup, &pFinalMeshGroup, uGlyphCount);
            if (IFXSUCCESS(iResult))
            {
    ...
    U32* pFinalMeshGroupIndex)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        if (pGlyph && pInitInfo->bTessellate == IFX_TRUE)
        {
            ...
            iResult = IFXCreateComponent(CID_IFXContourTessellator, IID_IFXContourTessellator, (void**)&pContourTessellator);
    ...
    if (IFXSUCCESS(iResult))
    {
        if (pGlyph && pInitInfo->bExtrude == IFX_TRUE)
        {
            ...
            iResult = IFXCreateComponent(CID_IFXContourExtruder, IID_IFXContourExtruder, (void**)&pContourExtruder);
            IFXASSERT(IFXSUCCESS(iResult));
    ...
    if (IFXSUCCESS(iResult))
    {
        if (pInitInfo->bTessellate == IFX_TRUE && pInitInfo->sCombiner.bFrontCap == IFX_TRUE)
        {
    ...
    if (IFXSUCCESS(iResult))
    {
        if (pInitInfo->bTessellate == IFX_TRUE && pInitInfo->sCombiner.bBackCap == IFX_TRUE)
        {
    ...
    IFXMeshGroup** ppMeshGroup)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        for (uPathIndex = 0; uPathIndex < uPathCount; uPathIndex++)
        {
            ...
            if (IFXSUCCESS(iResult) && pContour)
            {
                ...
                if (vMinPosition.x > vContourPosition.x)
                {
                    vMinPosition.x = vContourPosition.x;
                }
                if (vMinPosition.y > vContourPosition.y)
                {
                    vMinPosition.y = vContourPosition.y;
                }
                if (vMaxPosition.x < vContourPosition.x)
                {
                    vMaxPosition.x = vContourPosition.x;
                }
                if (vMaxPosition.y < vContourPosition.y)
                {
                    vMaxPosition.y = vContourPosition.y;
                }
    ...
    pvCubePosition[15] = pvCubePosition[17] = pvCubePosition[00];

    pvCubePosition[1].Set((F32)vMinPosition.x, (F32)vMaxPosition.y, fZmin);
    pvCubePosition[14] = pvCubePosition[20] = pvCubePosition[1];

    pvCubePosition[2].Set((F32)vMaxPosition.x, (F32)vMaxPosition.y, fZmin);
    pvCubePosition[5] = pvCubePosition[23] = pvCubePosition[2];

    pvCubePosition[3].Set((F32)vMaxPosition.x, (F32)vMinPosition.y, fZmin);
    pvCubePosition[4] = pvCubePosition[18] = pvCubePosition[3];

    pvCubePosition[8].Set((F32)vMaxPosition.x, (F32)vMinPosition.y, -fZmax);
    pvCubePosition[7] = pvCubePosition[19] = pvCubePosition[8];

    pvCubePosition[9].Set((F32)vMaxPosition.x, (F32)vMaxPosition.y, -fZmax);
    pvCubePosition[6] = pvCubePosition[22] = pvCubePosition[9];

    pvCubePosition[10].Set((F32)vMinPosition.x, (F32)vMaxPosition.y, -fZmax);
    pvCubePosition[13] = pvCubePosition[21] = pvCubePosition[10];

    pvCubePosition[11].Set((F32)vMinPosition.x, (F32)vMinPosition.y, -fZmax);
    pvCubePosition[12] = pvCubePosition[16] = pvCubePosition[11];

    pvCubeNormal[3].Set(0, 0, 1);
    pvCubeNormal[0] = pvCubeNormal[1] = pvCubeNormal[2] = pvCubeNormal[3];

    pvCubeNormal[7].Set(1, 0, 0);
    pvCubeNormal[4] = pvCubeNormal[5] = pvCubeNormal[6] = pvCubeNormal[7];

    pvCubeNormal[11].Set(0, 0, -1);
    pvCubeNormal[8] = pvCubeNormal[9] = pvCubeNormal[10] = pvCubeNormal[11];

    pvCubeNormal[15].Set(-1, 0, 0);
    pvCubeNormal[12] = pvCubeNormal[13] = pvCubeNormal[14] = pvCubeNormal[15];

    pvCubeNormal[19].Set(0, -1, 0);
    pvCubeNormal[16] = pvCubeNormal[17] = pvCubeNormal[18] = pvCubeNormal[19];

    pvCubeNormal[23].Set(0, 1, 0);
    pvCubeNormal[20] = pvCubeNormal[21] = pvCubeNormal[22] = pvCubeNormal[23];
...
IFXRESULT CIFXGlyph3DGenerator::StartGlyphString()
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        if (IFXSUCCESS(iResult))
        {
            iResult = IFXCreateComponent(CID_IFXSimpleList, IID_IFXSimpleList, (void**)&m_pGlyphString);
        ...
        if (IFXSUCCESS(iResult))
        {
            iResult = IFXCreateComponent(CID_IFXSimpleList, IID_IFXSimpleList, (void**)&m_pGlyphCommands);
        ...
        if (IFXSUCCESS(iResult))
        {
            iResult = IFXCreateComponent(CID_IFXGlyphTagBlock, IID_IFXGlyphTagBlock, (void**)&pTagBlock);
        }
        if (IFXSUCCESS(iResult))
        {
...
IFXRESULT CIFXGlyph3DGenerator::StartGlyph()
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        iResult = IFXCreateComponent(CID_IFXContourGenerator, IID_IFXContourGenerator, (void**)&m_pContourGenerator);
...
IFXRESULT CIFXGlyph3DGenerator::EndGlyph(F64 fWidth, F64 fHeight)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        if (IFXSUCCESS(iResult))
        {
            ...
            iResult = IFXCreateComponent(CID_IFXSimpleList, IID_IFXSimpleList, (void**)&pCompletedGlyph);
            if (IFXSUCCESS(iResult))
            {
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        if (IFXSUCCESS(iResult))
        {
            iResult = IFXCreateComponent(CID_IFXGlyphTagBlock, IID_IFXGlyphTagBlock, (void**)&pEndBlock);
        }
        if (IFXSUCCESS(iResult))
        {
...
IFXRESULT CIFXGlyph3DGenerator::LineTo(F64 fX, F64 fY)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        if (IFXSUCCESS(iResult))
        {
            iResult = IFXCreateComponent(CID_IFXGlyphLineToBlock, IID_IFXGlyphLineToBlock, (void**)&pLineToBlock);
        }
        if (IFXSUCCESS(iResult))
        {
...
IFXRESULT CIFXGlyph3DGenerator::MoveTo(F64 fX, F64 fY)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        if (IFXSUCCESS(iResult))
        {
            iResult = IFXCreateComponent(CID_IFXGlyphMoveToBlock, IID_IFXGlyphMoveToBlock, (void**)&pMoveToBlock);
        }
        if (IFXSUCCESS(iResult))
        {
...
IFXRESULT CIFXGlyph3DGenerator::CurveTo(F64 fCx1, F64 fCy1, F64 fCx2, F64 fCy2, F64 fAx, F64 fAy, U32 uNumberOfCurveSteps)
{
    ...
    if (IFXSUCCESS(iResult))
    {
        ...
        if (IFXSUCCESS(iResult))
        {
            iResult = IFXCreateComponent(CID_IFXGlyphCurveToBlock, IID_IFXGlyphCurveToBlock, (void**)&pCurveToBlock);
        }
        if (IFXSUCCESS(iResult))
        {
    ...
    if ((IFXSUCCESS(iResult)) && (pGlyphList != NULL))
    {
        ...
        for (i = 0; (i < uListCount) && (IFXSUCCESS(iResult)); i++)
        {
            ...
            if ((IFXSUCCESS(iResult)) && (pUnk != NULL))
            {
                ...
                iResult = pUnk->QueryInterface(IID_IFXGlyph2DCommands, (void**)&pGlyphCommand);
                ...
                if (IFXSUCCESS(iResult))
                {
                    switch (uCommandType)
                    {
                        ...
                        case IFXGlyph2DCommands::IGG_TYPE_MOVETO:
                        {
                            ...
                            iResult = pGlyphCommand->QueryInterface(IID_IFXGlyphMoveToBlock, (void**)&pGlyphMoveTo);
                            if (IFXSUCCESS(iResult))
                            {
                                iResult = pGlyphMoveTo->GetData(&fDoubleArray[0], &fDoubleArray[1]);
                            }
                        ...
                        case IFXGlyph2DCommands::IGG_TYPE_LINETO:
                        {
                            ...
                            iResult = pGlyphCommand->QueryInterface(IID_IFXGlyphLineToBlock, (void**)&pGlyphLineTo);
                            if (IFXSUCCESS(iResult))
                            {
                                iResult = pGlyphLineTo->GetData(&fDoubleArray[0], &fDoubleArray[1]);
                            }
                        ...
                        case IFXGlyph2DCommands::IGG_TYPE_CURVETO:
                        {
                            ...
                            iResult = pGlyphCommand->QueryInterface(IID_IFXGlyphCurveToBlock, (void**)&pGlyphCurveTo);
                            if (IFXSUCCESS(iResult))
                            {
                                iResult = pGlyphCurveTo->GetData(&fDoubleArray[0], &fDoubleArray[1], &fDoubleArray[2], &fDoubleArray[3], &fDoubleArray[4], &fDoubleArray[5],
                                                                 &uTemp); // not needed.
                            }
                        ...
                        case IFXGlyph2DCommands::IGG_TYPE_ENDGLYPH:
                        {
                            ...
                            iResult = pGlyphCommand->QueryInterface(IID_IFXGlyphTagBlock, (void**)&pGlyphTag);
                            if (IFXSUCCESS(iResult))
                            {
                                iResult = pGlyphTag->GetData(&fDoubleArray[0], &fDoubleArray[1]);
                            }
...
//---------------------------------------------------------------------------
IFXRESULT CIFXGlyph3DGenerator::CallTessellator(SIFXGlyphMeshParams* pInitInfo, IFXContourTessellator* pContourTessellator, IFXMeshGroup* pFinalMeshGroup, U32* pFinalMeshGroupIndex)
{
    IFXRESULT iResult = IFX_OK;
...
IFXRESULT CIFXGlyph3DGenerator::ApplyTextureCoordinates(IFXMesh* pMesh, IFXVector3 vBoxMin, IFXVector3 vBoxMax)
{
    IFXRESULT iResult = IFX_OK;
...
IFXRESULT CIFXGlyph3DGenerator::CalcBoundingBox(IFXMesh* pMesh, IFXVector3& vBoxMin, IFXVector3& vBoxMax)
{
    IFXRESULT iResult = IFX_OK;
...
IFXRESULT CIFXGlyph3DGenerator::CollapseFinalMeshGroup(IFXMeshGroup* pInMeshGroup, IFXMeshGroup** ppOutMeshGroup, U32 uGlyphCount)
{
    IFXRESULT iResult = IFX_OK;
...
IFXRESULT CIFXGlyph3DGenerator::CollapseGlyph(IFXMesh** ppMeshList, IFXMesh** ppOutMesh, U32 uMeshCount)
{
    IFXRESULT iResult = IFX_OK;
