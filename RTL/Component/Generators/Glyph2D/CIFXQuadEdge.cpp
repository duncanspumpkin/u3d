...
int InCircle(const SIFXPoint2d& a, const SIFXPoint2d& b, const SIFXPoint2d& c, const SIFXPoint2d& d);
...
static int ccw(const SIFXPoint2d& a, const SIFXPoint2d& b, const SIFXPoint2d& c)
{
    return (TriArea(a, b, c) > 0);
}

static int RightOf(const SIFXPoint2d& x, CIFXEdge* e)
{
    return ccw(x, e->Dest2d(), e->Org2d());
}

static int LeftOf(const SIFXPoint2d& x, CIFXEdge* e)
{
    return ccw(x, e->Org2d(), e->Dest2d());
}

static int Colinear(const SIFXPoint2d& x, CIFXEdge* e)
{
    F64 area2x = TriArea(x, e->Org2d(), e->Dest2d());
    return (fabs(area2x) == 0);
}

static inline int XOR(int x, int y) { return !x ^ !y; }

static int IntersectProp(CIFXEdge* a, CIFXEdge* b)
{
    if (
        Colinear(b->Dest2d(), a) || Colinear(b->Org2d(), a) || Colinear(a->Org2d(), b) || Colinear(a->Dest2d(), b))
    {
        return FALSE;
    }

    return XOR(LeftOf(b->Org2d(), a), LeftOf(b->Dest2d(), a)) && XOR(LeftOf(a->Org2d(), b), LeftOf(a->Dest2d(), b));
}

static int Between(const SIFXPoint2d& c, CIFXEdge* e)
{
    if (!Colinear(c, e))
    {
        return FALSE;
...
static int Intersect(CIFXEdge* a, CIFXEdge* b)
{
    if (IntersectProp(a, b))
    {
        ...
        Between(b->Org2d(), a) || Between(b->Dest2d(), a) || Between(a->Org2d(), b) || Between(a->Dest2d(), b))
    {
        return TRUE;
    }
    else
...
IFXRESULT CIFXSubdivision::Locate(const SIFXPoint2d& x, CIFXEdge** ppEdge)
{
    ...
    *ppEdge = NULL;
    ...
    if (
        x.x >= m_domainMax.x || x.x <= m_domainMin.x || x.y >= m_domainMax.y || x.y <= m_domainMin.y)
    {
        IFXASSERT(0);
...
static IFXRESULT SolveIntersection(CIFXEdge* a, CIFXEdge* b, SIFXPoint2d* site)
{
    IFXRESULT iResult = IFX_OK;
...
IFXRESULT CIFXSubdivision::IntersectInsertEdge(const SIFXPoint2d& A, const SIFXPoint2d& B)
{
    ...
    while ((newEdge = stack.GetHead()) != NULL)
    {
        ...
        {
            if (
                !(e->Lnext()->Dest2d() == b) && !(e->Lnext()->Org2d() == b) && !(e->Lnext()->Dest2d() == a) && !(e->Lnext()->Org2d() == a) && Intersect(e->Lnext(), proposedEdge))
            {
                cutEdge = e->Lnext();
                if (LeftOf(a, cutEdge))
                {
        ...
        else
        {
            ...
            while (IFXSUCCESS(iResult))
            {
                if (leftEdge->Dest2d() == b)
                {
                    ...
                    !(leftEdge->Dest2d() == b) && !(leftEdge->Org2d() == b) && !(leftEdge->Dest2d() == a) && !(leftEdge->Org2d() == a) && Intersect(leftEdge, proposedEdge))
                {
...
IFXRESULT CIFXSubdivision::InsertEdge(const SIFXPoint2d& a, const SIFXPoint2d& b)
{
    ...
    do
    {
        ...
        if (
            !(e->Lnext()->Dest2d() == b) && !(e->Lnext()->Org2d() == b) && !(e->Lnext()->Dest2d() == a) && !(e->Lnext()->Org2d() == a) && Intersect(e->Lnext(), base))
        {
            cutEdge = e->Lnext();
            if (LeftOf(a, cutEdge))
            {
    ...
    if (cutEdge != NULL)
    {
        ...
        while (IFXSUCCESS(iResult))
        {
            if (leftEdge->Dest2d() == b)
            {
                ...
                !(leftEdge->Dest2d() == b) && !(leftEdge->Org2d() == b) && !(leftEdge->Dest2d() == a) && !(leftEdge->Org2d() == a) && Intersect(leftEdge, base))
            {
                void** error = NULL;
