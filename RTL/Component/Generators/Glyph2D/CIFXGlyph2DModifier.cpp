...
CIFXGlyph2DModifier::CIFXGlyph2DModifier()
{
    ...
    IFXCreateComponent(CID_IFXGlyph3DGenerator, IID_IFXGlyph3DGenerator, (void**)&m_pGlyphGenerator);
...
const U32 CIFXGlyph2DModifier::m_scpMeshGroupUnchangedAttrs[] = { IFX_MESHGROUP_CB_ALL ^ (IFX_MESHGROUP_CB_NUMMESHES | IFX_MESHGROUP_CB_TRANSFORM),
                                                                  0 };
...
IFXRESULT CIFXGlyph2DModifier::GetOutputs(IFXGUID**& rpOutOutputs, U32& rOutNumberOfOutputs, U32*& rpOutOutputDepAttrs)
{
    ...
    U32*& rpOutOutputDepAttrs)
{
    ...
    else if ((pInOutputDID == &DID_IFXRenderableGroup) || (pInOutputDID == &DID_IFXRenderableGroupBounds))
    {
        rppOutInputDependencies = NULL;
        rOutNumberInputDependencies = 0;
...
IFXRESULT CIFXGlyph2DModifier::GenerateOutput(U32 inOutputDataElementIndex, void*& rpOutData, BOOL& rNeedRelease)
{
    IFXRESULT result = IFX_OK;
    ...
    else if (inOutputDataElementIndex == m_uBoundSphereDataElementIndex)
    {
        ...
        if (pUnk)
        {
            ...
            m_pBoundSphereDataElement->Bound() = pMG->CalcBoundingSphere();
            IFXRELEASE(pUnk);
            IFXRELEASE(pMG);
        }
    ...
    else if (inOutputDataElementIndex == m_uNeighborMeshDataElementIndex)
    {
        ...
        {
            IFXCreateComponent(CID_IFXNeighborMesh, IID_IFXNeighborMesh, (void**)&(m_sParameters.pNeighborMesh));
        }
    ...
    IFXModifierDataPacket* pInDataPacket)
{
    ...
    if (IFXSUCCESS(result))
    {
        m_pBoundSphereDataElement->RenderableIndex() = m_uMeshGroupDataElementIndex;
    }
...
IFXRESULT CIFXGlyph2DModifier::Notify(IFXModifierMessage eInMessage, void* pMessageContext)
{
    return IFX_OK;
}
...
IFXRESULT CIFXGlyph2DModifier::InvalidateMeshGroup()
{
    ...
    {
        return (m_pModifierDataPacket->InvalidateDataElement(m_uMeshGroupDataElementIndex));
    }
    else
    {
...
IFXMatrix4x4 CIFXGlyph2DModifier::CalcBillboardOffset()
{
    ...
    if (transformDataElementIndex != U32(-1))
    {
        ...
        m_pInputDataPacket->GetDataElement(transformDataElementIndex, (void**)&pMeshGroupTransforms);
