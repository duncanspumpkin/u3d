...
IFXRESULT CIFXAuthorPointSetResource::GetOutputs(IFXGUID**& rpOutOutputs, U32& rOutNumberOfOutputs, U32*& rpOutOutputDepAttrs)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT CIFXAuthorPointSetResource::BuildDataBlockQueue()
{
    ...
    result = IFXCreateComponent(CID_IFXDataBlockQueueX, IID_IFXDataBlockQueueX, (void**)&m_pDataBlockQueueX);
...
IFXRESULT CIFXAuthorPointSetResource::cpl_EvaluatePointSetSizes()
{
    ...
    if (IFXSUCCESS(ir))
    {
        ...
        for (i = 0; i < m_numMaterials; i++)
        {
            ...
            {
                m_pPointSetSizes[i].m_vertexAttributes.m_uData.m_bHasDiffuseColors = m_pMaterials[i].m_uDiffuseColors;
                m_pPointSetSizes[i].m_vertexAttributes.m_uData.m_bHasSpecularColors = m_pMaterials[i].m_uSpecularColors;
                m_pPointSetSizes[i].m_vertexAttributes.m_uData.m_uNumTexCoordLayers = m_pMaterials[i].m_uNumTextureLayers;
                m_pPointSetSizes[i].m_vertexAttributes.m_uData.m_bHasPositions = TRUE;
                m_pPointSetSizes[i].m_vertexAttributes.m_uData.m_bHasNormals = m_pAuthorPointSet->GetPointSetDesc()->m_numNormals > 0 ? TRUE : FALSE;
            }
        }
    }
...
IFXRESULT CIFXAuthorPointSetResource::cpl_AllocateOutput()
{
    ...
    if (IFXSUCCESS(ir))
    {
        ...
        for (i = 0; i < m_numMaterials && IFXSUCCESS(ir); i++)
        {
            ...
            if (IFXSUCCESS(ir))
            {
                ir = pMesh->Allocate(m_pPointSetSizes[i].m_vertexAttributes, m_pPointSetSizes[i].m_numVertices, 0);
            }
...
IFXRESULT CIFXAuthorPointSetResource::cpl_FindAndBuildVertex(U32 authorPointIndex, U32 mat, U32* pOutIFXIndex)
{
    IFXRESULT ir = IFX_OK;
