...
IFXRESULT CIFXAuthorLineSetAnalyzer::Initialize(IFXAuthorLineSet* pLineSet)
{
    ...
    if (pLineSet->GetMaxLineSetDesc()->m_numPositions < pLineSet->GetLineSetDesc()->m_numPositions)
    {
        IFXASSERT(0);
        return IFX_E_INVALID_RANGE;
    }
...
IFXRESULT CIFXAuthorLineSetAnalyzer::GetLines(U32 pointInd, IFXArray<U32>& lines, IFXArray<U32>& endPoints)
{
    IFXRESULT rc = IFX_OK;
...
IFXRESULT CIFXAuthorLineSetAnalyzer::Update()
{
    ...
    if (m_LineSetDesc.m_numPositions > pUpdatedLineSetDesc->m_numPositions || m_LineSetDesc.m_numLines > pUpdatedLineSetDesc->m_numLines)
    {
        IFXASSERT(0);
        return IFX_E_INVALID_RANGE;
    }
...
void CIFXAuthorLineSetAnalyzer::DumpAuthorLineSet()
{
    ...
    IFXTRACE_GENERIC(L" Diffuse colors: %i, Specular Colors: %i, Materials: %i\n", pLSDesc->m_numDiffuseColors, pLSDesc->m_numSpecularColors, pLSDesc->m_numMaterials);
    ...
    for (j = 0; j < pLSDesc->m_numLines; j++)
    {
        ...
        if (pLSDesc->m_numDiffuseColors > 0 && lineMaterial.m_uDiffuseColors)
        {
            ...
            IFXTRACE_GENERIC(L"\t Line Diffuse colors: %i %i ", line.VertexA(), line.VertexB());
        ...
        if (pLSDesc->m_numSpecularColors > 0 && lineMaterial.m_uSpecularColors)
        {
            ...
            IFXTRACE_GENERIC(L"\t Line Specular colors: %i %i ", line.VertexA(), line.VertexB());
