...
IFXRESULT CIFXAuthorLineSetResource::GetOutputs(IFXGUID**& rpOutOutputs, U32& rOutNumberOfOutputs, U32*& rpOutOutputDepAttrs)
{
    IFXRESULT result = IFX_OK;
    ...
    U32*& rpOutOutputDepAttrs)
{
    ...
    else if (pInOutputDID == &DID_IFXNeighborResController)
    {
        ...
        ruOutNumberOfOutputDependencies = sizeof(s_scpNeighborResControllerOutputDepDIDs) / sizeof(IFXGUID*);
        rpOutOutputDepAttrs = NULL;
    }
...
IFXRESULT CIFXAuthorLineSetResource::GenerateOutput(U32 inOutputDataElementIndex, void*& rpOutData, BOOL& rNeedRelease)
{
    IFXRESULT result = IFX_OK;
    ...
    IFXModifierDataPacket* pInDataPacket)
{
    ...
    if (pInInputDataPacket && pInDataPacket)
    {
        ...
        if (IFXSUCCESS(result))
        {
            m_pBoundSphereDataElement->RenderableIndex() = m_uRenderableGroupDataElementIndex;
        }
...
IFXRESULT CIFXAuthorLineSetResource::BuildDataBlockQueue()
{
    ...
    result = IFXCreateComponent(
        CID_IFXDataBlockQueueX, IID_IFXDataBlockQueueX, (void**)&m_pDataBlockQueueX);
...
IFXRESULT CIFXAuthorLineSetResource::BuildNeighborMesh()
{
    ...
    if (IFXSUCCESS(result))
    {
        ...
        IFXRELEASE(m_pNeighborMesh)
        result = IFXCreateComponent(
            CID_IFXNeighborMesh, IID_IFXNeighborMesh, (void**)&(m_pNeighborMesh));
    }
...
IFXRESULT CIFXAuthorLineSetResource::cpl_EvaluateLineSetSizes()
{
    ...
    if (IFXSUCCESS(ir))
    {
        ...
        for (i = 0; i < m_numMaterials; i++)
        {
            ...
            {
                m_pLineSetSizes[i].m_vertexAttributes.m_uData.m_bHasDiffuseColors = m_pMaterials[i].m_uDiffuseColors;
                m_pLineSetSizes[i].m_vertexAttributes.m_uData.m_bHasSpecularColors = m_pMaterials[i].m_uSpecularColors;
                m_pLineSetSizes[i].m_vertexAttributes.m_uData.m_uNumTexCoordLayers = m_pMaterials[i].m_uNumTextureLayers;
                m_pLineSetSizes[i].m_vertexAttributes.m_uData.m_bHasPositions = TRUE;
                m_pLineSetSizes[i].m_vertexAttributes.m_uData.m_bHasNormals = m_pAuthorLineSet->GetLineSetDesc()->m_numNormals > 0 ? TRUE : FALSE;
...
IFXRESULT CIFXAuthorLineSetResource::cpl_AllocateOutput()
{
    ...
    if (IFXSUCCESS(ir))
    {
        ...
        for (i = 0; i < m_numMaterials; i++)
        {
            ...
            if (IFXSUCCESS(ir))
            {
                ir = pMesh->Allocate(m_pLineSetSizes[i].m_vertexAttributes, m_pLineSetSizes[i].m_numVertices, 0);
            }
