...
IFXRESULT CIFXModifierDataPacket::QueryInterface(IFXREFIID interfaceId, void** ppInterface)
{
...
IFXRESULT CIFXModifierDataPacket::GetDataElementIndex(IFXREFGUID rInDataElementGUID, U32& rOutIndex)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT CIFXModifierDataPacket::GetDataElement(U32 inDataElementIndex, void** ppOutDataElement)
{
    IFXRESULT result = IFX_OK;
    ...
    if (IFXSUCCESS(result) && inDataElementIndex < m_pState->m_NumDataElements)
    {
        ...
        if (pDEState->State == IFXDATAELEMENTSTATE_INVALID || pDEState->State == IFXDATAELEMENTSTATE_VALIDATIONFAILED)
        {
...
IFXRESULT CIFXModifierDataPacket::GetDataElementChangeCount(U32 inDataElementIndex, U32& outChangeCount)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT CIFXModifierDataPacket::InvalidateDataElement(U32 inInvalidDataElementIndex)
{
    ...
    {
        result = m_pModChainInternal->Invalidate(inInvalidDataElementIndex, m_ModChainIndex);
    }

    return result;
}

IFXRESULT CIFXModifierDataPacket::DataElementIsValid(U32 inDataElementIndex, BOOL& rbOutIsValid)
{
    IFXDataElementState* pDEState = NULL;
    IFXASSERT(m_pState);
...
IFXRESULT CIFXModifierDataPacket::SetModifierChain(IFXModifierChain* pInModChain, U32 index, IFXDataPacketState* pInState)
{
    IFXRESULT result = IFX_OK;
...
// Iterator methods
IFXRESULT CIFXModifierDataPacket::GetIterator(U32 inFlags, IFXModifierDataElementIterator** pOutpItor)
{
    IFXRESULT result = IFX_OK;
    ...
    if (IFXSUCCESS(result))
    {
        pIterator->Initialize(inFlags, m_pState->m_pDids, m_pState->m_NumDataElements, this);
    }
