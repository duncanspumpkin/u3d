...
IFXRESULT IFXIntraDependencies::AddDependentElement(U32 in_DepEl, U32 in_Attr)
{
    ...
    if (AllocatedSize == Size)
    {
        ...
        if (pDepElementsList)
        {
            memcpy(pTmpDepElementsList, pDepElementsList, sizeof(sElementDependency) * (Size));
            IFXDELETE_ARRAY(pDepElementsList);
        }
...
IFXRESULT IFXIntraDependencies::CopyFrom(IFXIntraDependencies* in_pSrc)
{
    ...
    if (AllocatedSize)
    {
        ...
        {
            memcpy(pDepElementsList, in_pSrc->pDepElementsList, sizeof(sElementDependency) * (Size));
        }
    }
    return IFX_OK;
}
...
IFXRESULT IFXModifierChainState::Initialize(IFXModifierChainInternal* in_pModChain, IFXModifierChainInternal* in_pBaseChain, IFXModifierDataPacketInternal* in_pOverrideDP, U32 in_Size, IFXDidRegistry* in_pDidRegistry)
{
    ...
    if (IFXSUCCESS(result))
    {
        ...
        for (i = 0; i < m_NumModifiers; ++i)
        {
            ...
            result = IFXCreateComponent(CID_IFXModifierDataPacket, IID_IFXModifierDataPacketInternal, (void**)&(pDataPacket));

            if (IFXSUCCESS(result))
            {
                // set up the proxy data packet
                result = pDataPacket->SetModifierChain(in_pModChain, i - 1, m_pDataPacketState + i);
            }
...
IFXRESULT IFXModifierChainState::SetModifier(U32 in_Idx, IFXModifier* in_pMod, BOOL in_bEnabled)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT IFXModifierChainState::GetModifier(U32 in_Idx, IFXModifier** out_ppMod)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT IFXModifierChainState::GetModifierDataPacket(U32 in_Idx, IFXModifierDataPacket** out_ppModDP)
{
    IFXRESULT result = IFX_OK;
    ...
    if (m_NumModifiers > 1)
    {
        ...
        if (pMod)
        {
            if (m_pDataPacketState[1].m_Enabled)
            {
                ...
                result = pMod->SetDataPacket(GetBaseDataPacketNR(), m_pDataPacketState[1].m_pDataPacket);
            }
            else
            {
...
IFXRESULT IFXModifierChainState::NotifyActive()
{
    ...
    IFXASSERT(m_pDataPacketState);
    U32 stage;
    for (stage = 1; stage < m_NumModifiers && IFXSUCCESS(result); stage++)
    {
        if (m_pDataPacketState[stage].m_Enabled)
        {
...
IFXRESULT IFXModifierChainState::BuildModifierDataPacket(U32 in_ModIdx, BOOL in_bReqValidation)
{
    IFXRESULT result = IFX_OK;
    ...
    if (IFXSUCCESS(result))
    {
        result = pMod->GetOutputs(pOutputs, uOutputCount, upOutputUnchangedAttrs);
    }
    ...
    if (IFXSUCCESS(result))
    {
        result = BMDPConfigureOutputs(in_ModIdx, uOutputCount, pOutputIndexes);
    }
    ...
    while (o-- && IFXSUCCESS(result))
    {
        ...
        else
        {
            ...
            if (IFXSUCCESS(result))
            {
                result = BMDPConsumeElements(in_ModIdx, pOutputIndexes[o], upOutputUnchangedAttrs ? upOutputUnchangedAttrs[o] : 0);
            }
        }
    }
    ...
    while (o-- && IFXSUCCESS(result))
    {
        ...
        if (IFXSUCCESS(result))
        {
            result = pMod->GetDependencies(pOutputs[o], pInputs, uInputCount, pOutputDependencies, uOutputDependencyCount, upOutputDependencyAttrs);
        }
        ...
        if (IFXSUCCESS(result))
        {
            result = BMDPScheduleInvalidations(in_ModIdx, pOutputIndexes[o], pOutputs[o], pInputs, uInputCount);
        }
        ...
        if (IFXSUCCESS(result))
        {
            result = BMDPSetOutputDeps(in_ModIdx, pOutputIndexes[o], pOutputs[o], pOutputDependencies, uOutputDependencyCount, upOutputDependencyAttrs);
        }
    }
...
IFXRESULT IFXModifierChainState::BMDPVerifyInputs(U32 in_ModIdx, IFXModifier* pMod, IFXDID** ppOutputs, U32 NumOutputs)
{
    IFXRESULT result = IFX_OK;
    IFXGUID** pInputs = NULL;
    U32 uInputCount = 0;
    ...
    {
        result = pMod->GetDependencies(ppOutputs[i], pInputs, uInputCount, pOutputDependencies, uOutputDependencyCount, upOutputDependencyAttrs);
...
IFXRESULT IFXModifierChainState::BMDPAddOutputs(U32 in_ModIdx, IFXDID** in_ppOutputs, U32 in_uNumOutputs, U32* pOutputIndices)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT IFXModifierChainState::BMDPConfigureOutputs(U32 in_ModIdx, U32 in_uNumOutputs, U32* pOutputIndices)
{
    IFXRESULT result = IFX_OK;
    U32 o = in_uNumOutputs;
    ...
    while (o-- && IFXSUCCESS(result))
    {
        ...
        { // if all renderables
            ...
            IFXDataElementState* pSrcDEStates = m_pDataPacketState[in_ModIdx - 1].m_pDataElements;
...
IFXRESULT IFXModifierChainState::BMDPConsumeElements(U32 in_ModIdx, U32 in_OutputIdx, U32 in_UnChangedAttrs)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT IFXModifierChainState::BMDPScheduleInvalidations(U32 in_ModIdx, U32 uOutputIdx, IFXDID* pOutputDid, IFXDID** in_ppInputs, U32 uInputCount)
{
    IFXRESULT result = IFX_OK;
    ...
    while (e-- && IFXSUCCESS(result))
    {
        ...
        { // When a DataTypeSpecifier(DTS) is an input, all dataElements of that type are implied inputs.
            ...
            while (cnt--)
            {
                    ...
                    && pPrevDE[cnt].State != IFXDATAELEMENTSTATE_CONSUMED)
                {
                    ...
                    m_pDataPacketState[pPrevDE[cnt].Generator].m_pDataElements[cnt].AddInv(in_ModIdx, uOutputIdx);
                }
            }
        }
        ...
        else
        {
            ...
            IFXDataElementState* pGenDE = &(m_pDataPacketState[GenIdx].m_pDataElements[uTmpIndex]);
    ...
    U32* upOutputDependencyAttrs)
{
    ...
    while (e-- && IFXSUCCESS(result))
    {
        ...
        { // When a DataTypeSpecifier(DTS) is provided, all dataElements of that type are implied output dependencies.
            ...
            while (cnt--)
            {
                    ...
                    && uOutputIdx != cnt)
                {
                    ...
                    m_pDepSeq[cnt].AddDependentElement(uOutputIdx, upOutputDependencyAttrs ? upOutputDependencyAttrs[e] : 0xFFFFFFFF);
        ...
        else
        {
            ...
            IFXDataElementState* pDEState = &(m_pDataPacketState[in_ModIdx].m_pDataElements[uTmpIndex]);

            if (IFXSUCCESS(result))
            { // "reversing the order of the link:  element x invalidates y, if y depends on x...
                // say that if the Output dependency is invalidated, or changed
                // the dependent output should have the same happen
                m_pDepSeq[uTmpIndex].AddDependentElement(uOutputIdx, upOutputDependencyAttrs ? upOutputDependencyAttrs[e] : 0xFFFFFFFF);
