...
IFXRESULT CIFXModifierChain::QueryInterface(IFXREFIID interfaceId, void** ppInterface)
{
    IFXRESULT result = IFX_OK;
...
IFXRESULT CIFXModifierChain::Initialize()
{
    ...
    if (IFXSUCCESS(result))
    {
        result = IFXCreateComponent(CID_IFXDidRegistry, IID_IFXDidRegistry, (void**)&m_pDidRegistry);
    }
...
IFXRESULT CIFXModifierChain::PrependModifierChain(IFXModifierChain* pInModifierChain)
{
    ...
    if ((IFXSUCCESS(result)) && (m_pModChainState->GetPrevChainNR() != pModChainInt))
    {
        ...
        result = BuildNewModifierState(
            pModChainInt, NULL, INVALID_DATAPACKET_INDEX, NULL, &pNewModChainState, FALSE, FALSE);
    ...
    BOOL isReqValidation)
{
    ...
    if (IFXSUCCESS(result))
    {
        ...
        result = BuildNewModifierState(
            m_pModChainState->GetPrevChainNR(), NULL, index, &rInModifier, &pNewState, FALSE, isReqValidation);
    }
...
IFXRESULT CIFXModifierChain::SetModifier(IFXModifier& rInModifier, U32 index, BOOL isReqValidation)
{
    IFXRESULT result = IFX_OK;
    IFXModifierChainState* pNewState = NULL;
    ...
    {
        ++index; // adjust the index to the correct location
        result = BuildNewModifierState(m_pModChainState->GetPrevChainNR(), NULL, index, &rInModifier, &pNewState, TRUE, isReqValidation);
    }
...
IFXRESULT CIFXModifierChain::RemoveModifier(U32 index)
{
    ...
    {
        ++index; // adjust the index to the correct location
        result = BuildNewModifierState(m_pModChainState->GetPrevChainNR(), NULL, index, NULL, &pNewState, FALSE, TRUE);
    }
...
IFXRESULT CIFXModifierChain::RebuildDataPackets(BOOL isReqValidation)
{
    ...
    {
        result = BuildNewModifierState(
            m_pModChainState->GetPrevChainNR(), NULL, INVALID_DATAPACKET_INDEX, NULL, &m_pCachedState, FALSE, isReqValidation);
    }
...

{
    ...
    if (IFXSUCCESS(result))
    {
        result = pNewState->Initialize(
            this, pInBaseChain, pInOverrideDP, NewSize - 1, m_pDidRegistry);
    }
    ...
    if (IFXSUCCESS(result))
    {
        U32 OldStateIdx = 1;
        U32 NewStateIdx = 1;
        while (IFXSUCCESS(result) && (NewStateIdx < NewSize) && (OldStateIdx < OldSize))
        {
            ...
            else
            {
                ...
                result = pNewState->SetModifier(
                    NewStateIdx, pMod, m_pModChainState->GetModifierEnabled(OldStateIdx));
                if (IFXSUCCESS(result))
                {
                    ++NewStateIdx;
                    ++OldStateIdx;
    ...
    IFXModifierChainState* pInNewState)
{
    ...
    if (IFXSUCCESS(result))
    {
        bNewPrevChain = pInNewState->GetPrevChainNR() != m_pModChainState->GetPrevChainNR();
    ...
    IFXREFIID rIType)
{
    ...
    if ((IFXUnknown*)pInSubject == (IFXUnknown*)m_pClockSubjectNR)
    {
        ...
        { //  this where time notifications will come in ->
            ...
            IFXDataElementState* pDEState = &(m_pModChainState->m_pDataPacketState->m_pDataElements[TIME_ELEMENT_INDEX]);
...
IFXRESULT CIFXModifierChain::ProcessDependencies(U32 dataElementIndex, U32 modifierIndex)
{
    IFXRESULT result = IFX_OK;
            ...
            if (pDEState->State != IFXDATAELEMENTSTATE_CONSUMED)
            {
                ...
                else if (pDEState->Generator == 0)
                {
                    ...
                    if (dataElementIndex == TIME_ELEMENT_INDEX)
                    {
                        ...
                        m_Time = m_pClockNR->GetSimulationTime();
                        pGenDEState = &(m_pModChainState->m_pDataPacketState->m_pDataElements[TIME_ELEMENT_INDEX]);
                ...
                else
                {
                    ...
                    {
                        pGenDEState->State = IFXDATAELEMENTSTATE_VALID;
                        IFXASSERT(!dataElementIndex || (pGenData && (pGenDEState->pValue == pGenData)));
                    }
...
IFXRESULT CIFXModifierChain::Invalidate(U32 dataElementIndex, U32 modifierIndex)
{
    IFXRESULT result = IFX_OK;
    ...
    if (IFXSUCCESS(result))
    {
        ...
        if (IFXSUCCESS(result))
        {
            ...
            s_pInvState[s_invDepth].pDEState = &(pDataPacketState[modifierIndex].m_pDataElements[dataElementIndex]);
        ...
        while (IFXSUCCESS(result) && s_invDepth > StartDepth)
        {
            ...
            IFXModifierDataPacketInternal* pDP = pDataPacketState[pCurIterState->ModIdx].m_pDataPacket;
            IFXDidInvElement* pInvEl = &(pCurIterState->pDEState->m_pInvSeq[pCurIterState->InvIdx]);
            ...
            {
                IFXDataPacketState* pTrgDPState = &(pDataPacketState[pInvEl->uMIndex]);
                IFXDataElementState* pTrgDEState = &(pTrgDPState->m_pDataElements[pInvEl->uEIndex]);
                ...
                { // only invalidate if valid
                    ...
                    { // if this element has invalidations
                        ...
                        s_pInvState[s_invDepth].pDEState = &(pDataPacketState[pInvEl->uMIndex].m_pDataElements[pInvEl->uEIndex]);
                        s_pInvState[s_invDepth].InvIdx = 0;
                    }
                }
            ...
            {
                IFXDataPacketState* pTrgDPState = &(pDataPacketState[pCurIterState->ModIdx]);
                IFXDataElementState* pTrgDEState = &(pTrgDPState->m_pDataElements[pInvEl->uEIndex]);
                        ...
                        */
                        if (chainIndex != m_pModChainState->NumModifiers() - 1)
                        {
                                 ...
                                 cnt++)
                            {
                                ...
                                pDEState = &(pDPState->m_pDataElements[pInvEl->uEIndex]);
                            ...
                            IFXModifierDataPacketInternal* pLastDP = m_pModChainState->GetOutputDataPacketNR();
                            if (pLastDP != NULL)
                            {
                ...
                U32 AppChainIter = 0;
                IFXModifierChainInternal** pAppendedChain = m_appendedChains.Begin(AppChainIter);
                while (pAppendedChain)
                {
                    result = (*pAppendedChain)->Invalidate(pInvEl->uEIndex, PROXY_DATAPACKET_INDEX);
