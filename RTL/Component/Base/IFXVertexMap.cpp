...
IFXRESULT IFXVertexMap::ConcatenateVertexMap(IFXVertexMap* pVMap, IFXVertexMap** ppNewMap)
{
    IFXRESULT result = IFX_OK;
    BOOL duplicate;
        ...
        for (i = 0; i < numMapEntries; i++)
        {
            ...
            for (j = 0; j < numCopies; j++)
            {
                ...
                for (k = 0; k < numCopies2; k++)
                {
                    ...
                    {
                        IFXASSERT(l < MaxTemp);
                        if (pTemp[l].vert == pPotential->vert && pTemp[l].mesh == pPotential->mesh)
                        {
                            duplicate = TRUE;
                            break;
                        }
...
IFXRESULT IFXVertexMap::AddVertex(U32 origVertexIndex, U32 indexIFXMesh, U32 indexIFXVertex)
{
    IFXRESULT result = IFX_OK;
