        ...
        if (ret)
        {
            do
            {
                ...
                do
                {
                    ...
                    m_ppCoincidentVertexID[meshIndex][vertIndex] = coincidentVertexID;
...
IFXRESULT IFXCoincidentVertexMap::Initialize(IFXMeshGroup* pMeshGroup, IFXVertexMapGroup* pVertexMapGroup)
{
    IFXRESULT result = IFX_OK;
    U32 meshIndex;
            ...
            for (origMeshIndex = 0; origMeshIndex < numMaps; ++origMeshIndex)
            {
                     ...
                     ++origVertexIndex)
                {
                    ...
                    for (copyIndex = 0; copyIndex < numCopies; ++copyIndex)
                    {
                        ...
                        pVertexMap->GetVertexCopy(origVertexIndex, copyIndex, &meshIndex, &vertexIndex);

                        // Verify that vertex map values are valid
                        IFXASSERT(meshIndex < m_numMeshes);

                        m_ppCoincidentVertexID[meshIndex][vertexIndex] = m_numVertexID + origVertexIndex;
                    }
                }
