...
void CIFXPointSetDecoder::CalculatePredictedColorAtSplitPos(BOOL bDiffuse, U32 currPosInd, U32 splitPosInd, IFXVector4& v4PredictedVertColor)
{
    IFXASSERT(m_pPointSetDesc);
    ...
    if (currPosInd == 0)
    {
        return; // no Points defined yet - so predicted is just zero normal
    }

    if ((m_pPointSetDesc->m_numDiffuseColors == 0 && bDiffuse) || (m_pPointSetDesc->m_numSpecularColors == 0 && (!bDiffuse)))
    {
        return;
    }
...
void CIFXPointSetDecoder::ReconstructColorDifference(BOOL bDiffuseColor, U8& u8Signs, U32& udR, U32& udG, U32& udB, U32& udA, IFXVector4& v4VertColorDifference)
{
            ...
            if (m_pMaxPointSetDesc->m_numTexCoords)
            {
                ...
                for (iTex = 0; iTex < currMaterial.m_uNumTextureLayers; iTex++)
                {
                    ...
                    if (u8DupColorPointIndexFlag == 0)
                    {
                        ...
                        IFXCHECKX(m_pAuthorPointSet->SetTexCoord(m_pPointSetDesc->m_numTexCoords, &v4PredictedTexCoord));
...
void CIFXPointSetDecoder::CalculatePredictedTexCoordAtSplitPos(U32 texInd, U32 currPosInd, U32 splitPosInd, IFXVector4& v4PredictedTexCoord)
{
    IFXASSERT(m_pPointSetDesc);
    ...
    if (texInd > m_pPointSetDesc->m_numTexCoords)
    {
        return;
    }
...
void CIFXPointSetDecoder::ReconstructPosition(U8 u8Signs, U32 udX, U32 udY, U32 udZ, IFXVector3& vReconstructedPosition)
{
    if (u8Signs & 1)
    {
...
void CIFXPointSetDecoder::ReconstructNormal(U8 u8Signs, U32 udX, U32 udY, U32 udZ, IFXVector3& vReconstructedNormal)
{
    if (u8Signs & 1)
    {
...
void CIFXPointSetDecoder::ReconstructTexCoord(U8 u8Signs, U32 udR, U32 udG, U32 udB, U32 udA, IFXVector4& v4VertTexCoord)
{
    if (u8Signs & 1)
    {
