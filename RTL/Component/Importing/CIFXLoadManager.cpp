...
IFXRESULT CIFXLoadManager::QueryInterface(IFXREFIID interfaceId, void** ppInterface)
{
    IFXRESULT rc = IFX_OK;
...
//-----------------------------------------------------------------------------
IFXRESULT CIFXLoadManager::Load(IFXReadBuffer* pReadBuffer, const BOOL bAtomicLoad, const IFXString& sScope, const IFXCollisionPolicy uCollisionPolicy, const U32 uLoadId)
{
    IFXObjectFilters tmpObjectFilters;
    return Load(pReadBuffer, bAtomicLoad, sScope, uCollisionPolicy, uLoadId, tmpObjectFilters, PALETTE_DEFAULT_GROUP, FALSE);
}

IFXRESULT CIFXLoadManager::Load(IFXReadBuffer* pReadBuffer, const BOOL bAtomicLoad, const IFXString& sScope, const IFXCollisionPolicy uCollisionPolicy, const U32 uLoadId, const IFXObjectFilters& rObjectFilters, const IFXString& sWorldAlias, const BOOL bExternalLoad)
{
    IFXRESULT rc = IFX_OK;
...
void CIFXLoadManager::ProcessFileHeaderBlockX(IFXDataBlockX& rDataBlockX)
{
    ...
    if ((uFileVersion == FileHeader_VersionCurrent) || ((uFileVersion & 0xFFFFFF00) == (FileHeader_VersionCurrent & 0xFFFFFF00)) || ((uFileVersion & 0xFFFFFF00) == FileHeader_Version_1_2_Gold) || (uFileVersion == FileHeader_VersionFirstEdition) || (uFileVersion == FileHeader_VersionFirstEditionBeta))
    {
...
void CIFXLoadManager::ProcessPriorityUpdateBlockX(IFXDataBlockX& rDataBlockX)
{
    ...
    if (m_uPriority > 0)
    {
        ...
        // transfer now.
        if (IFX_LOAD_STATE_HEADER_LOADING == m_eState || IFX_LOAD_STATE_UNLOADED == m_eState || IFX_LOAD_STATE_ERR == m_eState)
        {
            m_bTransferNow = TRUE;
        }
        m_eState = IFX_LOAD_STATE_MEDIA_LOADING;
        ...
        if (isDummy || IFXSUCCESS(pDecoderX->QueryInterface(IID_IFXExtensionDecoderX, (void**)&pExtDec)))
        {
            ...
            for (i = 0; i < limit && !isRegistered; i++)
            {
                ...
                if (isRegistered)
                {
                    ...
                    {
                        pExtDec->SetContBlockTypes(m_extensionData.GetElement(i).m_pContBlockTypes, m_extensionData.GetElement(i).m_contCount);
                    }
                }
            }
        }
