...
void CIFXLineSetDecoder::ProcessAuthorLineSetContinuationBlockX()
{
    ...
    if (m_uWIPi == 0)
    {
        ...
        // that will help decode data from continuation block
        {
            ...
            IFXCHECKX(IFXCreateComponent(CID_IFXAuthorLineSetAnalyzer, IID_IFXAuthorLineSetAnalyzer, (void**)&m_pLineSetAnalyzer));
...
void CIFXLineSetDecoder::CalculatePredictedNormalAtSplitPosX(U32 currPosInd, U32 splitPosInd, IFXVector3& vPredictedNormal)
{
    ...
    if (m_pLineSetDesc->m_numLines == 0 || m_pLineSetDesc->m_numPositions == 0)
    {
        return;
    }
    ...
    if (currPosInd == 0)
    {
        return; // no lines deifed yet - so predicted is just zero normal
    }
...
void CIFXLineSetDecoder::CalculatePredictedColorAtSplitPosX(BOOL bDiffuse, U32 currPosInd, U32 splitPosInd, IFXVector4& v4PredictedVertColor)
{
    IFXASSERT(m_pLineSetDesc);
    IFXASSERT(m_pLineSetAnalyzer);
    IFXASSERT(m_pLineSetAnalyzer->IsInitialized());
    ...
    if ((m_pLineSetDesc->m_numDiffuseColors == 0 && bDiffuse) || (m_pLineSetDesc->m_numSpecularColors == 0 && (!bDiffuse)))
    {
        return;
    }
...
void CIFXLineSetDecoder::CalculatePredictedTexCoordAtSplitPosX(U32 texInd, U32 currPosInd, U32 splitPosInd, IFXVector4& v4PredictedTexCoord)
{
    IFXASSERT(m_pLineSetDesc);
    IFXASSERT(m_pLineSetAnalyzer);
    IFXASSERT(m_pLineSetAnalyzer->IsInitialized());
    ...
    if (texInd >= m_pLineSetDesc->m_numTexCoords)
    {
        return;
    }
...
void CIFXLineSetDecoder::ReconstructColorDifference(BOOL bDiffuseColor, U8& u8Signs, U32& udR, U32& udG, U32& udB, U32& udA, IFXVector4& v4VertColorDifference)
{
                ...
                if (m_pMaxLineSetDesc->m_numTexCoords)
                {
                    ...
                    for (iTex = 0; iTex < currMaterial.m_uNumTextureLayers; iTex++)
                    {
                        ...
                        if (u8DupAttribLineIndexFlag == 0)
                        {
                            ...
                            IFXCHECKX(m_pAuthorLineSet->SetTexCoord(m_pLineSetDesc->m_numTexCoords, &v4PredictedTexCoord));
...
void CIFXLineSetDecoder::ReconstructPosition(U8 u8Signs, U32 udX, U32 udY, U32 udZ, IFXVector3& vReconstructedPosition)
{
    if (u8Signs & 1)
    {
...
void CIFXLineSetDecoder::ReconstructNormal(U8 u8Signs, U32 udX, U32 udY, U32 udZ, IFXVector3& vReconstructedNormal)
{
    if (u8Signs & 1)
    {
...
void CIFXLineSetDecoder::ReconstructTexCoord(U8 u8Signs, U32 udR, U32 udG, U32 udB, U32 udA, IFXVector4& v4VertTexCoord)
{
    if (u8Signs & 1)
    {
