    ...
    if (IFXSUCCESS(result))
    {
        ...
        if (m_pTexture->IsExternal() == FALSE)
        {
            ...
            if (IFXSUCCESS(result))
            {
                ...
                if ((rImageType == IDTF_IMAGE_TYPE_RGB && channels != 3) || (rImageType == IDTF_IMAGE_TYPE_RGBA && channels != 4) /*||
                ...
                if (IFXSUCCESS(result) && 0 != m_limit)
                {
                    ...
                    if ((sourceImageInfo.m_width > m_limit) || (sourceImageInfo.m_height > m_limit))
#ifndef LIBIDTF
