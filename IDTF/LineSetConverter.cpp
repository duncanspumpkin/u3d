...
IFXRESULT LineSetConverter::ConvertLineSet(IFXAuthorLineSet** ppModel)
{
    ...
    if (IFXSUCCESS(result) && allocationDescriptor.m_numNormals > 0 && allocationDescriptor.m_numLines > 0)
    {
        IFXU32Line* pLine = NULL;
    ...
    // coordinate array)
    if (IFXSUCCESS(result) && (allocationDescriptor.m_numTexCoords > 0) && allocationDescriptor.m_numLines > 0)
    {
        U32 layers = 0;
    ...
    */
    ...
    if (IFXSUCCESS(result) && allocationDescriptor.m_numLines > 0 && allocationDescriptor.m_numDiffuseColors > 0)
    {
        IFXU32Line* pLine = NULL;
    ...
    if (IFXSUCCESS(result) && allocationDescriptor.m_numLines > 0 && allocationDescriptor.m_numSpecularColors > 0)
    {
        IFXU32Line* pLine = NULL;
