...
/**
        ...
        @return IFXRESULT           Return status of this method.
*/
IFXRESULT MeshConverter::ConvertMesh(IFXAuthorMesh** ppMesh, U32* pMinimumMeshResolution)
{
    IFXRESULT result = IFX_OK;
    ...
    if (IFXSUCCESS(result) && allocationDescriptor.NumFaces > 0)
    {
        ...
        if (NULL != pFace)
        {
            ...
            {
                const Int3& facePosition = m_pIDTFResource->m_facePositions.GetElementConst(i);
    ...
    if (IFXSUCCESS(result) && allocationDescriptor.NumNormals > 0 && allocationDescriptor.NumFaces > 0)
    {
        IFXAuthorFace* pFace = NULL;
        ...
        if (NULL != pFace)
        {
            for (i = 0; i < allocationDescriptor.NumFaces; ++i)
            {
                const Int3& faceNormal = m_pIDTFResource->m_faceNormals.GetElementConst(i);
    ...
    // coordinate array)
    if (IFXSUCCESS(result) && (allocationDescriptor.NumTexCoords > 0) && allocationDescriptor.NumFaces > 0)
    {
        U32 layers = 0;
        ...
        for (i = 0; i < allocationDescriptor.NumFaces && IFXSUCCESS(result); ++i)
        {
            ...
            const ShadingDescription& rShading = m_pIDTFResource->m_shadingDescriptions.GetShadingDescription(rShadingIndex);
            layers = rShading.GetTextureLayerCount();

            const FaceTexCoords& faceTexCoords = m_pIDTFResource->m_faceTextureCoords.GetElementConst(i);
    ...
    */
    ...
    if (IFXSUCCESS(result) && allocationDescriptor.NumFaces > 0 && allocationDescriptor.NumDiffuseColors > 0)
    {
        IFXAuthorFace* pFace = NULL;
    ...
    if (IFXSUCCESS(result) && allocationDescriptor.NumFaces > 0 && allocationDescriptor.NumSpecularColors > 0)
    {
        IFXAuthorFace* pFace = NULL;
