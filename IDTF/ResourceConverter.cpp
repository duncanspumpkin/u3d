...
{
    IFXRESULT result = IFX_OK;
    const MotionResourceList& rResourceList = m_pResources->GetMotionResourceList();
    ...
    for (i = 0; i < resourceCount && IFXSUCCESS(result); ++i)
    {
        ...
        U32 j;
        for (j = 0;
             j < rIDTFResource.GetMotionTrackCount() && IFXSUCCESS(result) && NULL != pMotionResource;
             ++j)
        {
            U32 trackId = 0;
...
IFXRESULT ResourceConverter::ConvertTextureResources()
{
    ...
    const TextureResourceList& rResourceList = m_pResources->GetTextureResourceList();
    const U32 resourceCount = rResourceList.GetResourceCount();
#ifdef LIBIDTF
    IFXString dbgMessage;
#endif
...
IFXRESULT ResourceConverter::ConvertMaterialResources()
{
    ...
    const MaterialResourceList& rResourceList = m_pResources->GetMaterialResourceList();
    const U32 resourceCount = rResourceList.GetResourceCount();
#ifdef LIBIDTF
    IFXString dbgMessage;
#endif
...
{
    IFXRESULT result = IFX_OK;
    const ShaderResourceList& rResourceList = m_pResources->GetShaderResourceList();
    const U32 resourceCount = rResourceList.GetResourceCount();
#ifdef LIBIDTF
    IFXString dbgMessage;
#endif
...
{
    IFXRESULT result = IFX_OK;
    const ModelResourceList& rResourceList = m_pResources->GetModelResourceList();
    const U32 resourceCount = rResourceList.GetResourceCount();
#ifdef LIBIDTF
    IFXString dbgMessage;
#endif
    ...
    for (i = 0; i < resourceCount && IFXSUCCESS(result); ++i)
    {
        ...
        {
            const MeshResource* pMeshResource = static_cast<const MeshResource*>(pIDTFResource);
        ...
        {
            const LineSetResource* pLineSetResource = static_cast<const LineSetResource*>(pIDTFResource);
            LineSetConverter converter(pLineSetResource, m_pSceneUtils);
            result = converter.Convert();
        }
        else if (pIDTFResource->m_type == IDTF_POINT_SET)
        {
            const PointSetResource* pPointSetResource = static_cast<const PointSetResource*>(pIDTFResource);
...
{
    IFXRESULT result = IFX_OK;
    const LightResourceList& rResourceList = m_pResources->GetLightResourceList();
    const U32 resourceCount = rResourceList.GetResourceCount();
#ifdef LIBIDTF
    IFXString dbgMessage;
#endif
...
{
    IFXRESULT result = IFX_OK;
    const ViewResourceList& rResourceList = m_pResources->GetViewResourceList();
