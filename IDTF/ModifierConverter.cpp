...
ModifierConverter::ModifierConverter(ModifierList* pModifierList, SceneUtilities* pSceneUtils)
    : m_pModifierList(pModifierList)
    ...
    const AnimationModifier* pIDTFModifier)
{
    ...
    if (IFXSUCCESS(result))
    {
        ...
        for (i = 0;
             ...
             ++i)
        {
            const MotionInfo& rMotionInfo = pIDTFModifier->GetMotionInfo(i);
    ...
    const BoneWeightModifier* pIDTFModifier)
{
    ...
    if (pModifier && IFXSUCCESS(result))
    {
        ...
        const U32& positionCount = pIDTFModifier->GetBoneWeightListCount();
        ...
        if (IFXSUCCESS(result))
        {
            ...
            if (IFXSUCCESS(result))
            {
                IFXAuthorMeshDesc* pAuthorMeshDesc = (IFXAuthorMeshDesc*)pAuthorMesh->GetMaxMeshDesc();
        ...
        {
            const BoneWeightList& rIDTFBoneWeightList = pIDTFModifier->GetBoneWeightList(i);
            const U32& boneWeightCount = rIDTFBoneWeightList.GetBoneIndexCount();
            ...
            if (boneWeightCount > 0)
            {
                ...
                if (NULL != pBoneIDs && NULL != pBoneWeights)
                {
                    ...
                    for (j = 0; j < boneWeightCount - 1; ++j)
                    {
                        ...
                        pBoneWeights[j] = inverseQuant * rIDTFBoneWeightList.GetBoneWeight(j);
                        boneWeightSum += pBoneWeights[j];
                    }

                    pBoneIDs[boneWeightCount - 1] = rIDTFBoneWeightList.GetBoneIndex(boneWeightCount - 1);
...
IFXRESULT ModifierConverter::ConvertCLODModifier(const CLODModifier* pIDTFModifier)
{
    ...
    if (IFXSUCCESS(result))
    {
        const IFXString& rAutoLODControl = pIDTFModifier->GetAutoLODControl();
...
IFXRESULT ModifierConverter::ConvertGlyphModifier(const GlyphModifier* pIDTFModifier)
{
         ...
         ++i)
    {
        ...
        if (NULL != pCommand)
        {
            ...
            {
                const LineTo* pLineTo = static_cast<const LineTo*>(pCommand);
            ...
            {
                const MoveTo* pMoveTo = static_cast<const MoveTo*>(pCommand);
            ...
            {
                const CurveTo* pCurveTo = static_cast<const CurveTo*>(pCommand);
            ...
            {
                const EndGlyph* pEndGlyph = static_cast<const EndGlyph*>(pCommand);
