...
{
    if (!m_plain && isIndent == true)
    {
        indent();
    }
    {
        toutf(id);
        fprintf(m_pFileHandle, "%s ", u8id);
        freetmp(id);
    }
...
void IFXWriter::output(const IFXCHAR* id, const IFXCHAR* param, bool isIndent)
{
    ...
    }
    {
        toutf(id);
        toutf(param);
        fprintf(m_pFileHandle, "%s \"%s\"", u8id, u8param);
        freetmp(param);
        freetmp(id);
    }
}
    ...
    if (FilterCount > 0)
    {
        ...
        for (U32 FilterIndex = 0; FilterIndex < FilterCount; FilterIndex++)
        {
            ...
            else if (FilterType == IDTF_FILTER_TYPE_TYPE)
            {
                indent();
                {
                    const IFXCHAR* ptmp = IDTF_FILTER_OBJECT_TYPE;
                    toutf(ptmp);
                    fprintf(m_pFileHandle, "%s %X", u8ptmp, rFilter.GetObjectType());
                    freetmp(ptmp);
                }
            }
            end();
...
void IFXWriter::exportTextureResource(const Texture& rTextureResource)
{
    ...
    if (rTextureResource.GetImageFormatCount() > 0)
    {
        ...
        if (!(ImageFormatCount == 1 && rTextureResource.IsExternal() == false && rTextureResource.GetImageFormat(0).m_compressionType == IDTF_IMAGE_COMPRESSION_TYPE_JPEG24 && rTextureResource.GetImageFormat(0).m_alpha == IDTF_FALSE && rTextureResource.GetImageFormat(0).m_red == IDTF_TRUE && rTextureResource.GetImageFormat(0).m_green == IDTF_TRUE && rTextureResource.GetImageFormat(0).m_blue == IDTF_TRUE && rTextureResource.GetImageFormat(0).m_luminance == IDTF_FALSE)
            || m_exportDefaults)
        {
